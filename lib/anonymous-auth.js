...
function validateAnonymousObjectAccess(req, res, next) {
    ...
    if (req.metadata && req.metadata.properties && req.metadata.properties.roles && Array.isArray(req.metadata.properties.roles)) {
        ...
        if (roleUuids.length > 0 && req.mahi) {
            req.mahi.getName({
                account: accountName,
                type: 'role',
                uuids: roleUuids
                ...
                // Note: Object.values() not available in older Node.js, use manual iteration
                var roleNamesList = [];
                if (roleNames) {
                    // The roleNames response format appears to be { "uuid": "name" }
