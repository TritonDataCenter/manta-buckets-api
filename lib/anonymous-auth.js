...
var ANONYMOUS_ACCESS_CONFIG = {
    ...
    allowedBuckets: (process.env.MANTA_ANONYMOUS_BUCKETS || 'public').split(','),
    strictMode: process.env.MANTA_ANONYMOUS_STRICT_MODE !== 'false', // Default true
    maxRequestsPerMinute: parseInt(process.env.MANTA_ANONYMOUS_RATE_LIMIT, 10) || 100,
...
function lookupBucketRoles(req, bucketName, callback) {
    ...
    function fallbackToNaming() {
        // SECURITY: Only allow exact bucket name matches, not substring matches
...
function validateAnonymousAccess(req, res, next) {
    ...
    // SECURITY: For object requests, still require public bucket OR explicit metadata
    // Don't grant access based on bucket name heuristics
    var shouldGrantAccess = isPublicBucket;
...
function validateAnonymousObjectAccess(req, res, next) {
    ...
    function continueValidation() {
        ...
        // should be public
        // This is a workaround since the metadata-based role system isn't
        // working as expected
        var objectKey = accountName + '/' + bucketName + '/' + objectName;
        // SECURITY: Remove heuristic-based access - only use explicit metadata
        // Objects must have explicit public-read role or be in exact "public" bucket
        isMarkedPublicByACL = false; // Disable heuristic access entirely
        ...
        if (hasPublicReader || (ANONYMOUS_ACCESS_CONFIG.strictMode ? 
            (bucketName === 'public') : (isInPublicBucket || isMarkedPublicByACL))) {
