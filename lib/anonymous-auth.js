...
function validateAnonymousObjectAccess(req, res, next) {
    ...
    if (req.metadata && req.metadata.properties && req.metadata.properties.roles && Array.isArray(req.metadata.properties.roles)) {
        ...
        if (hasPublicReader || uuidRoles.length === 0 || !req.mahi) {
            ...
            }, 'validateAnonymousObjectAccess: processed literal and UUID roles');
            continueValidation();
            return;
        }
        ...
        req.mahi.getName({
            ...
            if (err) {
                req.log.warn(err, 'validateAnonymousObjectAccess: failed to resolve role UUIDs');
    ...
    function continueValidation() {
        ...
        req.log.debug({
            ...
            roles: req.metadata && req.metadata.properties ? req.metadata.properties.roles : [],
            hasPublicReader: hasPublicReader
