...
var ANONYMOUS_ACCESS_CONFIG = {
    ...
    allowedBuckets: (process.env.MANTA_ANONYMOUS_BUCKETS || 'public').
        split(','),
    // Default true
    strictMode: process.env.MANTA_ANONYMOUS_STRICT_MODE !== 'false',
    maxRequestsPerMinute:
    parseInt(process.env.MANTA_ANONYMOUS_RATE_LIMIT, 10) || 100,
...
function validateAnonymousAccess(req, res, next) {
    ...
    // SECURITY: For object requests, still require public bucket OR
    // explicit metadata
...
function validateAnonymousObjectAccess(req, res, next) {
    ...
    function continueValidation() {
        ...
        // Objects must have explicit public-read role or
        // be in exact "public" bucket
        ...
        if (hasPublicReader || (ANONYMOUS_ACCESS_CONFIG.strictMode ?
            (bucketName === 'public') :
                                (isInPublicBucket || isMarkedPublicByACL))) {
