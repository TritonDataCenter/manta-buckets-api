 ...
 * event.  I.e.:
 *
 * server.on('after', restify.plugins.auditLogger({ log: myAuditStream }));
 *
...
function auditLogger(options) {
    ...
    function audit(req, res, route, err) {
        ...
        var sanitizedCaller = null;

        if (req.caller) {
            sanitizedCaller = {
                login: req.caller.login || null,
                uuid: req.caller.uuid || null,
                groups: req.caller.groups || null,
                user: req.caller.user || null
            };
            if (req.caller.account) {
                sanitizedCaller.account = {
                    type: req.caller.account.type || null,
                    uuid: req.caller.account.uuid || null,
                    login: req.caller.account.login || null,
                    groups: req.caller.account.groups || null,
                    approved_for_provisioning:
                    req.caller.account.approved_for_provisioning || null,
                    triton_cns_enabled: req.caller.account.triton_cns_enabled ||
                        null,
                    keys: req.caller.account.keys || null,
                    key_info: req.caller.account.key_info || null,
                    isOperator: req.caller.account.isOperator || false
                };
            }
        }
        ...
        if (req._timeToLastByte !== undefined &&
            ...
            obj._auditData = true;
            obj.dataLatency = req._timeToLastByte - req.time();
            obj.dataSize = req._totalBytes;
        }

        if (req.time() !== undefined &&
            req._timeAtFirstByte !== undefined) {
            obj.latencyToFirstByte = req._timeAtFirstByte - req.time();
        }
        ...
        // Add sanitized caller to the log object
        if (sanitizedCaller) {
            var reqCopy = {};
            for (var key in req) {
            ...
            reqCopy.caller = sanitizedCaller;
            obj.req = reqCopy;
        }

        log.debug(obj, 'handled: %d', res.statusCode);
