...
function checkAuthzScheme(req, res, next) {
    ...
    if (!ok) {
        req.log.info('S3_AUTH_DEBUG: AUTHENTICATION FAILED'+
...
function sigv4Handler(req, res, next) {
    ...
    // to sign the request
    // if pathPart is just a bucket request then add /.
    // This is true for third party s3 clients, like s3cmd.
    var regex = /\/[^/]+\.(?:$|\/)|\/[^/]+\/[^/]+/;
    if ((!('user-agent' in req.headers ) ||
       (req.headers['user-agent'] &&
       !req.headers['user-agent'].toLowerCase().includes('awscli'))) &&
