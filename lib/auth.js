...
function loadOwnerFromPath(req, p, next) {
    ...
    req.mahi.getUser(user, account, fallback, function (err, owner) {
        ...
        // Handle public-reader anonymous access
        if (req.isAnonymousAccess && req.caller && req.caller.publicReader) {
            req.log.debug({
                isAnonymousAccess: req.isAnonymousAccess,
                publicReader: req.caller.publicReader,
                callerRoles: req.caller.roles
            }, 'Allowing public-reader anonymous access in loadOwner');
            next();
            return;
...
function authorize(req, res, next) {
    ...
    if (req.isAnonymousAccess && req.caller && req.caller.publicReader) {
        ...
        log.debug({
            ...
            callerRoles: req.caller.roles,
            action: action
        }, 'authorize: bypassing Mahi authorization for public reader access');

        // For public reader anonymous access, completely bypass Mahi authorization
        // The bucket public status was already verified in the anonymous access handler
        if ((action === 'getobject' || action === 'getdirectory' || action === 'getbucket' || !action)) {
            log.debug('authorize: allowing public access - bypassing Mahi authorization');
            next();
            return;
        } else {
