...
function sigv4Handler(req, res, next) {
    ...
    // However, continuation-token parameters must preserve their exact encoding
    // as sent by the client to match the signature
    if (queryPart) {
        try {
            // Check if this is a continuation token request - if so,
            // preserve encoding
            if (queryPart.includes('continuation-token=')) {
                // For continuation token requests, don't decode to
                // preserve signature match
                urlForVerification = pathPart + '?' + queryPart;
                req.log.debug({
                    originalUrl: req.url,
                    finalUrl: urlForVerification,
                    reason: 'Continuation token present - preserving' +
                        ' original encoding'
