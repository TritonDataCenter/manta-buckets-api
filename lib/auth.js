...
function loadCaller(req, res, next) {
    ...
    if (req.isAnonymousAccess && req.caller && req.caller.publicReader) {
        req.log.debug('loadCaller: skipping for public reader anonymous access');
        next();
        return;
    }
...
function loadOwnerFromPath(req, p, next) {
    ...
    req.mahi.getUser(user, account, fallback, function (err, owner) {
        ...
        if ((req.isAnonymousAccess && req.caller && req.caller.publicReader) || 
            ...
            }, 'Allowing public-reader/potential anonymous access in loadOwner');
            next();
            return;
...
function authorize(req, res, next) {
    ...
    // Handle public bucket access for anonymous users - bypass all Mahi authorization
    // Also handle potential anonymous access that hasn't been validated yet
    if ((req.isAnonymousAccess && req.caller && req.caller.publicReader) || 
        req.potentialAnonymousAccess) {
        ...
        // For potential anonymous access, we need to defer authorization until after
        // bucket metadata is loaded and validated
        if (req.potentialAnonymousAccess) {
            log.debug('authorize: deferring authorization for potential anonymous access');
            next();
            return;
        }

        // For validated public reader anonymous access, completely bypass Mahi authorization
        // The bucket/object public status was already verified in the anonymous access handler
        if (req.isAnonymousAccess && req.caller.publicReader) {
            if ((action === 'getobject' || action === 'getdirectory' || action === 'getbucket' || !action)) {
                log.debug('authorize: allowing public access - bypassing Mahi authorization completely');
                next();
                return;
            } else {
                log.debug({action: action}, 'authorize: denying non-GET action for anonymous access');
                next(new AuthorizationError('anonymous', req.path(), 'Anonymous access only allowed for GET operations'));
                return;
            }
        }
        
        // If we reach here with anonymous access, something is wrong
        log.warn('authorize: unexpected state - anonymous access not properly handled');
        next(new AuthorizationError('anonymous', req.path(), 'Anonymous access configuration error'));
        return;
    }
