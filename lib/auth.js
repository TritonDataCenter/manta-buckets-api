...
// Helper function to safely send InvalidSignatureError without crashing
...
function convertS3PresignedToManta(req, res, next) {
    ...
    if (req.query['X-Amz-Algorithm'] && req.query['X-Amz-Signature'] &&
        ...
        log.info('convertS3PresignedToManta: detected S3' +
...
function checkIfPresigned(req, res, next) {
    ...
    if (hasAuthHeader || (!hasMantaPresigned && !hasS3Presigned)) {
        ...
        req.log.info('checkIfPresigned: detected presigned request');
        req._presigned = true;
        next();
    }
}
...
function preSignedUrl(req, res, next) {
    req.log.debug({isPresigned: req.isPresigned()},
    ...
    req.log.info('preSignedUrl: processing presigned URL request');
...
function sigv4Handler(req, res, next) {
    ...
    var shouldAddSlash = (!('user-agent' in req.headers) ||
       ...
       (req.headers['user-agent'].toLowerCase().includes('minio') ||
        ...
        req.headers['user-agent'].toLowerCase().includes('cyberduck')))) &&
        regex.test(pathPart) === false && pathPart !== '/' &&
        !pathPart.endsWith('/') && !isAwsCli;
...
function loadCaller(req, res, next) {
    ...
    if (accessKeyId && req.auth.method === 'presigned-s3') {
        ...
        // Try getUserById first (works for both sub-users and accounts in mahi)
        // If it fails with WrongTypeError, fallback to getAccountById
        req.mahi.getUserById(req.auth.accountid, function (err, data) {
            if (err) {
                // Check if this is a type mismatch
                // (account UUID passed to getUserById)
                if (err.message && err.message.
                    includes('expected to be type')) {
                    req.log.debug({
                        userUuid: req.auth.accountid,
                        error: err.message,
                        action: 'Trying getAccountById as fallback'
                    }, 'S3_AUTH_DEBUG: getUserById failed with type error,' +
                                  ' trying account lookup');

                    // Fallback to getAccountById for account owners
                    req.mahi.getAccountById(req.auth.accountid,
                        ...
                        if (err2) {
                            ...
                            ' - Both getUserById and getAccountById failed');
                        } else {
                            req.log.debug({
                                accessKeyId: accessKeyId,
                            ...
                            ' - Account lookup successful (fallback)');
                        }
                        gotCaller(err2, data2);
                    });
                    return;
                }

                // Other error, not a type mismatch
                req.log.debug({
                    error: err.message || err,
                    userUuid: req.auth.accountid,
                    accessKeyId: accessKeyId
                }, 'S3_AUTH_DEBUG: AUTHENTICATION FAILED'+
                ' - User lookup by UUID failed');
                gotCaller(err, data);
            } else {
                req.log.debug({
                    accessKeyId: accessKeyId,
                    userUuid: data ? data.uuid : 'unknown',
                    userLogin: data ? data.login : 'unknown',
                    userType: data && data.user ? 'sub-user' : 'account'
                }, 'S3_AUTH_DEBUG: AUTHENTICATION SUCCESS'+
                ' - User lookup by UUID successful');
                gotCaller(null, data);
            }
        });
