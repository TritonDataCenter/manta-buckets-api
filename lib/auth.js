...
function sigv4Handler(req, res, next) {
    ...
    if (req._rawBodyBuffer && req._originalContentMD5) {
        req.log.debug({
            ...
            contentMD5Match: req._originalContentMD5 === req.headers['content-md5']
        }, 'S3_AUTH_DEBUG: Using preserved raw body for signature verification');
        
        // If our calculated MD5 doesn't match the header, the client sent the wrong MD5
        // But we should use the client's MD5 for signature verification since that's what they signed
        if (req._originalContentMD5 !== req.headers['content-md5']) {
            ...
            }, 'S3_AUTH_DEBUG: Content-MD5 mismatch - client may have sent incorrect MD5');
        }
    }
    ...
    // Don't override content-md5 for SigV4 verification - must use client's signed value
    if (req._rawBodyBuffer && req._originalContentMD5) {
        // Only log the discrepancy if there is one, but preserve client's signed value
        if (req._originalContentMD5 !== req.headers['content-md5']) {
            req.log.warn({
                calculated: req._originalContentMD5,
                header: req.headers['content-md5'],
                action: 'Preserving client header for SigV4 verification'
            }, 'S3_AUTH_DEBUG: Content-MD5 mismatch - keeping client signed value');
        }
        // Keep the original client header value for signature verification
        // The client signed the request with this value, so mahi must verify with the same
    }
    ...
    if ('content-md5' in filteredHeaders) {
        filteredHeaders['edgecast-content-md5'] =
    ...
    var bucketName = null;
    if (virtualHostBucketName !== hostHeader && virtualHostBucketName.length > 0) {
    ...
    // If the URI path is the bucket name, some S3 clients sign the request with a
    // canonical URI ending with /. We must normalize the URL we send to mahi for
    // verification to match what the client signed.
    // AWS CLI does NOT add trailing slash, but other clients like s3cmd do.
    if (bucketName && pathPart === '/' + bucketName) {
        var isAwsCli = userAgent && userAgent.toLowerCase().includes('aws-cli');
        ...
        if (!isAwsCli) {
            req.log.debug({
                ...
                newPath: '/' + bucketName + '/',
                userAgent: userAgent
            }, 'S3_AUTH_DEBUG: Adding trailing slash for bucket URI SigV4 verification');
            pathPart = '/' + bucketName + '/';
        } else {
            req.log.debug({
                bucket: bucketName,
                originalPath: pathPart,
                userAgent: userAgent
            }, 'S3_AUTH_DEBUG: AWS CLI detected - NOT adding trailing slash for bucket URI');
        }
    }
    ...
    var regex = /\/[^/]+\.(?:$|\/)|\/[^/]+\/[^/]+/;
    var isAwsCli = userAgent && userAgent.toLowerCase().includes('aws-cli');
    var shouldAddSlash = (!('user-agent' in req.headers) ||
       (req.headers['user-agent'] &&
       (req.headers['user-agent'].toLowerCase().includes('minio') ||
        req.headers['user-agent'].toLowerCase().includes('s3cmd'))) ||
       !isAwsCli) &&
        regex.test(pathPart) === false && pathPart !== '/' && !pathPart.endsWith('/');
    ...
    if (req._rawBodyBuffer) {
        ...
        }, 'S3_AUTH_DEBUG: Including preserved raw body in verification request');
    } else if (req.method === 'POST' && req.headers['content-length'] && 
               parseInt(req.headers['content-length']) > 0) {
        req.log.warn({
            method: req.method,
            contentLength: req.headers['content-length']
        }, 'S3_AUTH_DEBUG: POST request with body but no preserved raw body available');
    }
    ...
    req.log.debug({
        ...
        CONTENT_MD5_DEBUG: {
            ...
            match: req.headers['content-md5'] === requestForVerification.headers['content-md5']
        }
    }, 'S3_AUTH_DEBUG:'+
