...
function sigv4Handler(req, res, next) {
    ...
    req.log.info({
        ...
        headerKeys: Object.keys(requestForVerification.headers).sort()
    }, 'S3_AUTH_DEBUG: Calling node-mahi verifySigV4 for signature verification');
...
function loadCaller(req, res, next) {
    ...
    // Add support for SigV4 access key lookup
    if (accessKeyId && req.auth.method === 'sigv4') {
        req.log.info({
            accessKeyId: accessKeyId,
            method: 'sigv4'
        }, 'S3_AUTH_DEBUG: Loading caller via access key lookup');
        
        // Use node-mahi's getUserByAccessKey method
        req.mahi.getUserByAccessKey(accessKeyId, function(err, data) {
            if (err) {
                req.log.info({
                    error: err.message || err.toString(),
                    accessKeyId: accessKeyId
                }, 'S3_AUTH_DEBUG: AUTHENTICATION FAILED - Access key lookup failed');
            } else {
                req.log.info({
                    accessKeyId: accessKeyId,
                    userUuid: data ? data.uuid : 'unknown',
                    userLogin: data ? data.login : 'unknown'
                }, 'S3_AUTH_DEBUG: AUTHENTICATION SUCCESS - Access key lookup successful');
            }
            gotCaller(err, data);
        });
