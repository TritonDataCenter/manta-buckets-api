...
function sigv4Handler(req, res, next) {
    ...
    // Handle problematic headers that can cause mahi connection issues
    var filteredHeaders = {};
    var emptyValueHeaders = [
        'content-encoding',
    ...
    Object.keys(req.headers).forEach(function(key) {
        ...
        if (skipHeaders.indexOf(lowerKey) !== -1) {
            ...
            return;
        } else if (emptyValueHeaders.indexOf(lowerKey) !== -1) {
            // Include but with empty value (these are in SignedHeaders)
            filteredHeaders[key] = '';
        } else {
    ...
    var emptiedHeaders = originalHeaderKeys.filter(function(key) {
        return emptyValueHeaders.indexOf(key.toLowerCase()) !== -1;
    });
    ...
    req.log.info({
        ...
        emptiedHeaders: emptiedHeaders,
        filteredHeaderKeys: filteredHeaderKeys
    }, 'S3_AUTH_DEBUG: Calling node-mahi verifySigV4 for signature verification (processed problematic headers)');
