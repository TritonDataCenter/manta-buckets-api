...
function sigv4Handler(req, res, next) {
    ...
    // TODO: Temporary bypass until mahi SigV4 support is properly deployed
//    req.log.warn('sigv4Handler: SigV4 authentication detected but bypassed - mahi SigV4 support not available');
 //   next(new InvalidSignatureError('SigV4 authentication not yet supported'));
  //  return;

    req.log.debug('sigv4Handler: processing AWS SigV4 authentication');
    ...
    if (!authHeader) {
        req.log.debug('sigv4Handler: missing authorization header');
        next(new InvalidSignatureError('Missing Authorization header'));
        return;
    }
    
    if (!dateHeader) {
        req.log.debug('sigv4Handler: missing date header');
        next(new InvalidSignatureError('Missing date header'));
        return;
    }

    // Prepare request for Mahi SigV4 verification
    var verifyOptions = {
        method: 'POST',
        path: '/aws-verify',
        headers: {
            ...
            'X-Amz-Content-Sha256': req.headers['x-amz-content-sha256'] || 'UNSIGNED-PAYLOAD',
            'Host': req.headers.host,
            'Content-Type': 'application/json'
        }
    };

    // Include original request details for signature verification
    var requestDetails = {
        method: req.method,
        path: req.path(),
        query: req.query,
    ...
    }, 'sigv4Handler: calling mahi for SigV4 verification');

    // Call Mahi's SigV4 verification endpoint using HTTP request

    // Call /aws-verify endpoint for SigV4 verification
    var verifyPath = '/aws-verify';
    var verifyData = {
        method: req.method,
        path: req.path(),
        query: req.query,
    ...
    req.mahi.post(verifyPath, verifyData, function(err, req2, res2, result) {
        if (err) {
            req.log.debug(err, 'sigv4Handler: SigV4 verification failed');
            
            // Map Mahi errors to appropriate HTTP errors
        ...
        if (!result || !result.userUuid) {
            req.log.warn('sigv4Handler: Invalid response from mahi SigV4 verification');
        ...
        }, 'sigv4Handler: SigV4 verification successful');
...
function loadCaller(req, res, next) {
    ...
    if (accessKeyId && req.auth.method === 'sigv4') {
        req.log.debug({accessKeyId: accessKeyId}, 'loadCaller: using access key lookup');
        
        // Use Mahi's access key lookup endpoint via GET request
        var lookupPath = '/aws-auth/' + encodeURIComponent(accessKeyId);
        
        req.mahi.get(lookupPath, function(err, req2, res2, data) {
            gotCaller(err, data);
        });
