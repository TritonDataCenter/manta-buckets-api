...
function sendInvalidSignatureError(req, res, next, message) {
    try {
        ...
        if (req.s3Request && req.s3Request.isS3Request) {
            ...
            res.send(constants.HTTP_STATUS.FORBIDDEN, xmlError);
        } else {
            res.send(constants.HTTP_STATUS.FORBIDDEN, authError.message);
        }
    } catch (sendErr) {
...
function createAuthToken(opts, aes, cb) {
    ...
    if (str.length > constants.SIZE_LIMITS.COMPRESSION_MAX) {
    ...
    if (isTemporaryCredential) {
        ...
        if (result && isTemporaryCredential) {
            req.log.info({
                ...
                assumedRoleKeys: result.assumedRole && typeof(result.assumedRole) === 'object' ?
                    Object.keys(result.assumedRole) : null,
                assumedRoleArn: result.assumedRole && typeof(result.assumedRole) === 'object' ?
                    result.assumedRole.arn : null,
                isTemporaryCredential: result.isTemporaryCredential,
                principalUuid: result.principalUuid
            }, 'STEP8_DEBUG: Raw mahi response for temporary credentials');
        }
        ...
        if (req.auth.isTemporaryCredential && req.auth.assumedRole) {
            ...
            loadAssumedRoleInfo(roleArn,
                ...
                req.log.info({
                    ...
                    roleArnIsUndefined: req.caller.assumedRole.arn === undefined,
...
function loadAssumedRole(req, res, next) {
    ...
    if (!roleName) {
        log.debug({roleArn: roleArn}, 'AUTH_DEBUG: Invalid role ARN format');
