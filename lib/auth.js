...
function checkAuthzScheme(req, res, next) {
    ...
    if (!ok) {
        req.log.info('S3_AUTH_DEBUG: AUTHENTICATION FAILED - Invalid authorization scheme: ' + scheme);
...
function sigv4Handler(req, res, next) {
    ...
    req.log.info({
        authorizationScheme: req.authorization ? req.authorization.scheme : 'undefined',
        hasAuthorizationHeader: !!req.headers.authorization,
        authHeaderLength: req.headers.authorization ? req.headers.authorization.length : 0,
        authHeaderPrefix: req.headers.authorization ? req.headers.authorization.substring(0, 100) + '...' : 'none'
    ...
    if (!authHeader) {
        req.log.info('S3_AUTH_DEBUG: AUTHENTICATION FAILED - Missing Authorization header');
        next(new InvalidSignatureError('Missing Authorization header'));
        return;
    }

    if (!dateHeader) {
        req.log.info('S3_AUTH_DEBUG: AUTHENTICATION FAILED - Missing date header');
        next(new InvalidSignatureError('Missing date header'));
        return;
    }

    // Prepare request object for node-mahi verifySigV4 function
    // Only skip headers that cause connection issues, but preserve all values for signature verification
    var filteredHeaders = {};
    var skipHeaders = [
        'transfer-encoding',
        'expect'
    ...
    Object.keys(req.headers).forEach(function (key) {
        ...
        if (skipHeaders.indexOf(lowerKey) !== -1) {
            ...
            // Include with original value (needed for correct signature verification)
    ...
    // Parse and fix URL encoding for query parameters
    // AWS CLI sends delimiter=%2F but when we URL-encode the whole URL for mahi,
    // it becomes delimiter=%252F which breaks signature verification
    if (urlForVerification.indexOf('?') !== -1) {
        ...
        try {
            ...
            }, 'S3_AUTH_DEBUG: Fixed query parameter encoding for SigV4 verification');
        } catch (e) {
            // If decoding fails, keep original URL
            req.log.info({
                originalUrl: req.url,
                error: e.message,
                reason: 'Query decoding failed, keeping original'
    ...
    var filteredHeaderKeys = Object.keys(requestForVerification.headers).sort();
    ...
    req.log.info({
        ...
        authHeaderPrefix: (requestForVerification.headers.authorization || '').substring(0, 50) + '...',
        hasAuthHeader: !!requestForVerification.headers.authorization,
        hasDateHeader: !!(requestForVerification.headers['x-amz-date'] || requestForVerification.headers.date),
        ...
        criticalHeaders: {
            'content-encoding': requestForVerification.headers['content-encoding'],
            'x-amz-decoded-content-length': requestForVerification.headers['x-amz-decoded-content-length'],
            'x-amz-sdk-checksum-algorithm': requestForVerification.headers['x-amz-sdk-checksum-algorithm'],
            'x-amz-trailer': requestForVerification.headers['x-amz-trailer']
        }
    }, 'S3_AUTH_DEBUG: Calling node-mahi verifySigV4 for signature verification (only skipped connection-breaking headers)');
    ...
    req.mahi.verifySigV4(requestForVerification, function (err, result) {
        if (err) {
            ...
            }, 'S3_AUTH_DEBUG: AUTHENTICATION FAILED - SigV4 verification failed');

            // Map node-mahi errors to appropriate HTTP errors
            switch (err.restCode || err.name) {
            case 'InvalidSignature':
            case 'SignatureDoesNotMatch':
                next(new InvalidSignatureError(err.message || 'Invalid signature'));
                break;
            ...
            case 'RequestTimeTooSkewed':
                ...
                break;
            default:
                next(new InvalidSignatureError('Authentication failed: ' + (err.message || 'Unknown error')));
                break;
            }
            return;
        }
        ...
        if (!result || !result.valid || !result.userUuid) {
            ...
            }, 'S3_AUTH_DEBUG: AUTHENTICATION FAILED - Invalid response from SigV4 verification');
        ...
        req.log.info({
            ...
            method: 'sigv4'
        }, 'S3_AUTH_DEBUG: AUTHENTICATION SUCCESS - SigV4 verification successful');
...
function loadCaller(req, res, next) {
    ...
    var accessKeyId = req.auth.accessKeyId;  // Add support for access key lookup
    ...
    function gotCaller(err, info) {
        ...
        if (req.isS3Request) {
            req.log.info({
                ...
                isProvisioned: info.account ? info.account.approved_for_provisioning : false
            }, 'S3_AUTH_DEBUG: AUTHENTICATION COMPLETE - Caller loaded successfully');
        }
    ...
    if (accessKeyId && req.auth.method === 'sigv4') {
        req.log.info({
            ...
            method: 'sigv4'
        }, 'S3_AUTH_DEBUG: Loading caller via userUuid from SigV4 verification');

        // Use the userUuid from SigV4 verification to get actual account details
        req.mahi.getAccountById(req.auth.accountid, function (err, data) {
            if (err) {
                req.log.info({
                    ...
                    userUuid: req.auth.accountid,
                    accessKeyId: accessKeyId
                }, 'S3_AUTH_DEBUG: AUTHENTICATION FAILED - Account lookup by UUID failed');
            } else {
                req.log.info({
                    accessKeyId: accessKeyId,
                    userUuid: data ? data.uuid : 'unknown',
                    userLogin: data ? data.login : 'unknown'
                }, 'S3_AUTH_DEBUG: AUTHENTICATION SUCCESS - Account lookup by UUID successful');
            }
            gotCaller(err, data);
        });
...
function verifySignature(req, res, next) {
    // Skip SSH signature verification if SigV4 authentication was already completed
    if (req.auth.method === 'sigv4') {
        req.log.debug('verifySignature: skipping SSH signature verification for SigV4 request');
        setImmediate(next);
        return;
    }
...
function loadOwnerFromPath(req, p, next) {
    ...
    // For S3 requests, use the authenticated user's account instead of extracting from path
    // S3 paths like '/mybucket' don't contain account info - bucket is owned by authenticated user
    if (req.isS3Request && req.caller && req.caller.account && req.caller.account.login) {
