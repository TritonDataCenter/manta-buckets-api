...
var InvalidArgumentError = restifyErrors.InvalidArgumentError;
...
function authorize(req, res, next) {
    ...
    try {
        ...
        case 'CrossAccount':
            /*
             * Cross-account access is NOT supported in Manta due to its
             * single-account RBAC model. The libmantalite authorization
             * library only evaluates roles within the resource owner's account.
             * This error case should never be reached because cross-account
             * roles are filtered before authorization. If we reach here, it
             * indicates a bug in the role filtering or authorization logic.
             */
            next(new AuthorizationError(login, req.path(), e));
            return;
