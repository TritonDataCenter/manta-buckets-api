...
function scopeGrantsAccess(permissions, bucket, needed) {
    for (var i = 0; i < permissions.length; i++) {
        ...
        if (granted >= needed &&
            matchBucketPattern(perm.bucket, bucket)) {
            return true;
        }
    }
    return false;
...
function enforceBucketScope(req, res, next) {
    ...
    if (scope === null) {
        /*
         * Malformed scope JSON: fail closed (NFR-10).
     ...
     */
    if (bucket === null) {
        req._bucketScopeFilter =
            allowedBucketPatterns(scope.permissions);
        next();
        return;
    }
    ...
    if (!scopeGrantsAccess(
        scope.permissions, bucket, needed)) {
        req.log.info({
