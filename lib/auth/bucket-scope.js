...
var scopeSchema = require('mahi/lib/scope-schema');
...
function enforceBucketScope(req, res, next) {
            ...
            req.log.warn(
                {
                    ...
                    assumedRole:
                        typeof (req.auth.assumedRole) === 'string'
                            ? req.auth.assumedRole
    ...
    /*
     * Cache parsed scope on req to avoid
     * repeated JSON.parse on the same request (copy
     * handlers may re-check scope for the source
     * bucket).
     */
    var scope;
    if (req._parsedBucketScopeRaw === req.caller.bucketScope) {
        scope = req._parsedBucketScope;
    } else {
        scope = parseScope(req.caller.bucketScope);
        req._parsedBucketScope = scope;
        req._parsedBucketScopeRaw = req.caller.bucketScope;
    }
