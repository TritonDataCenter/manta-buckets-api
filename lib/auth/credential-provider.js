    ...
    function gotCaller(err, info) {
        ...
        if (req.auth && req.auth.method === 'sigv4' && req.auth.accessKeyId &&
            ...
            if (Array.isArray(accesskeys)) {
                ...
                if (keyInfo) {
                    // Debug: log what properties keyInfo actually has
                    req.log.debug({
                        accessKeyId: req.auth.accessKeyId,
                        keyInfoType: typeof (keyInfo),
                        keyInfoKeys: typeof (keyInfo) === 'object' ?
                            Object.keys(keyInfo) : undefined,
                        hasSecret: !!(keyInfo.secret),
                        hasSecretAccessKey: !!(keyInfo.secretAccessKey),
                        keyInfoValue: typeof (keyInfo) === 'string' ?
                            '[REDACTED]' : undefined
                    }, 'Debug: keyInfo structure');
                    secretKey = keyInfo.secret || keyInfo.secretAccessKey;
                }
            }

            // Debug: Log structure when key not found
            if (!secretKey) {
                req.log.debug({
                    accessKeyId: req.auth.accessKeyId,
                    isArray: Array.isArray(accesskeys),
                    keyCount: Array.isArray(accesskeys) ?
                        accesskeys.length : Object.keys(accesskeys).length,
                    arrayKeys: Array.isArray(accesskeys) ?
                        accesskeys.map(function (k) {
                            return (k.accessKeyId);
                        }) : undefined,
                    objectKeys: !Array.isArray(accesskeys) ?
                        Object.keys(accesskeys) : undefined
                }, 'Debug: accesskeys structure when secret not found');
            }
