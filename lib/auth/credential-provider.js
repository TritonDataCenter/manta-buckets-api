...
var parseScope = require('mahi').scopeSchema.parseScope;
...
function loadCaller(req, res, next) {
    ...
    function gotCaller(err, info) {
         ...
         * See also role-manager.js gatherContext() for the same
         * pattern.
         */
        var callerCopy = {};
         ...
         * Delete from req.auth after copying — it was only a
         * transport from signature-verifier to here.
         */
        ...
        /*
         * For subusers, ensure roles and defaultRoles are on the
         * user object.  Clone req.caller.user first — it still
         * references the cached info.user (CHG-070).
         */
        if (req.caller.user && req.caller.type === 'user') {
            if ((req.caller.roles && !req.caller.user.roles) ||
                ...
                var userCopy = {};
                Object.keys(req.caller.user).forEach(function (k) {
        ...
        if (req.auth.isTemporaryCredential && req.auth.assumedRole) {
            ...
            }, 'loadCaller: loading assumed role for temp credential');
            ...
            loadAssumedRoleInfo(roleArn,
                ...
                req.log.debug({
                    roleName: req.caller.assumedRole.name,
            ...
            /*
             * No explicit clear needed — req.caller is a fresh
             * shallow clone (see CHG-070 above), so stale
             * assumedRole from a previous request cannot be
             * present.
             */
            return (continueWithNormalAuth());
        }
