...
function getActiveRoles(req, res, next) {
    ...
    if (isRoleOper || isGlobalReader) {
        /*
         * The req.caller object is cached and
         * potentially shared between multiple
         * requests. We're either going to alter the
         * roles or the isOperator flag on
         * req.caller.account on a per-request basis,
         * so we need a per-request copy of
         * req.caller, req.caller.account and
         * req.caller.roles.
         *
         * We can keep sharing all the other child
         * objects of req.caller other than
         * req.caller.account and req.caller.roles
         * (i.e. we don't have to do a full deep
         * copy), because we're not changing those.
         */
        var newCaller = {};
    ...
    if (requestedRoles) {   // The user passed in roles
        ...
        for (i = 0; i < names.length; ++i) {
            ...
            if (roles === undefined || roles.length < 1) {
                var InvalidRoleError =
                    require('../errors').InvalidRoleError;
