...
function sigv4Handler(req, res, next) {
    ...
    if (isTemporaryCredential) {
        ...
        // NOTE: Requires Mahi API extension to return secret key
        // in verifySigV4 response
        if (result.secretAccessKey) {
            ...
            req.log.debug({
                hasSecretKey: false,
                isTemporaryCredential: isTemporaryCredential
            }, 'Secret access key not available from Mahi - ' +
                'AWS chunked signature verification will be disabled');
        }
