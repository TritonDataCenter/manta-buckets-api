 ...
 *
 * For S3 requests, returns an XML error body with the
 * appropriate S3 error code. For non-S3 requests,
 * returns a plain 403 with the error message.
 *
 * @param {Object} req - Restify request
 * @param {Object} res - Restify response
 * @param {Function} next - Callback
 * @param {String} message - Human-readable error message
 * @param {String} [s3ErrorCode] - S3 error code to use
 *     as restCode (e.g. 'InvalidAccessKeyId',
 *     'SignatureDoesNotMatch'). Defaults to
 *     'InvalidSignature'.
 */
function sendInvalidSignatureError(req, res, next,
    ...
    try {
         ...
         * distinct S3 error code. This lets
 ...
 * HTTP error response.
 *
 * Centralizes the error-code switch logic that was
 * previously duplicated in the temporary-credential
 * and permanent-credential callbacks.
 *
 * Invariant: every mahi error is mapped to exactly one
 * sendInvalidSignatureError call; no error falls through
 * without a response.
 *
 * Time complexity: O(1) — constant-time switch lookup.
    ...
    if (req._s3PresignedConverted) {
        ...
        try {
            ...
            req.mahi.verifySigV4(requestForVerification,
                ...
                req.auth.bucketScope = result.bucketScope || null;
...
function sigv4Handler(req, res, next) {
    ...
    if (isTemporaryCredential) {
        ...
        req.auth = {
            ...
            // Per-bucket access key scope (JSON string or null)
