...
function sigv4Handler(req, res, next) {
    ...
    // Detect CLI tools vs SDK libraries
    // CLI tools (aws-cli, boto3, botocore) sign WITHOUT trailing slash
    // SDK libraries (aws-sdk-js, aws-sdk-java, etc.) sign WITH trailing slash
    ...
    // AWS SDK libraries (not CLI tools) sign with trailing slashes
    var isAwsSdk = (userAgent &&
    ...
    // AWS CLI/boto* do NOT add trailing slash
    // AWS SDKs and other clients DO add trailing slash
    var bucketPathHandled = false;
    if (bucketName && pathPart === '/' + bucketName) {
        bucketPathHandled = true;
        if (isAwsCli) {
            // AWS CLI and boto* sign WITHOUT trailing slash
            req.log.debug({
                bucket: bucketName,
                originalPath: pathPart,
                userAgent: userAgent
            }, 'S3_AUTH_DEBUG: AWS CLI/boto detected -' +
                          ' NOT adding trailing slash for bucket URI');
            // pathPart stays as-is (no trailing slash)
        } else {
            // AWS SDKs and other S3 clients sign WITH trailing slash
            req.log.debug({
                bucket: bucketName,
                originalPath: pathPart,
                newPath: '/' + bucketName + '/',
                userAgent: userAgent,
                isAwsSdk: isAwsSdk
            }, 'S3_AUTH_DEBUG: S3 client detected -' +
    ...
    // AWS CLI/boto* are excluded as they sign without trailing slashes.
    // AWS SDKs and other clients sign WITH trailing slashes.
    ...
    // Add slash for all S3 clients EXCEPT AWS CLI/boto*
