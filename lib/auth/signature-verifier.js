    ...
    if (isTemporaryCredential) {
        ...
        if (err) {
            ...
            case 'AccessKeyNotFound':
                /*
                 * Key not in Redis cache.  May be a
                 * genuinely invalid key, or a newly
                 * created key that hasn't replicated
                 * yet (~2s).  Return 503 with
                 * Retry-After so S3 clients retry.
                 */
                req.log.warn({
                    accessKeyId: err.accessKeyId ||
                        '(unknown)'
                }, 'Access key not found in cache' +
                    ' — may be propagation delay');
                res.setHeader('Retry-After', '3');
                next(new restify.ServiceUnavailableError(
                    'Access key not yet available.' +
        ...
        req.mahi.verifySigV4(requestForVerification,
            ...
            if (err) {
                ...
                case 'AccessKeyNotFound':
                    ...
                    res.setHeader('Retry-After', '3');
                    next(
                        new AccessKeyNotYetAvailableError());
                    return;
                case 'RequestTimeTooSkewed':
                    sendInvalidSignatureError(req, res, next,
                        'Request timestamp too skewed');
