...
function createAuthToken(opts, aes, cb) {
    ...
    zlib.gzip(new Buffer(payload, 'utf8'),
        function handleGzipCompress(err, buf) {
        if (err) {
            cb(err);
            return;
        }
...
function parseAuthToken(token, aes, cb) {
    ...
    try {
        ...
        setImmediate(function returnInvalidTokenError() {
    ...
    zlib.gunzip(new Buffer(buf, 'binary'),
        function handleGzipDecompress(err, str) {
        if (err) {
            cb(new InvalidAuthTokenError());
            return;
        }
...
function createAuthTokenHandler(req, res, next) {
    ...
    createAuthToken(opts, aes, function handleCreateAuthToken(err, token) {
        if (err) {
            log.error(err,
...
function parseAuthTokenHandler(req, res, next) {
    ...
    parseAuthToken(token, aes, function handleParseAuthToken(err, obj) {
        if (err) {
            log.debug(err,
