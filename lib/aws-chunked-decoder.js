...
AwsChunkedDecoder.prototype._readChunkSize = function () {
    ...
    // Enhanced logging for immediate debugging
    if (this._log) {
        this._log.info({
...
AwsChunkedDecoder.prototype._readChunkData = function () {
    ...
    if (bytesNeeded > 0) {
        ...
        if (this._log) {
            // Calculate MD5 of the chunk we're about to push for debugging
            var crypto = require('crypto');
            var chunkMd5 = crypto.createHash('md5').update(chunkData).digest('hex');
            this._log.debug({
                chunkSize: bytesNeeded,
                chunkMd5: chunkMd5,
                chunkDataPreview: chunkData.slice(0, 32).toString('hex'),
                totalDecoded: this._totalBytesDecoded + bytesNeeded,
                remainingBufferAfterSkip: this._buffer.length - bytesNeeded - 2
            }, 'AWS chunked decoder: pushing verified chunk data');
        ...
        // Only push data after CRLF verification
        console.log('AWS Decoder: About to push chunk #' + 
                    (this._totalBytesDecoded === 0 ? '1' : '2+') + 
                    ' of ' + bytesNeeded + ' bytes');
        this.push(chunkData);
        console.log('AWS Decoder: Successfully pushed chunk of ' + bytesNeeded + ' bytes');
