...
function AwsChunkedDecoder(options) {
    ...
    this._previousSignature = this._seedSignature; // Chain starts with seed
    this._validateSignatures = options.validateSignatures !== false; // Default: true
...
AwsChunkedDecoder.prototype._readChunkData = function () {
    ...
    if (bytesNeeded > 0) {
        ...
        if (this._validateSignatures && this._currentChunkSignature) {
            var verifier = require('./aws-chunked-signature-verifier');
            ...
            if (!this._signingKey || !this._previousSignature ||
                ...
                throw new InvalidChunkSignatureError(this._chunkNumber);
            }
