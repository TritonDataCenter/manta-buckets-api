 ...
 *
 * According to the specification in https://docs.aws/amazon.com/AmazonS3/late\
 * st/API/sigv4-streaming.html, the headers that signal that the request is
 * chunked-encoded are the following:
 * - x-amz-content-sha256 with values:
 *            - STREAMING-AWS4-HMAC-SHA256-PAYLOAD (chunked body)
 *            - STREAMING-AWS4-HMAC-SHA256-PAYLOAD-TRAILER (chunked body
 *                                                   plus a trailing
 *                                                   checksum chunk)
 * - Content-Encoding with the value 'aws-chunked'
 * The problem is that the only header that is actually required for the
 * Sigv4 validation is x-amz-content-sha256, so some clients omit this header.
 * So to guard ourselves of clients not following the spec we check for the onl
 * y one that they cannot omit.
 */
