 ...
 *   https://docs.aws.amazon.com/AmazonS3/latest/API/sigv4-streaming.html
 * the headers that signal a chunked-encoded request are:
 * - x-amz-content-sha256 with one of these values:
 *     - STREAMING-AWS4-HMAC-SHA256-PAYLOAD (chunked body)
 *     - STREAMING-AWS4-HMAC-SHA256-PAYLOAD-TRAILER (chunked body
 *       plus a trailing checksum chunk)
 * - Content-Encoding with the value 'aws-chunked'
 *
 * The problem is that the only header actually required for the
 * SigV4 signature to validate is x-amz-content-sha256, so some
 * clients omit Content-Encoding. To guard against clients not
 * following the spec we check the only header they cannot omit.
 */
