 ...
 *
 * This module provides cryptographic utilities for verifying AWS SigV4
 * streaming signatures (aws-chunked encoding). Each chunk in an aws-chunked
 * upload has a signature that forms a chain - each chunk's signature depends
 * on the previous chunk's signature (or the seed signature from the
 * Authorization header for the first chunk).
 *
 * References:
 * - AWS SigV4 Streaming:
 ...
 *   sigv4-calculate-signature.html
 */
 ...
 *
 * @param {String} secretKey - The secret access key
 *     (e.g., from IAM credentials)
 ...
 *   <hash-of-chunk-data>
 *
 * @param {Buffer} signingKey - The derived signing key from
 *     deriveSigningKey()
 * @param {String} previousSignature - The previous chunk's signature
 *     (or seed signature)
 * @param {String} timestamp - ISO 8601 timestamp
 *     (e.g., "20250124T123456Z")
 * @param {String} credentialScope - Scope string
 *     (e.g., "20250124/us-east-1/s3/aws4_request")
 * @param {Buffer} chunkData - The chunk data to sign
 * @return {String} The calculated signature as a hex string
 */
function
calculateChunkSignature(signingKey, previousSignature, timestamp,
 ...
 * @param {String} expectedSignature - The signature from chunk header
 * @return {Boolean} true if the signature is valid, false otherwise
 */
function
verifyChunkSignature(signingKey, previousSignature, timestamp,
    credentialScope, chunkData, expectedSignature)
{
    ...
    // Node v0.10.48 doesn't have crypto.timingSafeEqual, so implement it
    if (calculated.length !== expectedSignature.length) {
        return (false);
    }

    var mismatch = 0;
    for (var i = 0; i < calculated.length; i++) {
        mismatch |= calculated.charCodeAt(i) ^
