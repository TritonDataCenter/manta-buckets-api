...
function validateAndSetPrecondition(req) {
    ...
    for (var name of [ 'if-modified-since', 'if-unmodified-since' ]) {
        if (req.headers[name]) {
            ...
            precondition[name] = new Date(value).toISOString();
        }
    }

    for (var name of [ 'if-match', 'if-none-match' ]) {
        if (req.headers[name]) {
            var value = [];
