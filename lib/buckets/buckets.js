...
function loadRequest(req, res, next) {
    ...
    var preconditionErr = validateAndSetPrecondition(req);
    if (preconditionErr) {
        next(preconditionErr);
        return;
    }
...
function getObject(req, res, next) {
    ...
    var precondition = {};
    precondition['if-match'] = req.precondition['if-match'];
    precondition['if-unmodified-since'] =
        req.precondition['if-unmodified-since'];

    client.getObject(owner, bucket.id, bucketObject.name,
        metadataLocation.vnode, precondition, requestId, onGetObject);
}
...
/*
 * This function pulls applicable precondition headers out of req.headers and
 * creates/populates a new object called req.precondition.  This new object is
 * intended to be passed down to buckets-mdapi as part of a structured set of
 * conditional parameters.
 */
function validateAndSetPrecondition(req) {
    var precondition = {};

    if (req.headers['if-modified-since']) {
        var ifModifiedSince = Date.parse(req.headers['if-modified-since']);

        if (isNaN(ifModifiedSince) || !ifModifiedSince) {
            return (new restifyErrors.BadRequestError('if-modified is baaaad'));
        }

        assert.number(ifModifiedSince);
    ...
    if (req.headers['if-unmodified-since']) {
        var ifUnmodifiedSince = Date.parse(req.headers['if-unmodified-since']);

        if (isNaN(ifUnmodifiedSince) || !ifUnmodifiedSince) {
            return (new restifyErrors.BadRequestError('if-unmodified is baaaad'));
        }

        assert.number(ifUnmodifiedSince);

        precondition['if-unmodified-since'] =
            new Date(ifUnmodifiedSince).toISOString();
    }

    if (req.headers['if-match']) {
        var ifMatch = [];
        var etags = req.headers['if-match'].split(/\s*,\s*/);
        ...
        if (ifMatch.length > 0) {
            precondition['if-match'] = ifMatch;
        }
    }

    if (req.headers['if-none-match']) {
        var ifNoneMatch = [];
        var etags = req.headers['if-none-match'].split(/\s*,\s*/);
        ...
        for (var i = 0; i < etags.length; i++) {
            ...
            ifNoneMatch.push(cur);
        }

        if (ifNoneMatch.length > 0) {
            precondition['if-none-match'] = ifNoneMatch;
        }
    }
