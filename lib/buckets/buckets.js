...
function conditionalHandler(req, res, next) {
    ...
    // For S3 requests, if-match is stripped from mdapi conditions
    // (mdapi compares against UUID, S3 clients send MD5 hex).
    // Evaluate if-match here against the ETag we set on the response.
    // Etag value is the MD5 hex for objects stored through
    // the S3 layer.
    if (req.isS3Request && req.conditions['if-match']) {
        var if_match = req.conditions['if-match'];
        var etag = res.header('Etag');
        var matched = false;

        if_match.forEach(function (client_etag) {
            if (client_etag === '*' || client_etag === etag) {
                matched = true;
            }
        });
        // If none of the if-match conditions succeeds,
        // then HTTP return code 412 (Precondition failed) is set.
        if (!matched) {
            code = 412;
        }
    }
