...
function _list(type, req, bucket_id) {
    ...
    if (prefix) {
        ...
        // NEW: Binary data check
        if (/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F-\x9F]/.test(prefix)) {
            process.nextTick(function () {
                ee.emit('error', new InvalidParameterError('prefix',
                    'Prefix parameter contains invalid characters'));
            });
        ...
        // NEW: Length check (AWS S3 has a practical limit around 1000 chars)
    ...
    if (marker) {
        ...
        // NEW: CRLF and binary data checks for marker
        if (/[\r\n]/.test(marker) ||
            /[\x00-\x08\x0B\x0C\x0E-\x1F\x7F-\x9F]/.test(marker)) {
            process.nextTick(function () {
                ee.emit('error', new InvalidParameterError('marker',
    ...
    if (delimiter) {
        ...
        // NEW: CRLF and binary data checks for delimiter
        ...
        if (/[\r\n]/.test(delimiter) ||
            /[\x00-\x08\x0B\x0C\x0E-\x1F\x7F-\x9F]/.test(delimiter)) {
            process.nextTick(function () {
                ee.emit('error', new InvalidParameterError('delimiter',
    ...
    // Enhanced limit validation (handle both 'limit' and 'max-keys')
    var limit;
    ...
    // NEW: Check for continuation-token (ListObjectsV2)
    if (req.query['continuation-token']) {
        var token = req.query['continuation-token'];
        if (nullRegex.test(token) || /[\r\n]/.test(token) ||
            /[\x00-\x08\x0B\x0C\x0E-\x1F\x7F-\x9F]/.test(token) ||
            token.length > 1024) {
            process.nextTick(function () {
