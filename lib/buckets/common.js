...
function _list(type, req, bucket_id) {
    ...
    paginationStream(opts,
        function onRecord(record) {
            ...
            // Filter out internal multipart objects from user-visible listings
            if (record.name.indexOf('.mpu-') === 0) {
                log.debug({recordName: record.name},
                         'onRecord: filtering out .mpu object');
...
function paginationStream(opts, onRecord, done) {
    ...
    function processRecords(cb) {
        ...
        // Skip internal multipart directories from user-visible listings
        if (base.indexOf('.mpu-') === 0) {
            // Don't send group record for internal multipart directories
            // and don't send the record at all - just continue processing
            log.debug({baseName: base},
                     'skipRecord: filtering out .mpu directory');
            cb();
            return;
        }
