...
function _list(type, req, bucket) {
    ...
    switch (type) {
    case 'buckets':
        mreq = listBucketsNoVnode(owner, prefix, limit, marker,
            delimiter, requestId, req.log);
        break;
    case 'objects':
...
function listBucketsNoVnode(owner, prefix, limit, marker, delimiter, req) {
    assert.uuid(owner, 'owner');
    ...
    paginationStream(opts,
        function onRecord(record) {
            ...
            ee.emit('record', formatRecord(record));
        },
        function done(err) {
            if (err) {
