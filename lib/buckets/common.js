 ...
 */
var CRLF_REGEX = /[\r\n]/;
var BINARY_CHARS_REGEX = /[\x00-\x08\x0B\x0C\x0E-\x1F\x7F-\x9F]/;
var PATH_TRAVERSAL_REGEX = /\.\.\//;
...
function translateBucketError(req, err) {
    ...
    case 'PreconditionFailedError':
        err = new restifyErrors.PreconditionFailedError(err.message);
        break;
    default:
        break;
    }
...
function _list(type, req, bucket_id) {
    ...
    if (prefix) {
        ...
        if (nullRegex.test(prefix)) {
            process.nextTick(function () {
                ee.emit('error', new InvalidParameterError('prefix',
                    'Prefix parameter contains null bytes'));
            });
            return (ee);
        }

        // CRLF injection check
        if (CRLF_REGEX.test(prefix)) {
            process.nextTick(function () {
                ee.emit('error', new InvalidParameterError('prefix',
                    'Prefix parameter contains invalid characters'));
            });
            return (ee);
        }

        // Binary data check
        if (BINARY_CHARS_REGEX.test(prefix)) {
            process.nextTick(function () {
                ee.emit('error', new InvalidParameterError('prefix',
                    'Prefix parameter contains invalid characters'));
            });
        ...
        if (PATH_TRAVERSAL_REGEX.test(prefix)) {
            process.nextTick(function () {
                ee.emit('error', new InvalidParameterError('prefix',
        ...
        // Length check (AWS S3 has a practical limit around 1000 chars)
        if (prefix.length > 1000) {
            process.nextTick(function () {
                ee.emit('error', new InvalidParameterError('prefix',
                    'Prefix parameter too long'));
            });
            return (ee);
        }
    }
    ...
    // Marker validation
    if (marker) {
        if (nullRegex.test(marker)) {
            process.nextTick(function () {
                ee.emit('error', new InvalidParameterError('marker',
                    'Marker parameter contains null bytes'));
            });
            return (ee);
        }

        // CRLF and binary data checks for marker
        if (CRLF_REGEX.test(marker) || BINARY_CHARS_REGEX.test(marker)) {
            process.nextTick(function () {
                ee.emit('error', new InvalidParameterError('marker',
                    'Marker parameter contains invalid characters'));
            });
            return (ee);
        }

        // Length check for marker
        if (marker.length > 1024) {
            process.nextTick(function () {
                ee.emit('error', new InvalidParameterError('marker',
                    'Marker parameter too long'));
            });
            return (ee);
        }
    }
    ...
    if (delimiter) {
        if (nullRegex.test(delimiter)) {
            process.nextTick(function () {
                ee.emit('error', new InvalidParameterError('delimiter',
                    'Delimiter parameter contains null bytes'));
            });
            return (ee);
        }
        ...
        // CRLF and binary data checks for delimiter
        // - \x00-\x08 = Null byte through Backspace (ASCII 0-8)
        // - \x0B = Vertical Tab (ASCII 11)
        // - \x0C = Form Feed (ASCII 12)
        // - \x0E-\x1F = Shift Out through Unit Separator (ASCII 14-31)
        // - \x7F-\x9F = Delete through Control characters (ASCII 127-159)
        if (CRLF_REGEX.test(delimiter) || BINARY_CHARS_REGEX.test(delimiter)) {
            process.nextTick(function () {
                ee.emit('error', new InvalidParameterError('delimiter',
                    'Delimiter contains invalid characters'));
            });
            return (ee);
        }
    }

    // Limit validation (handle both 'limit' and 'max-keys')
    var limit;
    var limitParam = req.query.limit || req.query['max-keys'];
    if (limitParam) {
        limit = parseInt(limitParam, 10);
        if (isNaN(limit) || limit <= 0 || limit > LIST_LIMIT) {
            process.nextTick(function () {
                ee.emit('error', new InvalidLimitError(limitParam));
            });
            return (ee);
        }
    } else {
    ...
    // Check for continuation-token (ListObjectsV2)
    if (req.query['continuation-token']) {
        var token = req.query['continuation-token'];
        if (nullRegex.test(token) || CRLF_REGEX.test(token) ||
            BINARY_CHARS_REGEX.test(token) ||
            token.length > 1024) {
            process.nextTick(function () {
                ee.emit('error', new InvalidParameterError('continuation-token',
    ...
    // Cap the limit to the maximum allowed by buckets-mdapi (LIST_LIMIT)
    bucketsMdapiLimit = Math.min(bucketsMdapiLimit, LIST_LIMIT);
    ...
    var opts = {
        ...
        log: log,
        vnodes: vnodes,
        isS3Request: req.isS3Request || false
    };
    paginationStream(opts,
        function onRecord(record) {
            assert.object(record, 'record');

            log.debug({paginationRecord: record},
                      'onRecord: processing record');
            ...
            } else if (record.bucket_name) {
                record.name = record.bucket_name;
            } else {
                log.debug({record: record},
                   'skipping record without name or bucket_name');
                return;
            }

            // Allow .mpu-parts to be visible for manual cleanup by users
            // but still hide .mpu-uploads (active upload state)
            if (record.name.indexOf('.mpu-uploads') === 0) {
                log.debug({recordName: record.name},
                         'onRecord: filtering out .mpu-uploads object');
                return; // Skip this record entirely
            }

            // Filter .mpu-parts from S3 listings - these are MPU internal
            // objects whose shark data is deleted after v2 commit assembly.
            // AWS S3 never exposes multipart internals in ListObjects.
            // Keep visible for Manta API for manual cleanup.
            if (opts.isS3Request &&
                record.name.indexOf('.mpu-parts') === 0) {
                log.debug({recordName: record.name},
                    'onRecord: filtering out .mpu-parts for S3 request');
                return;
            }
...
LimitMarkerStream.prototype.getNextRecord =
    ...
    if (self.pendingRecord) {
        ...
        self.pendingRecord = null;
        self.log.debug({record: r}, 'returning pendingRecord');
        sendRecord(r);
        return;
    }
...
function paginationStream(opts, onRecord, done) {
    ...
    function processRecords(cb) {
        ...
        // Allow .mpu-parts directories to be visible for manual cleanup by
        // users but still hide .mpu-uploads directories (active upload state)
        if (base.indexOf('.mpu-uploads') === 0) {
            // Don't send group record for internal multipart upload directories
            // and don't send the record at all - just continue processing
            log.debug({baseName: base},
                     'skipRecord: filtering out .mpu-uploads directory');
            cb();
            return;
        }

        // Filter .mpu-parts directory prefix from S3 listings.
        // When delimiter='/' is used (rclone, aws cli), .mpu-parts/
        // would appear as a CommonPrefix in the XML response.
        if (opts.isS3Request && base.indexOf('.mpu-parts') === 0) {
            log.debug({baseName: base},
                'skipRecord: filtering out .mpu-parts directory for S3');
            cb();
            return;
        }
        ...
        function sendRecord(_rec) {
            // Only count records that actually get sent (not filtered out)
            var wasFiltered = _rec.name && _rec.name.indexOf('.mpu-') === 0;
            if (!wasFiltered) {
                totalRecordsSent++;
            }
            onRecord(_rec);
