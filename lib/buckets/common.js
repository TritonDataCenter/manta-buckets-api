...
var MIN_BUCKET_NAME_BYTE_SIZE = 3;
var MAX_BUCKET_NAME_BYTE_SIZE = 63;
...
function isValidBucketName(name) {
    ...
    return (bucketRegex.test(name) && !resemblesIpRegex.test(name) &&
        !nullRegex.test(name) &&
        Buffer.byteLength(name) >= MIN_BUCKET_NAME_BYTE_SIZE &&
        Buffer.byteLength(name) <= MAX_BUCKET_NAME_BYTE_SIZE);
}
