...
// restify error
function translateBucketErrors(req, err) {
    assert.ok(req, 'req');
    assert.ok(err, 'err');

    // These errors are directly from boray
    switch (err.name) {
...
module.exports = {
    ...
    translateBucketErrors: translateBucketErrors,
...
/*
 * paginationStream takes a collection of vnodes (boray clients specifically in
 * this module) and a number of options to manage multiple Streams
 * simultaneously.  The purpose of this function is to take multiple
