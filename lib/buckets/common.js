...
function _list(type, req, bucket_id) {
    ...
    bucketsMdapiLimit = Math.min(bucketsMdapiLimit, 1024);
...
function paginationStream(opts, onRecord, done) {
    ...
    function processRecords(cb) {
        ...
        // Allow .mpu-parts directories to be visible for manual cleanup by users
        // but still hide .mpu-uploads directories (active upload state)
