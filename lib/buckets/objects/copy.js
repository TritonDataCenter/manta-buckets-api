...
function copyObject(req, res, next) {
    ...
    // Do NOT decodeURIComponent here — object names are stored
    // URL-encoded by the S3 PUT handler (restify does not decode
    // regex route params), so the copy-source value already matches
    // the stored name.
    var sourcePathWithoutLeadingSlash = copySource.replace(/^\//, '');
 ...
 *
 * Tries each available source shark in turn, starting from a
 * random offset to spread load.  If a shark is unreachable
 * (connect timeout, non-200, etc.), the next replica is tried.
 * Fails only when every source shark has been exhausted.
 *
 * Time:  O(n) attempts in the worst case, where n is the
 *        replica count (typically 2-3).
 * Space: O(1) beyond the existing request context.
 *
 * @param {Object} req          - restify request
 * @param {Object} sourceReq    - source bucket/object info
 * @param {Object} sourceMetadata - metadata incl. sharks[]
 * @param {Array}  destinationSharks - allocated dest sharks
 * @param {String} objectId     - new destination object UUID
 * @param {Function} callback   - (err)
 */
function streamSourceToDestination(req, sourceReq, sourceMetadata,
                                   destinationSharks, objectId,
                                   callback) {
    var log = req.log;
    var owner = req.owner.account.uuid;
    var bucket = req.bucket;
    var bucketObject = req.bucketObject;
    var sharks = sourceMetadata.sharks;
    var startIndex = Date.now() % sharks.length;

    var sourceStoragePath = sharkClient.storagePath({
    ...
    log.debug({
        availableSourceSharks: sharks.length,
        startIndex: startIndex,
        allSourceSharks: sharks.map(function (s) {
            return (s.manta_storage_id);
        }),
        objectId: sourceMetadata.id,
        contentLength: sourceMetadata.content_length
    }, 'streamSourceToDestination: starting with fallback');

    var attempt = 0;

    /**
     * Try the shark at position (startIndex + attempt)
    ...
    function tryNextShark() {
        if (attempt >= sharks.length) {
            ...
            }, 'streamSourceToDestination: all source ' +
               'sharks exhausted');
        ...
        var idx = (startIndex + attempt) % sharks.length;
        var sourceShark = sharks[idx];
        attempt++;

        log.debug({
            attempt: attempt,
            sharkIndex: idx,
            shark: sourceShark.manta_storage_id
        }, 'streamSourceToDestination: trying source shark');
        ...
        sourceClient.get(sourceOpts,
            function onGet(getErr, _srcReq, sourceRes) {

            if (getErr || !sourceRes ||
                sourceRes.statusCode !== 200) {
                if (sourceRes) {
                    sourceRes.resume();
                }
                ...
                log.warn({
                    ...
                    shark: sourceShark.manta_storage_id,
                    attempt: attempt,
                    remaining: sharks.length - attempt,
                    statusCode: sourceRes ?
                        sourceRes.statusCode :
                        'no-response'
                }, 'streamSourceToDestination: source ' +
                   'shark failed, trying next');
...
function createDestinationMetadata(req, sourceMetadata, destinationSharks,
    ...
    client.createObject(
        ...
        function (createErr, result) {
            ...
            var copyEtag = req.isS3Request ?
                (common.md5Base64ToHex(
                    sourceMetadata.content_md5) ||
                result.id) : result.id;
            callback(null, {
                etag: copyEtag,
...
module.exports = {
    ...
    getSourceObjectMetadata: getSourceObjectMetadata
};
