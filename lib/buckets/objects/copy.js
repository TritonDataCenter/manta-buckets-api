...
function performServerSideCopy(req, res, sourceReq, sourceMetadata, callback) {
    ...
    var copies = sourceMetadata.sharks ? sourceMetadata.sharks.length : 2;
    var size = Math.ceil(sourceMetadata.content_length / (1024 * 1024));
    ...
    req.storinfo.choose({
        ...
        streamSourceToDestination(req, sourceReq, sourceMetadata,
             sharks, objectId, function (streamErr, streamResult) {
            if (streamErr) {
            ...
            createDestinationMetadata(req, sourceMetadata, sharks, objectId,
                streamResult, function (metadataErr, result) {
...
function streamSourceToDestination(req, sourceReq, sourceMetadata,
    ...
    var sourceSpark = sourceMetadata.sharks[sourceSharkIndex];

    log.debug({
        availableSourceSharks: sourceMetadata.sharks.length,
        selectedSourceSharkIndex: sourceSharkIndex,
        selectedSourceShark: sourceSpark.manta_storage_id,
    ...
    log.debug({
        sourceSpark: sourceSpark.manta_storage_id,
        objectId: sourceMetadata.id,
        contentLength: sourceMetadata.content_length
    }, 'streamSourceToDestination: creating source stream');
    ...
    sourceClient.get(sourceOpts, function (getErr, sourceClientReq, sourceRes) {
...
function streamToDestinationSharks(sourceSharkInfo, destinationSharks,
                                   opts, callback) {
    var log = opts.log || require('bunyan').createLogger({name: 'copy'});
    var crypto = require('crypto');
        ...
        func: function (shark, next) {
            ...
            function attemptSourceRead() {
                sourceClient.get(sourceSharkInfo.opts,
                                 function (getErr, sourceClientReq, sourceRes) {
                    if (getErr || !sourceRes || sourceRes.statusCode !== 200) {
                        retryCount++;
                ...
                destClient.put(putOpts, function (putErr, putReq) {
                    if (putErr) {
                        log.error({
                            error: putErr,
...
function createDestinationMetadata(req, sourceMetadata, destinationSharks,
    ...
    if (req.headers['x-amz-metadata-directive'] === 'REPLACE') {
        copyHeaders = {}; // Clear existing headers
        Object.keys(req.headers).forEach(function (key) {
            var lowerKey = key.toLowerCase();
            // Handle both S3 format (x-amz-meta-*) and internal format (m-*)
