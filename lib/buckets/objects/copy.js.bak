...
function copyObject(req, res, next) {
    ...
    buckets.loadRequest(sourceReq, null, function onSourceRequestLoaded(loadErr) {
        if (loadErr) {
        ...
        buckets.getBucketIfExists(sourceReq, null,
            ...
            getSourceObjectMetadata(sourceReq,
                                    function onSourceMetadataLoaded(metadataErr,
                                                                    sourceMetadata) {
                if (metadataErr) {
...
function performServerSideCopy(req, res, sourceReq, sourceMetadata, callback) {
    ...
    req.storinfo.choose({
        ...
        streamSourceToDestination(req, sourceReq, sourceMetadata,
             sharks, objectId, function onStreamComplete(streamErr, streamResult) {
            if (streamErr) {
        ...
        func: function streamToShark(shark, next) {
            ...
            function attemptSourceRead() {
                sourceClient.get(sourceSharkInfo.opts,
                                 function onSharkSourceStreamReady(getErr,
                                                                   sourceClientReq,
                                                                   sourceRes) {
                    if (getErr || !sourceRes || sourceRes.statusCode !== 200) {
                        retryCount++;
