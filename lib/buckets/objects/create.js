...
function createObject(req, res, next) {
    ...
    buckets.createObjectMetadata(req, type,
            ...
            if (createErr2) {
                ...
                function sendResponse() {
                    ...
                    res.header('Last-Modified',
                               new Date(response_data.modified));
                    res.header('Computed-MD5', req._contentMD5);
                    res.send(200);
                    next(null, response_data);
                }

                // Apply CORS headers using bucket-level CORS configuration
        ...
        // Include roles in props if present in object metadata
        if (object_data.roles && object_data.roles.length > 0) {
            props.roles = object_data.roles;
        }
...
function parseArguments(req, res, next)  {
    ...
    // Check for both Transfer-Encoding: chunked AND
    // aws-chunked (signalled via Content-Encoding or SigV4 streaming sha256)
    if (req.isChunked() || awsChunkedEncoding.isAwsChunked(req.headers)) {
        // For AWS S3 chunked uploads, check for decoded content length first
        var awsDecodedLength = req.header('x-amz-decoded-content-length');
        req.log.debug({
            isChunked: true,
            awsDecodedLength: awsDecodedLength,
            contentLength: req.header('content-length'),
            allHeaders: Object.keys(req.headers)
        }, 'S3_DEBUG: Chunked upload - checking for AWS decoded ' +
             'content length');
        if (awsDecodedLength !== undefined) {
            len = parseInt(awsDecodedLength, 10);
            req.log.debug({
                awsDecodedLength: awsDecodedLength,
                parsedLen: len
            }, 'S3_DEBUG: Using AWS decoded content length for chunked ' +
                'upload');
            if (len < 0) {
                next(new MaxContentLengthError(len));
                return;
            }
            // Handle zero-byte AWS S3 objects
            if (len === 0) {
                req._contentMD5 = common.ZERO_BYTE_MD5;
                req.sharks = [];
                req._zero = true;
                req.log.debug('AWS S3 chunked upload: zero-byte object, ' +
                    'skipping shark allocation');
            } else {
                req.log.debug('AWS S3 chunked upload: using ' +
                    'decoded_content_length=%d', len);
            }
        } else {
            // Fall back to traditional streaming size logic
            ...
            req.log.debug('traditional streaming upload: using ' +
                'max_size=%d', len);
        }
    ...
    req.log.debug({
        ...
        isAwsChunked: awsChunkedEncoding.isAwsChunked(req.headers),
        objectId: req.objectId
    }, 'S3_DEBUG: Final object parameters set');
...
function maybeGetObject(req, res, next) {
    ...
    function onGetObject(err, object_data) {
        ...
        common.setEtag(req, res, object_data);
...
module.exports = {
    createBucketObjectHandler: function createBucketObjectHandler() {
        var chain = [
            ...
            common.addCustomHeaders, // Apply CORS headers
