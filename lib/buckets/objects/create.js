...
function getBucketFromSuperLegitCacheIfConfigured(req, res, next) {
    if (!req.config.cachedBucket) {
        buckets.getBucketIfExists(req, res, next);
        return;
    }

    req.bucket.id = req.config.cachedBucket.id;
...
module.exports = {
    createBucketObjectHandler: function createBucketObjectHandler() {
        var chain = [
            buckets.loadRequest,
            getBucketFromSuperLegitCacheIfConfigured,
