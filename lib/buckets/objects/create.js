...
function parseArguments(req, res, next)  {
    ...
    if (req.isChunked()) {
        ...
        }, 'S3_DEBUG: Chunked upload - checking for AWS decoded content length');
        ...
        if (awsDecodedLength !== undefined) {
            ...
            req.log.info({
                awsDecodedLength: awsDecodedLength,
                parsedLen: len
            }, 'S3_DEBUG: Using AWS decoded content length for chunked upload');
            ...
            if (len === 0) {
                ...
                req.sharks = [];
                req._zero = true;
                req.log.debug('AWS S3 chunked upload: zero-byte object, skipping shark allocation');
            } else {
                req.log.debug('AWS S3 chunked upload: using decoded_content_length=%d', len);
            }
        } else {
            ...
            req.log.debug('traditional streaming upload: using max_size=%d', len);
        }
