...
function deleteObject(req, res, next) {
    ...
    // Add debug logging for .mpu objects
    if (bucketObject.name.indexOf('.mpu-') === 0) {
        log.debug({
            objectName: bucketObject.name,
            nameHash: bucketObject.name_hash,
            metadataLocationVnode: metadataLocation.vnode,
            metadataLocationPnode: metadataLocation.pnode
        }, 'S3_DELETE_DEBUG: Attempting to delete .mpu object');
    }
