...
function getObject(req, res, next) {
    ...
    function onGetObject(err, object_data) {
        ...
        log.debug({
            ...
            roles: object_data.roles,
            hasRoles: !!object_data.roles,
            rolesType: typeof (object_data.roles)
        }, 'getBucketObject: done - object metadata loaded');
    ...
    var conditions = {};
    /*
     * S3 clients send MD5 hex in If-Match but
     * mdapi compares against UUID.  Strip if-match
     * for S3 requests; conditionalHandler handles
     * If-None-Match.
     */
    if (!req.isS3Request) {
        conditions['if-match'] =
            req.conditions['if-match'];
    }
...
module.exports = {
    ...
    getBucketObjectHandler: function getBucketObjectHandler() {
        var chain = [
            ...
            common.addCustomHeaders,  // Add CORS headers processing
