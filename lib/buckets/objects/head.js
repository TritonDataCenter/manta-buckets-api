...
function headObject(req, res, next) {
     ...
     * XXX
     *
     * - Just conditional headers?
     * - buckets-mdapi client needs changes for this.
     *
     * Also totally not hot about this.  It needs to be done (at least for now)
     * because in the case of a precondition failure for GET and HEAD requests
     * with If-Modified-Since and If-None-Match we actually return a 304 and
     * some metadata, not a 412 error.  Perhaps buckets-mdapi needs to handle
     * this instead of buckets-api, but it's living here for now.
     */
    var precondition = {};
    precondition["if-unmodified-since"] = req.headers["if-unmodified-since"];
    precondition["if-match"] = req.headers["if-match"];
