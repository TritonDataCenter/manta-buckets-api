...
function updateObjectMetadata(req, res, next) {
    ...
    buckets.createObjectMetadata(req, type,
        ...
        if (createErr) {
            createErr = common.translateBucketError(req, createErr);
            next(createErr);
            return;
        }
        ...
        var onUpdateObject = function onUpdate(createErr2, response_data) {
            ...
            if (createErr2) {
                createErr2 = common.translateBucketError(req, createErr2);
        ...
        precondition["if-modified-since"] = req.headers["if-modified-since"];
        precondition["if-none-match"] = req.headers["if-none-match"];

        client.updateObject(owner, bucket.id,
            bucketObject.name, objectId, object_data.contentType,
            object_data.headers, props, metadataLocation.vnode, precondition,
            requestId, onUpdateObject);
    });
}
