...
CheckStream.prototype.abandon = function abandon() {
    ...
    // Ensure digest is calculated if not already done
    if (!this._digest) {
        try {
            this._digest = this.hash.digest('buffer');
        } catch (err) {
            // Ignore digest errors during abandonment
        }
    }
...
CheckStream.prototype.digest = function digest(encoding) {
    ...
    if (!this._digest) {
        ...
        // Debug logging for final MD5 calculation - use console temporarily for urgent debugging
        console.log('CheckStream.digest() - Final MD5 calculated:' +
                   ' bytes=' + this.bytes + 
                   ' md5=' + this._digest.toString('base64') +
                   ' hex=' + this._digest.toString('hex') +
                   ' chunks=' + (this._chunkCount || 0));
    }
...
CheckStream.prototype._write = function _write(chunk, encoding, cb) {
    if (this._dead || this._digest) {
        // Stream is dead or digest already called - ignore writes
        cb();
        return;
    }
    var self = this;
    ...
    // Debug logging for AWS chunked decoder troubleshooting
    if (!this._chunkCount) this._chunkCount = 0;
    this._chunkCount++;
    
    // Log chunk details to debug MD5 mismatch - use console temporarily for urgent debugging
    console.log('CheckStream._write chunk #' + this._chunkCount + 
                ', size: ' + chunk.length + 
                ', total bytes so far: ' + this.bytes +
                ', chunk preview: ' + chunk.slice(0, 16).toString('hex'));

    try {
        this.hash.update(chunk, encoding);
    } catch (err) {
        // Handle "Digest already called" error gracefully
        if (err.message &&
            err.message.indexOf('Digest already called') !== -1) {
            // Stream ended, ignore this write
            cb();
            return;
        }
