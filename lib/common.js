...
function sharkStreams(req, res, next) {
    ...
    if (isAwsChunked && req.isS3Request) {
        // AWS chunked encoding adds overhead (chunk sizes, checksums, metadata)
        // Add 20% buffer for encoding overhead, with minimum 1KB buffer
        var overhead = Math.max(Math.ceil(req._size * 0.2), 1024);
        maxBytes = req._size + overhead;
        
        req.log.info({
            originalSize: req._size,
            encodingOverhead: overhead,
            adjustedMaxBytes: maxBytes
        }, 'S3_DEBUG: Adjusted maxBytes for AWS chunked encoding overhead');
    }
