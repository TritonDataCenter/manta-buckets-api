...
function sharkStreams(req, res, next) {
    ...
    // FIX: For AWS chunked uploads, listen to decoder end instead of
    // request end, failure to due so will cause md5 checksum
    // inconsistencies.
    if (isAwsChunked && req.isS3Request && awsDecoder) {
        awsDecoder.once('end', onEnd);
        req.log.debug('sharkStreams: AWS chunked upload' +
                      ' - listening to decoder end event');
    } else {
        req.once('end', onEnd);
    }
