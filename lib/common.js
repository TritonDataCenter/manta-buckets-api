...
function sharkStreams(req, res, next) {
    ...
    var isAwsChunked = req.isChunked() && req.header('content-encoding') === 'aws-chunked';
    
    if (isAwsChunked && req.isS3Request) {
        // AWS chunked encoding can have unpredictable overhead due to chunk sizes, checksums, metadata
        // For S3 compatibility, disable the size check and let underlying Manta limits handle it
        maxBytes = Math.max(req._size * 10, 50 * 1024 * 1024); // 10x file size or 50MB, whichever is larger
        
        req.log.info({
            originalSize: req._size,
            adjustedMaxBytes: maxBytes,
            note: 'AWS chunked encoding - using very generous limit to avoid false size failures'
        }, 'S3_DEBUG: Disabled strict size checking for AWS chunked encoding compatibility');
    }
