...
function sharkStreams(req, res, next) {
    ...
    if (isAwsChunked && req.isS3Request) {
        ...
        if (req.authorization && req.authorization.scheme === 'AWS4-HMAC-SHA256' &&
            ...
            var sigMatch = authzHeader.match(/Signature=([a-f0-9]+)/);

            // Extract credential scope from Authorization header
            // Format: Credential=AKIAIOSFODNN7EXAMPLE/20130524/us-east-1/s3/aws4_request
            var credMatch = authzHeader.match(/Credential=([^,]+)/);
            ...
            if (sigMatch && credMatch) {
                ...
                if (credParts.length === 5) {
                    ...
                    var sigVerifier = require('./aws-chunked-signature-verifier');
                    ...
                    }, 'AWS chunked: invalid credential format in Authorization header');
                }
            } else {
                req.log.warn({
                ...
                }, 'AWS chunked: cannot extract signature or credential from Authorization header');
            }
        } else {
