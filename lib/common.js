...
function findSharks(req, res, next) {
    ...
    if (req.preAllocatedSharks && Array.isArray(req.preAllocatedSharks)) {
        // Wrap pre-allocated sharks in array format expected by startSharkStreams
        // Format should be: [[shark1, shark2]] to match storinfo.choose() output
        ...
        req.log.info({
            ...
            wrappedFormat: true
        }, 'findSharks: using pre-allocated sharks for MPU part (wrapped format)');
        next();
        return;
    }
        ...
        vasync.forEachParallel({
            ...
            if (err || req.sharks.length < req._copies) {
                ...
                if (results && results.operations) {
                    results.operations.forEach(function (op, i) {
                        if (op.err) {
                            ...
                            // Only blacklist on infrastructure errors,
                            // not timeouts
                            var shouldBlacklist = op.err.code ===
                                'ECONNREFUSED' ||
                                op.err.code === 'ENOTFOUND' ||
                                op.err.code === 'ENETUNREACH';

                            // Don't blacklist cueball timeout errors -
                ...
                if (ndx < req._sharks.length) {
                    ...
                    var isMPURequest = req.preAllocatedSharks && Array.isArray(req.preAllocatedSharks);
                    
                    if (isMPURequest) {
                        // For MPU requests with pre-allocated sharks, don't reallocate
                        // All parts must go to the same sharks for v2 commit to work
                        log.warn({
                            reallocationAttempt: reallocationCount + 1,
                            failedSharks: req.failedSharks,
                            preAllocatedSharks: req.preAllocatedSharks.map(function (s) {
                                return s.manta_storage_id;
                            })
                        }, 'startSharkStreams: MPU part failed on pre-allocated sharks, not reallocating');
                        
                        // Fail the request rather than breaking MPU shark consistency
                        next(new SharksExhaustedError(res));
                        return;
                    }
                    
                    // Reallocate with blacklisted sharks filtered out (non-MPU only)
                    log.debug({
                        reallocationAttempt: reallocationCount + 1,
...
function sharkStreams(req, res, next) {
    ...
    barrier.once('drain', function onCompleteStreams() {
        ...
        // Skip MD5 check for AWS chunked multipart uploads
        // due to decoder issues
        ...
        if (!skipMd5Check && req.sharks.some(function (s) {
            ...
            if (skipMd5Check) {
                log.debug('sharkStreams:' +
                          ' done (skipped MD5 check for AWS chunked MPU)');
            } else {
