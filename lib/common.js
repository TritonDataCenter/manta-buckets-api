...
function elapsedSince(start) {
    ...
    return (Math.ceil((timeDelta[0] * 1e6) + (timeDelta[1] / 1e3)));
}
...
function sharkStreams(req, res, next) {
    ...
    function _lapTime(name) {
        if (!timers.hasOwnProperty(name)) {
            timers[name] = 0;
        }
        timers[name] += elapsedSince(startTime);
        startTime = process.hrtime();
    }

    function _dumpTimers() {
       log.info({
            source: 'hacketyhotpatch',
    ...
    var next_err = once(function _next_err(err) {
        ...
        req.removeListener('error', next_err);
         ...
         _lapTime('sharkStreams-abandoningSharks');

        req.abandonSharks();
        req.unpipe(check);
        check.abandon();

         _lapTime('sharkStreams-abandonedSharks');
    ...
    _lapTime('sharkStreams-preBarrier');

    barrier.start('client');
    req.pipe(check);
    req.sharks.forEach(function (s) {
        var thisIdx = idx++;
        _lapTime('sharkStreams-forEach' + thisIdx);
        barrier.start(s._shark.manta_storage_id);
        req.pipe(s);
        s.once('response', function onSharkResult(sres) {
            _lapTime('sharkStreams-forEach' + thisIdx + '-response');
            log.debug({
            ...
            _lapTime('sharkStreams-forEach' + thisIdx + '-getSharkInfo');
    ...
    if (req.header('expect') === '100-continue') {
        _lapTime('sharkStreams-100-Continue');
        res.writeContinue();
        log.info({
