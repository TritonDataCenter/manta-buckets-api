...
function addCustomHeaders(req, res, next) {
    ...
    if (!res.getHeader('access-control-allow-origin') &&
        ...
        tryBucketLevelCors(req, res, origin);
    }

    // Continue to next middleware (e.g., streamFromSharks) if
    // this is used as middleware
    if (next && typeof (next) === 'function') {
        next();
    }
}

function tryBucketLevelCors(req, res, origin) {
    ...
    log.debug('tryBucketLevelCors: attempting to load bucket CORS config');
    ...
    var objectLocation = req.metadataPlacement.getObjectLocation(owner,
        req.bucket.id || req.bucket.name, corsNameHash);
    var objectClient = req.metadataPlacement.
        getBucketsMdapiClient(objectLocation);

    objectClient.getObject(owner, req.bucket.id ||
                           req.bucket.name, corsObjectName,
        objectLocation.vnode, {}, requestId, function (getErr, corsObjectData) {
        if (getErr) {
            log.debug('tryBucketLevelCors: no bucket CORS config found');
            return;
        }
