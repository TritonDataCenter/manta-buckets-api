...
var CORS_OBJECT_NAME = '.cors-configuration';
var CORS_NAME_HASH = crypto.createHash('md5')
    .update(CORS_OBJECT_NAME).digest('hex');
...
/*
 * Convert base64-encoded MD5 to 32-char hex string
 * for S3 ETag compatibility.  Returns null if input
 * is empty or an invalid base64.
 */
function md5Base64ToHex(base64Md5) {
    if (!base64Md5) {
        return (null);
    }
    try {
        return (Buffer.from(base64Md5, 'base64')
            .toString('hex'));
    } catch (e) {
        return (null);
    }
...
/*
 * Set the ETag response header. S3 clients expect MD5 hex; native Manta
 * clients expect the object UUID.
 */
function setEtag(req, res, object_data) {
    if (req.isS3Request) {
        res.set('Etag',
            md5Base64ToHex(object_data.content_md5) ||
            object_data.id);
    } else {
...
function tryBucketLevelCors(req, res, origin, callback) {
    ...
    var corsObjectName =  CORS_OBJECT_NAME;
    ...
    var corsNameHash = CORS_NAME_HASH;
    ...
    objectClient.getObject(owner, bucketIdentifier, corsObjectName,
        objectLocation.vnode, {}, requestId, handleCorsObjectRetrieval);

    function handleCorsObjectRetrieval(getErr, corsObjectData) {
        if (getErr) {
            log.debug({
                err: getErr,
                errCode: getErr.code,
        ...
        processCorsConfig(corsObjectData);
    }

    function processCorsConfig(corsObjectData) {
        try {
            ...
            var matchingRule = findMatchingCorsRule(corsRules, origin, log);
            if (matchingRule) {
                applyCorsHeaders(res, matchingRule.rule,
                    matchingRule.matchedOrigin, origin, log);
            }
...
/**
 * Find matching origin in allowed origins list
 * @param {Array} allowedOrigins - List of allowed origins
 * @param {String} origin - Request origin
 * @returns {String|null} Matched origin or null
 */
function findMatchingOrigin(allowedOrigins, origin) {
    for (var i = 0; i < allowedOrigins.length; i++) {
        var allowedOrigin = allowedOrigins[i];
...
/**
 * Find matching CORS rule for the given origin
 * @param {Array} corsRules - List of CORS rules
 * @param {String} origin - Request origin
 * @param {Object} log - Logger instance
 * @returns {Object|null} Object with rule and matchedOrigin, or null
 */
function findMatchingCorsRule(corsRules, origin, log) {
    for (var i = 0; i < corsRules.length; i++) {
        var rule = corsRules[i];
        var allowedOrigins = rule.AllowedOrigins || [];
        var matchedOrigin = findMatchingOrigin(allowedOrigins, origin);
...
/**
 * Apply CORS headers to response based on matching rule
 * @param {Object} res - Response object
 * @param {Object} rule - CORS rule
 * @param {String} matchedOrigin - Matched origin (* or specific origin)
 * @param {String} origin - Request origin
 * @param {Object} log - Logger instance
 */
function applyCorsHeaders(res, rule, matchedOrigin, origin, _log) {
    // Set Access-Control-Allow-Origin
    if (matchedOrigin === '*' && (!origin || origin === 'null')) {
        // For presigned URLs (no origin), use wildcard when allowed
        res.setHeader('Access-Control-Allow-Origin', '*');
    } else {
        // Always reflect the actual origin for security when origin is present
    ...
    if (rule.MaxAgeSeconds) {
        res.setHeader('Access-Control-Max-Age', rule.MaxAgeSeconds);
    }

    // Set Access-Control-Allow-Credentials for credentials requests
    if (rule.AllowCredentials === true || rule.AllowCredentials === 'true') {
        ...
        if (currentExposeHeaders &&
            ...
            includes('Access-Control-Allow-Credentials')) {
            currentExposeHeaders += ', Access-Control-Allow-Credentials';
        } else if (!currentExposeHeaders) {
            currentExposeHeaders = 'Access-Control-Allow-Credentials';
        }
        res.header('Access-Control-Expose-Headers', currentExposeHeaders);
    }
}
...
function findSharks(req, res, next) {
    ...
    opts.log = req.log;
    req.storinfo.choose(opts, function onSharksChosen(err, sharks) {
        if (err) {
            next(err);
        } else {
            req._sharks = sharks;
...
function startSharkStreams(req, res, next) {
    ...
    (function attempt(inputs) {
        vasync.forEachParallel({
            ...
            inputs: inputs
        }, function onSharkConnectionsComplete(err, results) {
            ...
            if (err || req.sharks.length < req._copies) {
                ...
                if (ndx < req._sharks.length) {
                    ...
                    storinfo.choose(reallocOpts,
                        function onSharksReallocated(reallocErr,
                            availableSharks) {
                        if (reallocErr) {
                            log.debug({
                                err: reallocErr,
...
function sharkStreams(req, res, next) {
    ...
    var isAwsChunked = req._awsChunkedMPU || // Support multipart uploads
        // Detect aws-chunked via either Content-Encoding: aws-chunked
        // or x-amz-content-sha256: STREAMING-AWS4-HMAC-SHA256-PAYLOAD
        // (the SigV4 streaming signal used by mc/SDKs).
        (req.isS3Request && awsChunkedEncoding.isAwsChunked(req.headers));
    ...
    if (isAwsChunked && req.isS3Request) {
        // Extract verification context for chunk signature validation
        var verificationContext = {};

        // Only set up verification if we have SigV4 auth with secret key
        if (req.authorization &&
            req.authorization.scheme === 'AWS4-HMAC-SHA256' &&
            req.auth && (req.auth.secretAccessKey ||
            req.auth.signingKey)) {

            var authzHeader = req.headers.authorization;

            // Extract seed signature from Authorization header
            var sigMatch = authzHeader.match(
                new RegExp('Signature' + '=' + '([a-f0-9]+)'));

            // Extract credential scope from Authorization header
            // Format:
            // Credential=AKIAIOSFODNN7EXAMPLE/20130524/...
            var credMatch = authzHeader.match(
                new RegExp('Credential' + '=' + '([^,]+)'));

            if (sigMatch && credMatch) {
                var seedSignature = sigMatch[1];
                var credential = credMatch[1];
                ...
                if (credParts.length === 5) {
                    ...
                    var credentialScope = credParts.slice(1).join('/');

                    // Derive signing key from secret access key
                    var sigVerifier = awsChunkedSignatureVerifier;

                    try {
                        var signingKey;
                        if (req.auth.signingKey) {
                            signingKey = req.auth.signingKey;
                        } else {
                            signingKey = sigVerifier.deriveSigningKey(
                                req.auth.secretAccessKey,
                                dateStamp,
                                region,
                                service);
                        }
                        ...
                        req.log.debug({
                            hasSeedSignature: true,
                            hasSigningKey: true,
                            credentialScope: credentialScope,
                            region: region,
                            service: service
                        }, 'AWS chunked: signature verification enabled');
                    } catch (keyErr) {
                        req.log.error({
                            err: keyErr,
                            message: keyErr.message
                        }, 'AWS chunked: failed to derive signing key');
                    }
                } else {
                    req.log.warn({
                        credentialFormat: credential,
                        partsCount: credParts.length
                    }, 'AWS chunked: invalid credential format ' +
                        'in Authorization header');
                }
            } else {
                req.log.warn({
                    hasAuthorization: !!authzHeader,
                    hasSigMatch: !!sigMatch,
                    hasCredMatch: !!credMatch
                }, 'AWS chunked: cannot extract signature or ' +
                    'credential from Authorization header');
            }
        } else {
            // Missing required context for verification
            var logData = {
                hasAuthorization: !!req.authorization,
                authScheme: req.authorization ? req.authorization.scheme : null,
                hasAuth: !!req.auth,
                hasSecretKey: req.auth ? !!req.auth.secretAccessKey : false
            };

            req.log.warn(logData,
                'AWS chunked: signature verification disabled - ' +
                'missing SigV4 authorization or secret key');
        }

        // Create decoder with verification context
        awsDecoder = new AwsChunkedDecoder(
            Object.assign({
                log: req.log.child({component: 'aws-chunked-decoder'})
            }, verificationContext));

        // Warn if processing aws-chunked without signature verification
        if (!verificationContext.validateSignatures) {
            req.log.warn({
                hasVerificationContext: Object.keys(verificationContext)
                    .length > 0,
                contentEncoding: req.headers['content-encoding']
            }, 'AWS chunked: processing chunked upload ' +
                'without signature verification - security risk');
        }

        // Set up error handling for decoder
        awsDecoder.on('error', function onDecoderError(err) {
            req.log.error({
                error: err.message,
        ...
        if (req._awsChunkedMPU || isAwsChunked && req.isS3Request) {
            awsDecoder.on('trailerHeaders', function onTrailerHeaders(headers) {
                req.log.debug({
    ...
    req.sharks.forEach(function (s) {
        ...
        sharkStream.once('response', function onSharkResult(sres) {
            ...
            if (sres.statusCode === 469) {
                ...
                sres.once('end', function onSharkErrorResponseEnd() {
                    log.debug({
                        mako: sharkId,
                        client_res: sres,
                        body: body
                ...
                sres.once('error', function onSharkResponseError(err) {
...
function streamFromSharks(req, res, next) {
    ...
    function respond(shark, sharkReq, sharkInfo) {
        ...
        // Monitor connection state during streaming (event-driven)
        var connectionCleaned = false;

        function cleanupOnDisconnect(reason) {
            ...
            log.warn({
                ...
                reason: reason
            }, 'streamFromSharks: connection closed, aborting streams');
        ...
        function onConnectionError(err) {
            cleanupOnDisconnect('connection error: ' + (err.message || err));
        }

        req.connection.once('close', onConnectionClose);
        req.connection.once('error', onConnectionError);

        // Clean up listeners when check completes
        check.once('done', function () {
            req.connection.removeListener('close', onConnectionClose);
            req.connection.removeListener('error', onConnectionError);
        });

        // Clean up listeners on errors
    ...
    queue = vasync.queuev({
        ...
        worker: function start(s, cb) {
            if (connected) {
                ...
                s.get(opts, function onSharkGetComplete(err, cReq, cRes) {
                    if (err) {
                        sharkInfo.result = 'fail';
                        sharkInfo.timeTotal = Date.now() - sharkInfo._startTime;

                        // Calculate expected storage path for debugging
                        var expectedPath = sharkClient.storagePath({
    ...
    queue.once('end', function onSharkQueueEnd() {
        if (!connected) {
...
function sharkConnect(opts, sharkInfo, cb) {
    ...
    client.put(opts, function onSharkPutStarted(err, req) {
        if (err) {
            cb(err);
        } else {
...
/*
 * Check if an error or a wrapped cause is a client error.
 * Client errors are for example a 404 when an object is not
 * found, so as this is a common situation we don't need to
 * log it.
 * @param {Object} err - error object
 * @returns {boolean} true if error is a client error, false if not.
 */
function isClientError(err) {
    if (err && err.statusCode >= 400 && err.statusCode < 500)
        return (true);
    // Multierror
    if (err && err.ase_errors && Array.isArray(err.ase_errors)) {
        return (err.ase_errors.every(isClientError));
    }
    // Verror cause chain
    if (err && err.cause && typeof (err.cause) === 'function') {
        var cause = err.cause();
        if (cause)
            return (isClientError(cause));
    }
    return (false);
}
