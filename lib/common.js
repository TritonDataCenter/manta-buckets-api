...
function sharkStreams(req, res, next) {
    ...
    if (isAwsChunked && req.isS3Request) {
        ...
        // Handle finish event to ensure proper stream completion
        awsDecoder.on('finish', function () {
            req.log.debug({
                isMultipartUpload: !!req._awsChunkedMPU,
                totalDecoded: awsDecoder._totalBytesDecoded
            }, 'AWS chunked decoder: stream finished');
        });
        ...
        // Debug: Log when data flows through the pipeline
        req.on('data', function (chunk) {
            req.log.debug({
                reqDataChunkSize: chunk.length
            }, 'AWS chunked: request data received');
        });

        awsDecoder.on('data', function (chunk) {
            req.log.info({
                decoderDataChunkSize: chunk.length
            }, 'AWS chunked: decoder output data');
        });

        // Debug CheckStream events
        check.on('finish', function () {
            req.log.info('CheckStream: finish event received');
        });

        check.on('done', function () {
            req.log.info('CheckStream: ' +
                         'done event received (before our handler)');
        });
    }
    ...
    if (isAwsChunked && req.isS3Request && awsDecoder) {
        ...
        function maybeCallOnEnd() {
            req.log.info({
                decoderEnded: decoderEnded,
                checkStreamDone: checkStreamDone
            }, 'AWS chunked coordination: checking if ready to call onEnd');
            if (decoderEnded && checkStreamDone) {
                req.log.info('AWS chunked coordination:' +
                             ' calling onEnd - both conditions met');
                onEnd();
            }
        }

        awsDecoder.once('end', function () {
            req.log.info('AWS chunked decoder: end event received ' +
                         '- setting decoderEnded=true');
            decoderEnded = true;
            maybeCallOnEnd();
        });
        ...
        check.once('done', function () {
            req.log.info('CheckStream: done event received ' +
                         '- setting checkStreamDone=true');
            checkStreamDone = true;
            barrier.done('check_stream');
            maybeCallOnEnd();
        });

        req.log.debug('sharkStreams: AWS chunked upload' +
            ' - coordinating decoder end and CheckStream done events');
    } else {
        req.once('end', onEnd);
    }
