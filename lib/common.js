        ...
        if (req._awsChunkedMPU) {
            awsDecoder.on('trailerHeaders', function (headers) {
                ...
                if (headers['x-amz-checksum-crc64nvme']) {
                    req._awsTrailerChecksum = headers['x-amz-checksum-crc64nvme'];
                }
            });
        }
    ...
    function respond(shark, sharkReq, sharkInfo) {
        ...
        if (req.headers['range'] !== undefined) {
            log.info({
                ...
                isAwsCli: isAwsCli,
                objectId: md.objectId
            }, 'streamFromSharks: RANGE REQUEST DEBUG - shark response details');
            ...
            if (shark.statusCode !== 206) {
                log.warn({
                    ...
                    expected: 206,
                    objectId: md.objectId
                }, 'streamFromSharks: WARNING - shark did not return 206 for range request');
            }
            ...
            if (!sh['content-range']) {
                log.error({
                    ...
                    sharkHeaders: sh,
                    objectId: md.objectId
                }, 'streamFromSharks: ERROR - shark missing Content-Range header for range request');
            }
        ...
        if (isAwsCli && req.headers.range) {
            ...
            res.header('Content-Length', sh['content-length']); // Only the range size
            ...
            try {
                ...
                log.debug(e, 'streamFromSharks: Socket option setting failed (non-critical)');
            }
            ...
            log.debug({
                ...
                objectId: md.objectId,
                requestId: req.getId()
            }, 'streamFromSharks: AWS CLI range request - configured headers and socket');
        }
        ...
        if (isAwsCli) {
            log.debug({
                ...
                hasRangeHeader: !!req.headers.range,
                objectId: md.objectId
            }, 'streamFromSharks: AWS CLI detected, enabling enhanced error handling');
        }
        ...
        if (isAwsCli && req.headers.range) {
            ...
            }, 'streamFromSharks: AWS CLI range response - headers before writeHead');
            ...
            log.debug({
                ...
                objectId: md.objectId
            }, 'streamFromSharks: AWS CLI range response - status after writeHead');
            
            // Immediate validation - check if client is still connected after writeHead
            if (req.connection.destroyed || !req.connection.writable) {
                log.error({
                    objectId: md.objectId,
                    rangeHeader: req.headers.range,
                    statusCode: res.statusCode
                }, 'streamFromSharks: AWS CLI disconnected immediately after writeHead');
                return;
            }
        } else {
        ...
        if (isAwsCli) {
            ...
            var connectionMonitor = setInterval(function() {
                if (req.connection.destroyed || !req.connection.writable) {
                    ...
                    }, 'streamFromSharks: AWS CLI connection closed, aborting streams');
                    ...
                    try {
                        ...
                        log.debug(e, 'streamFromSharks: Error during stream cleanup');
                    }
                    return;
                }
        ...
        if (isAwsCli && req.headers.range) {
            ...
            shark.on('error', function(err) {
                log.warn({
                    ...
                    objectId: md.objectId,
                    rangeHeader: req.headers.range
                }, 'streamFromSharks: shark stream error during AWS CLI range request');
            });
            ...
            check.on('error', function(err) {
                ...
                }, 'streamFromSharks: check stream error during AWS CLI range request');
            });
            ...
            if (connected) {
                ...
                s.get(opts, function (err, cReq, cRes) {
                    if (err) {
                        ...
                        var expectedPath = require('./shark_client').storagePath({
                        ...
                        log.warn({
                            ...
                            storageLayoutVersion: opts.storageLayoutVersion
                        }, 'mako: connection failed - multipart object might not exist at expected path');
                        savedErr = err;
                        cb();
                    } else {
                        sharkInfo.result = 'ok';
