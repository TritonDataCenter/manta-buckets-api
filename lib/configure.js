...
function configure(appName, opts, dtProbes) {
    ...
    assert.optionalObject(cfg.cachedBucket, 'cfg.cachedBucket');
    if(cfg.cachedBucket) {
        assert.uuid(cfg.cachedBucket.id, 'cfg.cachedBucket.id');
    }
    ...
    [ cfg.auth, cfg.cueballHttpAgent, cfg.sharkConfig, cfg.buckets_mdapi
    ].forEach(function (x) {
        x.log = cfg.log;
    });
