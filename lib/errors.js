 ...
 */
/**
 * @brief Access key not yet available in auth cache
 *
 * Returned when a newly created key has not yet
 * replicated from UFDS to the mahi Redis cache.
 * The 503 status with Retry-After header tells S3
 * clients to retry automatically.
 *
 * @since 2.3.0
 */
function AccessKeyNotYetAvailableError() {
    BucketsApiError.call(this, {
        restCode: 'ServiceUnavailable',
        statusCode: 503,
        message: 'Access key not yet available.' +
            ' Retry in a few seconds.'
    });
}
util.inherits(AccessKeyNotYetAvailableError,
    BucketsApiError);


function AccessDeniedByKeyScopeError() {
    BucketsApiError.call(this, {
