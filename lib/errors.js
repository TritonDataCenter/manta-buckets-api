...
/**
 * @brief Denied by per-bucket access key scope
 *
 * Returned when a scoped access key attempts an
 * operation that its bucket scope does not permit.
 * Distinct from other 403 errors for operator
 * troubleshooting.
 *
 * @param {string} bucket - Target bucket name
 * @param {string} action - Attempted action
 */
function AccessDeniedByKeyScopeError(bucket, action) {
    BucketsApiError.call(this, {
        restCode: 'AccessDeniedByKeyScope',
        statusCode: 403,
...
util.inherits(AccessDeniedByKeyScopeError, BucketsApiError);
...
function BucketNotEmptyError(bucket) {
    BucketsApiError.call(this, {
        ...
        statusCode: 409,
        message: sprintf('The bucket you tried to delete is not empty: %s',
                         bucket)
    });
}
util.inherits(BucketNotEmptyError, BucketsApiError);
...
function InvalidSignatureError(message) {
    var errorMessage = message ||
        'The signature we calculated does not match the one you sent';
    BucketsApiError.call(this, {
...
function InvalidChunkSignatureError(chunkNumber) {
    var message = 'The chunk signature we calculated does not match the ' +
...
util.inherits(InvalidChunkSignatureError, BucketsApiError);
...
/*
 * Moved here from lib/s3-multipart.js where they were defined as
 * plain Error returns rather than RestError subclasses. Restify
 * does not serialize plain Errors with tacked-on statusCode/restCode
 * properties; it falls back to 500 InternalError on the wire.
 * Inheriting from BucketsApiError -> restify RestError gives us the
 * correct S3-style XML body and HTTP status code.
 */

function NoSuchUploadError(uploadId) {
    BucketsApiError.call(this, {
        restCode: 'NoSuchUpload',
        statusCode: 404,
        message: sprintf('The specified upload (%s) does not exist',
                         uploadId || 'unknown')
    });
}
util.inherits(NoSuchUploadError, BucketsApiError);


function InvalidPartNumberError(partNumber) {
    BucketsApiError.call(this, {
        restCode: 'InvalidPartNumber',
        statusCode: 400,
        message: sprintf('Part number %s is not between 1 and 10000',
                         partNumber)
    });
}
util.inherits(InvalidPartNumberError, BucketsApiError);


function InvalidPartOrderError() {
    BucketsApiError.call(this, {
        restCode: 'InvalidPartOrder',
        statusCode: 400,
        message: 'The list of parts was not in ascending order'
    });
}
util.inherits(InvalidPartOrderError, BucketsApiError);
...
function InvalidPartError(message) {
    BucketsApiError.call(this, {
        ...
        statusCode: 400,
        message: message || 'Invalid part in multipart upload'
    });
}
util.inherits(InvalidPartError, BucketsApiError);
...
function EntityTooSmallError(message) {
    BucketsApiError.call(this, {
        ...
        statusCode: 400,
        message: message || 'Part is too small'
    });
}
util.inherits(EntityTooSmallError, BucketsApiError);


function EntityTooLargeError(message) {
    BucketsApiError.call(this, {
        restCode: 'EntityTooLarge',
        statusCode: 400,
        message: message || 'Entity too large'
    });
}
util.inherits(EntityTooLargeError, BucketsApiError);


function MalformedXMLError(message) {
    BucketsApiError.call(this, {
        restCode: 'MalformedXML',
        statusCode: 400,
        message: message || 'The XML you provided was not well-formed'
    });
}
util.inherits(MalformedXMLError, BucketsApiError);


function InvalidRequestError(message) {
    BucketsApiError.call(this, {
        restCode: 'InvalidRequest',
        statusCode: 400,
        message: message || 'Invalid request'
    });
}
util.inherits(InvalidRequestError, BucketsApiError);
 ...
 */
function ParentNotBucketError(_req) {
    BucketsApiError.call(this, {
...
function ParentNotBucketRootError(_req) {
    BucketsApiError.call(this, {
...
// Export translateError function
