 ...
 */
/**
 * @brief Create IAM client instance
 *
 * @param {Object} config - Server configuration
 * @param {Object} log - Bunyan logger
 * @param {Object} [agent] - Cueball HTTP agent for
 *   DNS resolution via binder.
 */
function createIAMClient(config, log, agent) {
...
function createRoleHandler(iamClient) {
    return function handleCreateRole(req, res, next) {
        ...
        // Parse from body or query parameters (body takes precedence)
        roleName = req.body.RoleName || req.query.RoleName;
        assumeRolePolicyDocument = req.body.AssumeRolePolicyDocument ||
            req.query.AssumeRolePolicyDocument;
        ...
        iamClient.createRole({
            ...
            path: path,
            caller: req.caller
        }, function handleCreateRoleResponse(err, result) {
            if (err) {
...
function getRoleHandler(iamClient) {
    return function handleGetRole(req, res, next) {
        ...
        // Parse IAM parameters (body takes precedence)
        var roleName = req.body.RoleName || req.query.RoleName;
        ...
        iamClient.getRole({
            roleName: roleName,
            caller: req.caller
        }, function handleGetRoleResponse(err, result) {
            if (err) {
...
function putRolePolicyHandler(iamClient) {
    return function handlePutRolePolicy(req, res, next) {
        ...
        // Parse from body or query parameters (body takes precedence)
        roleName = req.body.RoleName || req.query.RoleName;
        policyName = req.body.PolicyName || req.query.PolicyName;
        policyDocument = req.body.PolicyDocument || req.query.PolicyDocument;
        ...
        iamClient.putRolePolicy({
            ...
            caller: req.caller
        }, function handlePutRolePolicyResponse(err, _result) {
            if (err) {
...
function deleteRolePolicyHandler(iamClient) {
    return function handleDeleteRolePolicy(req, res, next) {
        ...
        iamClient.deleteRolePolicy({
            ...
            policyName: policyName,
            caller: req.caller
        }, function handleDeleteRolePolicyResponse(err, _result) {
            if (err) {
...
function deleteRoleHandler(iamClient) {
    return function handleDeleteRole(req, res, next) {
        ...
        iamClient.deleteRole({
            roleName: roleName,
            caller: req.caller
        }, function handleDeleteRoleResponse(err, _result) {
            if (err) {
...
function listRolesHandler(iamClient) {
    return function handleListRoles(req, res, next) {
        ...
        // Extract parameters (body takes precedence)
        var maxItems = req.body.MaxItems || req.query.MaxItems || 100;
        var marker = req.body.Marker || req.query.Marker;

        iamClient.listRoles({
            maxItems: maxItems,
            marker: marker,
            caller: req.caller
        }, function handleListRolesResponse(err, result) {
            if (err) {
...
function buildListRolesXMLResponse(result) {
    ...
    if (result.roles && result.roles.length > 0) {
        result.roles.forEach(function formatEachRole(role) {
            xml += '      <member>\n';
...
function listRolePoliciesHandler(iamClient) {
    return function handleListRolePolicies(req, res, next) {
        ...
        var roleName = req.body.RoleName || req.query.RoleName;
        var marker = req.body.Marker || req.query.Marker;
        var maxItems = parseInt(req.body.MaxItems ||
                                req.query.MaxItems || '100', 10);
        ...
        iamClient.listRolePolicies({
            ...
            maxItems: maxItems,
            caller: req.caller
        }, function handleListRolePoliciesResponse(err, result) {
            if (err) {
...
function getRolePolicyHandler(iamClient) {
    return function handleGetRolePolicy(req, res, next) {
        ...
        // Parse IAM parameters (body takes precedence)
        var roleName = req.body.RoleName || req.query.RoleName;
        var policyName = req.body.PolicyName || req.query.PolicyName;
        ...
        iamClient.getRolePolicy({
            ...
            policyName: policyName,
            caller: req.caller
        }, function handleGetRolePolicyResponse(err, result) {
            if (err) {
...
function buildListRolePoliciesXMLResponse(result) {
    ...
    if (result.PolicyNames && Array.isArray(result.PolicyNames)) {
        xml += '    <PolicyNames>\n';
        result.PolicyNames.forEach(function formatEachPolicyName(policyName) {
