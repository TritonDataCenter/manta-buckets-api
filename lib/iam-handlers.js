...
function buildCreateRoleXMLResponse(result) {
    ...
    xml += '    <Role>\n';
    xml += '      <Path>' + escapeXml(role.Path) + '</Path>\n';
    xml += '      <RoleName>' + escapeXml(role.RoleName) + '</RoleName>\n';
    xml += '      <RoleId>' + escapeXml(role.RoleId) + '</RoleId>\n';
    xml += '      <Arn>' + escapeXml(role.Arn) + '</Arn>\n';
    xml += '      <CreateDate>' + escapeXml(role.CreateDate) + '</CreateDate>\n';
    xml += '      <AssumeRolePolicyDocument>' + escapeXml(role.AssumeRolePolicyDocument) + '</AssumeRolePolicyDocument>\n';
    if (role.Description) {
        xml += '      <Description>' + escapeXml(role.Description) + '</Description>\n';
    }
...
function buildIAMErrorXMLResponse(error) {
    ...
    var errorMessage = escapeXml(error.message || 'An error occurred');

    return '<?xml version="1.0" encoding="UTF-8"?>\n' +
           '<ErrorResponse>\n' +
           '  <Error>\n' +
           '    <Type>Sender</Type>\n' +
           '    <Code>' + escapeXml(errorCode) + '</Code>\n' +
...
function listRolesHandler(iamClient) {
    return function handleListRoles(req, res, next) {
        ...
        iamClient.listRoles({
            ...
            if (err) {
                log.error({err: err}, 'IAM ListRoles failed');
...
function buildListRolesXMLResponse(result) {
    ...
    if (result.roles && result.roles.length > 0) {
        result.roles.forEach(function (role) {
            xml += '      <member>\n';
            xml += '        <RoleName>' + escapeXml(role.RoleName || role.name) + '</RoleName>\n';
            xml += '        <Arn>' + escapeXml(role.Arn) + '</Arn>\n';
            xml += '        <CreateDate>' + (role.CreateDate || new Date().toISOString()) + '</CreateDate>\n';
            xml += '        <Path>' + escapeXml(role.Path || '/') + '</Path>\n';
            if (role.AssumeRolePolicyDocument) {
                xml += '        <AssumeRolePolicyDocument>' + escapeXml(encodeURIComponent(role.AssumeRolePolicyDocument)) + '</AssumeRolePolicyDocument>\n';
            }
            xml += '      </member>\n';
        });
    }
    ...
    if (result.Marker) {
        xml += '    <Marker>' + escapeXml(result.Marker) + '</Marker>\n';
    }
...
function buildListRolePoliciesXMLResponse(result) {
    ...
    if (result.PolicyNames && Array.isArray(result.PolicyNames)) {
        ...
        result.PolicyNames.forEach(function (policyName) {
            xml += '      <member>' + escapeXml(policyName) + '</member>\n';
        });
        xml += '    </PolicyNames>\n';
    } else {
...
function buildGetRolePolicyXMLResponse(result) {
    ...
    xml += '    <RoleName>' + escapeXml(result.RoleName) + '</RoleName>\n';
    xml += '    <PolicyName>' + escapeXml(result.PolicyName) + '</PolicyName>\n';
    xml += '    <PolicyDocument>' + escapeXml(result.PolicyDocument) + '</PolicyDocument>\n';
    xml += '  </GetRolePolicyResult>\n';
    xml += '  <ResponseMetadata>\n';
    xml += '    <RequestId>' + generateRequestId() + '</RequestId>\n';
    xml += '  </ResponseMetadata>\n';
