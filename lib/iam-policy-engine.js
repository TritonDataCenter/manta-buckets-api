    ...
    for (var i = 0; i < policyDoc.Statement.length; i++) {
        ...
        if (statement.Effect !== 'Allow') {
            log.debug({
                statementIndex: i,
                effect: statement.Effect
            }, 'IAM_DEBUG: Skipping non-Allow statement');
            continue;
        }

        log.debug({
            statementIndex: i
        }, 'IAM_DEBUG: Evaluating Allow statement');

        if (evaluateStatement(statement, action, resource,
            log)) {
            return (true);
        }
    }

    log.debug('IAM_DEBUG: No statements matched - denying');
    return (false);
}
