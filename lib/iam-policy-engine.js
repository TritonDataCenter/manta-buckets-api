...
/**
 * Match a string against an IAM-style glob pattern.
 *
 * AWS IAM wildcard matching supports exactly two operators:
 *   *  matches any combination of characters (zero or more)
 *   ?  matches exactly one character
 * All other characters are case-sensitive literals.
 *
 * This function implements iterative glob matching (same approach
 * as fnmatch(3) in libc) with no regex engine involvement.
 *
 * Reference:
 * https://docs.aws.amazon.com/IAM/latest/UserGuide/
 *   reference_policies_elements_condition_operators.html
 *
 * @param {String} pattern - IAM pattern (may contain * and ?)
 * @param {String} str - The string to match against
 * @return {Boolean} true if str matches pattern
 */
function iamGlobMatch(pattern, str) {
    var pIdx = 0;       // position in pattern
    var sIdx = 0;       // position in str
    var starPIdx = -1;  // pattern position after last *
    var starSIdx = -1;  // str position when last * was hit

    while (sIdx < str.length) {
        if (pIdx < pattern.length &&
            (pattern[pIdx] === str[sIdx] ||
             pattern[pIdx] === '?')) {
            // Literal match or ? matches one char
            pIdx++;
            sIdx++;
            ...
            // Mismatch — backtrack: * matches one more char
            pIdx = starPIdx + 1;
            starSIdx++;
            sIdx = starSIdx;
        } else {
            // No match, no * to backtrack to
            return (false);
        }
    }
...
function matchesAction(statementActions, requestAction) {
    ...
    for (var i = 0; i < actions.length; i++) {
        ...
        // Handle IAM wildcards: * (any chars) and ? (one char)
        if (actionPattern.indexOf('*') !== -1 ||
            actionPattern.indexOf('?') !== -1) {
            if (iamGlobMatch(actionPattern, requestAction)) {
                return (true);
            }
        }
    }
...
function matchesResource(statementResources, requestResource) {
    ...
    for (var i = 0; i < resources.length; i++) {
        ...
        if (resourcePattern.indexOf('*') !== -1 ||
            resourcePattern.indexOf('?') !== -1) {
            if (iamGlobMatch(resourcePattern, requestResource)) {
                return (true);
            }
        }
    }
