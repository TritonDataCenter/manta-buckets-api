...
module.exports.ANONYMOUS_USER = ANONYMOUS_USER;
module.exports.PATH_LOGIN_RE = PATH_LOGIN_RE;
module.exports.normalizeMantaPath = normalizeMantaPath;
module.exports.authorize = authorize;

/**
 * Given a resource like `/mark/buckets/my%20bucket`, this API will convert to
 * `/mark/buckets/my bucket`; if you optionally pass in an `account` object as
 * returned from mahi.js (in this lib), the path will be converted to
 * `/d3d38d1a-e26c-11e2-9a8a-475696be42f1/buckets/my bucket`, which is the
 * correct normal form everwhere in the system.
 *
 * This function also acts as a form of validation for URL form.
 *
...
function normalizeMantaPath(opts, cb) {
    ...
    try {
         ...
         * ]
         *
         * This gets validated belowe
         */
        var parts = opts.path.split('/');

        assert.equal(parts[0], '', 'blank first element');
        ...
        log.warn(e, 'failed to normalizeMantaPath');
        cb(e);
        return;
    }

    log.debug({original: opts.path, normalized: key}, 'normalized manta path');
    cb(null, key);
}
