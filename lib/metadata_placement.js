...
function MetadataPlacement(opts) {
    ...
    this.buckets_mdplacement_client.once('connect', function _onConnect() {
        ...
        self.buckets_mdplacement_client.getPlacementData(function (pErr, pd) {
            ...
            // Create a filtered logger for buckets-mdapi to reduce user error noise
            var mdapiLogger = self.log.child({
                component: 'buckets-mdapi-client'
            });
            
            // Override error method to suppress known user errors
            var originalError = mdapiLogger.error.bind(mdapiLogger);
            var suppressUserErrors = process.env.SUPPRESS_USER_ERROR_LOGS !== 'false'; // Default: suppress
            
            mdapiLogger.error = function(err, msg) {
                // Check if this is a known user error that should be suppressed
                var isUserError = false;
                if (err && (err.name === 'BucketNotFound' || 
                           err.name === 'ObjectNotFound' ||
                           (err.message && (err.message.includes('not found') || 
                                          err.message.includes('already exists') ||
                                          err.message.includes('requested bucket not found') ||
                                          err.message.includes('requested object not found'))))) {
                    isUserError = true;
                }
                
                if (isUserError && suppressUserErrors) {
                    // Completely suppress user errors - no logging at all
                    // These are expected errors from user typos and shouldn't clutter logs
                    return;
                } else if (isUserError && !suppressUserErrors) {
                    // Log user errors at debug level when suppression is disabled
                    mdapiLogger.debug({
                        err: {
                            name: err.name,
                            message: err.message,
                    ...
                    }, 'buckets-mdapi user error: ' + (msg || ''));
                } else {
                    // Log real system errors normally
                    originalError(err, msg);
                }
            };
            ...
            buckets_mdapi_client.createClient({
                collector: opts.collector,
                pnodes: pnodes,
                bucketsMdapiOptions: self.cfg.buckets_mdapi.bucketsMdapiOptions,
                log: mdapiLogger
...
MetadataPlacement.prototype._getLocation = function _getLocation(tkey) {
    ...
    var ret = common.getDataLocation(this.placementData, tkey);
