...
function MetadataPlacement(opts) {
    ...
    this.buckets_mdplacement_client.once('connect', function _onConnect() {
        ...
        self.buckets_mdplacement_client.getPlacementData(function (pErr, pd) {
            ...
            // Create a filtered logger for buckets-mdapi to
            // reduce user error noise
            ...
            var suppressUserErrors = process.env.SUPPRESS_USER_ERROR_LOGS
            !== 'false'; // Default: suppress

            mdapiLogger.error = function (err, msg) {
                // Check if this is a known user error
                ...
                if (isUserError && suppressUserErrors) {
                    ...
                    // These are expected errors from user typos and
                    // shouldn't clutter logs
                    return;
                } else if (isUserError && !suppressUserErrors) {
                    // Log user errors at debug level when
