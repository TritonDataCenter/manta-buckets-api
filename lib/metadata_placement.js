...
function pollPlacementData() {
    ...
    this.log.trace('MetadataPlacement.pollPlacementData: entered');
...
function MetadataPlacement(opts) {
    ...
    this.electric_boray_client.once('error', function (err) {
        this.electric_boray_client.removeAllListeners('connect');

        this.log.error(err, 'electric-boray: failed to connect');
    });

    this.electric_boray_client.once('connect', function _onConnect() {
        this.electric_boray_client.removeAllListeners('error');

        this.log.info('electric-boray: connected');

        // Get the placement data
        this.placementData = this.electric_boray_client.getPlacementData();

        // Establish a boray connection to each shard
        this.log.info('creating boray clients');
        boray_client.createClient({
            pnodes: getPnodes(this.placementData),
            borayOptions: this.cfg.boray,
            log: this.log,
            ...
            this.log.info('finished boray client connection attempts');
            this.boray_clients = clients;
            this.emit('connect');
        });
    });
...
MetadataPlacement.prototype.getBorayClient =
    ...
    log.debug('MetadataPlacement.getBorayClient: entered');

    var log = this.log;
    var pnode = locationData.pnode;

    log.debug('MetadataPlacement.getBorayClient: done');
...
MetadataPlacement.prototype.getBucketLocation =
    ...
    log.debug({
        owner: owner,
        bucket: bucket
    }, 'MetadataPlacement.getBucketLocation: entered');

    var tkey = owner + ':' + bucket;

    log.debug({
        tkey: tkey
    }, 'MetadataPlacement.getBucketLocation: key transformed');
    ...
    log.debug({
        owner: owner,
        bucket: bucket
    }, 'MetadataPlacement.getBucketLocation: done');
...
MetadataPlacement.prototype.getObjectLocation =
    ...
    log.debug({
        ...
        bucket: bucket,
        key: key
    }, 'MetadataPlacement.getObjectLocation: entered');
    ...
    log.debug({
        key: key,
        tkey: tkey
    }, 'MetadataPlacement.getObjectLocation: key transformed');
    ...
    log.debug({
        ...
        bucket: bucket,
        key: key
    }, 'MetadataPlacement.getObjectLocation: done');
