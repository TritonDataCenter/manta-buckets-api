...
var S3_ERROR_RESPONSES = {
    ...
    'InvalidPart': {
        code: 'InvalidPart',
        description: 'One or more of the specified parts could not be found or the specified entity tag might not have matched the part\'s entity tag.',
        httpStatusCode: 400
    },
    'EntityTooSmall': {
        code: 'EntityTooSmall',
...
function convertErrorToS3(error, s3Request, req) {
    console.log('DEBUG: convertErrorToS3 called - req present?', !!req);
    ...
    if (req && req.log) {
        ...
        }, 'S3_DEBUG: convertErrorToS3 called with detailed MultiError analysis');
    ...
    // Handle MultiError by extracting the first cause that has proper error codes
    var actualError = error;
    if (error.name === 'MultiError') {
        // MultiError can store errors in different properties depending on the library version
        ...
        if (req && req.log) {
            req.log.debug({
                ...
                firstErrorRestCode: errorList[0] ? errorList[0].restCode : 'none'
        ...
        if (causeError) {
            ...
            if (causeError.name === 'MultiError') {
                ...
                return xmlResult;
            } else if (causeError.restCode || (causeError.name && causeError.name !== 'Error')) {
        ...
        if (actualError === error && errorList.length > 0) {
            ...
            for (var i = 0; i < errorList.length; i++) {
                ...
                if (subError.restCode || (subError.name && subError.name !== 'Error')) {
            ...
            if (actualError === error && errorList[0]) {
                ...
                if (req && req.log) {
                    req.log.debug('S3_DEBUG: Using first error from MultiError list as fallback');
                }
            }
        }
    }
    ...
    var errorMessage;
    if (error.name === 'MultiError' && actualError !== error && actualError.message) {
        errorMessage = actualError.message; // Use specific underlying error message
    } else {
        errorMessage = error.message || s3Error.description; // Use original message
    }
    ...
    // Preserve status code from underlying error (important for EntityTooSmall, etc.)
    if (actualError !== error && actualError.statusCode) {
        ...
        if (s3ErrorForStatus && s3ErrorForStatus.httpStatusCode && s3ErrorForStatus.httpStatusCode !== 500) {
        ...
        if (req.isS3Request) {
            ...
            ((typeof (actualCode)) === 'number' && actualCode >= 400)) {
                ...
                try {
                    ...
                    responseBody = convertErrorToS3(xmlError, req.s3Request, req);
                    res.setHeader('Content-Type', contentType);
