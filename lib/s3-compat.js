...
var S3_ERROR_RESPONSES = {
    ...
    'InvalidSignature': {
        ...
        description: 'The request signature we calculated ' +
    ...
    /*
     * The MPU-local errors used to be plain Error objects with a
     * restCode that did not match any entry below; they all surfaced
     * as <Code>InternalError</Code> in the response body even though
     * HTTP status was 4xx (the status was preserved separately via
     * preserveStatusCode). CHG-141 moved the constructors to
     * lib/errors.js, but the body Code still depended on having a
     * row in this table. These five entries close that gap.
     */
    'NoSuchUpload': {
        code: 'NoSuchUpload',
    ...
    'InvalidPartNumber': {
        ...
        description: 'Part number must be between 1 and 10000, ' +
            'inclusive.',
        httpStatusCode: 400
    },
    'InvalidPartOrder': {
    ...
    'EntityTooLarge': {
        code: 'EntityTooLarge',
        description: 'Your proposed upload exceeds' +
    ...
    'NoSuchEntity': {
        code: 'NoSuchEntity',
        description: 'The request was rejected because it referenced ' +
...
function parseS3Request(req) {
    req.log.debug('S3_DEBUG: ===== parseS3Request CALLED ==========');
    var path = req.path();
    ...
    if (pathParts.length === 0) {
        ...
        if (isS3Request) {
            ...
            switch (method) {
            case 'GET':
                // Check for CORS configuration query parameter
                if (query.cors !== undefined) {
                    s3Request.operation = 'GetBucketCors';
                    req.log.debug('S3_DEBUG_QUERY: parseS3Request -'+
                                  ' SET OPERATION TO GetBucketCors');
            ...
            case 'PUT':
                // Check for CORS configuration query parameter
                if (query.cors !== undefined) {
                    s3Request.operation = 'PutBucketCors';
                    req.log.debug('S3_DEBUG_QUERY: parseS3Request -'+
                                  ' SET OPERATION TO PutBucketCors');
                } else {
            ...
            case 'DELETE':
                // Check for CORS configuration query parameter
                if (query.cors !== undefined) {
                    s3Request.operation = 'DeleteBucketCors';
                    req.log.debug('S3_DEBUG_QUERY: parseS3Request -'+
                                  ' SET OPERATION TO DeleteBucketCors');
                } else {
        ...
        if (isS3Request) {
            ...
            if (query.uploads !== undefined) {
                if (s3Request.object) {
                    ...
                    s3Request.operation = 'ListMultipartUploads';
                    req.log.debug('S3_DEBUG:' +
                                  ' Detected ListMultipartUploads operation');
                }
                ...
                case 'DELETE':
                    ...
                    break;
                case 'POST':
                    // Support SigV4 POST uploads
                    // (same auth as PUT, different method)
                    req.log.debug('S3_DEBUG: Detected POST object upload');
                    s3Request.operation = 'CreateBucketObject';
                    break;
                default:
                    // Log unexpected method but don't fail parsing
 ...
 *      .html
 */
function convertMantaToS3Headers(mantaHeaders, _operation) {
    var s3Headers = {};
...
function convertObjectListToS3(mantaResponse, bucketName, paginationInfo) {
    ...
    objects.forEach(function (obj) {
        if (obj && obj.name) {
            ...
            + '</LastModified>\n';
            var s3Etag = common.md5Base64ToHex(
                obj.contentMD5);
            xml += '    <ETag>"' + (s3Etag ||
                obj.etag ||
...
function convertObjectListToS3V2(mantaResponse, bucketName, paginationInfo) {
    ...
    validObjects.forEach(function (obj) {
        ...
        var s3EtagV2 = common.md5Base64ToHex(
            obj.contentMD5);
        xml += '    <ETag>"' + (s3EtagV2 ||
            obj.etag ||
            'd41d8cd98f00b204e9800998ecf8427e') +
...
function convertErrorToS3(error, s3Request, req) {
    logErrorDetails(error, s3Request, req);
    ...
    // Unwrap MultiError to get the actual underlying error
    var actualError = unwrapMultiError(error, s3Request, req);

    // Build and return S3 XML error response
    return (buildS3ErrorXml(error, actualError, s3Request, req));
}

/**
 * Log error details for debugging
 */
function logErrorDetails(error, s3Request, req) {
    if (!req || !req.log) {
        return;
    }
    ...
    }, 'S3: convertErrorToS3 called with detailed MultiError analysis');
    ...
    if (error.name === 'MultiError') {
        ...
        }, 'S3: MultiError deep structure analysis');
    }
}

/**
 * Unwrap MultiError to extract the actual underlying error
 */
function unwrapMultiError(error, s3Request, req) {
    // If not a MultiError, return as-is
    if (error.name !== 'MultiError') {
        return (error);
    }
    ...
    // Try to extract error from cause
    var extractedError = extractErrorFromCause(error, causeError,
        s3Request, req);
    if (extractedError) {
        return (extractedError);
    }

    // Try to extract error from error list
    extractedError = extractErrorFromList(error, errorList, req);
    if (extractedError) {
        return (extractedError);
    }
...
/**
 * Extract actual error from MultiError cause
 */
function extractErrorFromCause(error, causeError, s3Request, req) {
    if (!causeError) {
        return (null);
    }
    ...
    if (causeError.name === 'MultiError') {
        ...
        // This returns the unwrapped error, not XML
        return (unwrapMultiError(causeError, s3Request, req));
    }

    // Use cause if it has a restCode or proper name
    if (causeError.restCode ||
        (causeError.name && causeError.name !== 'Error')) {
        if (req && req.log) {
            req.log.debug({
                selectedErrorName: causeError.name,
                selectedErrorRestCode: causeError.restCode,
                selectedErrorMessage: causeError.message
...
/**
 * Extract actual error from MultiError error list
 */
function extractErrorFromList(originalError, errorList, req) {
    ...
    for (var i = 0; i < errorList.length; i++) {
        ...
        if (subError.restCode ||
            ...
            if (req && req.log) {
                req.log.debug({
                    selectedErrorName: subError.name,
                    selectedErrorRestCode: subError.restCode,
                    selectedErrorMessage: subError.message
                }, 'S3_DEBUG: Selected specific error from MultiError list');
            }
            return (subError);
        }
    }
 ...
 */
function buildS3ErrorXml(originalError, actualError, s3Request, req) {
    var errorCode = actualError.restCode || actualError.name ||
        'InternalError';
    /*
     * The S3_ERROR_RESPONSES table is mostly keyed by restCode
     * but a few entries (BucketNotFoundError, ObjectNotFoundError,
     * NotEnoughSpaceError, ...) are keyed by the JS class name —
     * the restCode form ('BucketNotFound') would otherwise miss
     * the table and incorrectly fall through to InternalError,
     * surfacing real 404s as 500s. Try the suffix form before
     * giving up so both keying styles work without forcing a
     * rename in errors.js.
     */
    var s3Error = S3_ERROR_RESPONSES[errorCode] ||
        S3_ERROR_RESPONSES[errorCode + 'Error'] ||
    ...
    var errorMessage;
    if (originalError.name === 'MultiError' &&
        actualError !== originalError && actualError.message) {
        errorMessage = actualError.message;
    } else {
        errorMessage = originalError.message || s3Error.description;
    }
    ...
    preserveStatusCode(originalError, actualError, errorCode, req);
 ...
 * Preserve status code from underlying error
 */
function preserveStatusCode(error, actualError, errorCode, req) {
    ...
    if (actualError === error && error.name === 'MultiError') {
...
/**
 * @brief Generate AWS-compatible request ID
 * @details Creates a random hexadecimal string for use as request
 *          identifier in AWS API responses. Uses cryptographically
 *          secure random bytes converted to uppercase hex.
 * @param {number} [byteLength=8] Number of random bytes to generate
 * @return {string} Uppercase hexadecimal request ID string
 * @example
 *   generateRequestId()     // Returns 8-byte hex: "A1B2C3D4E5F6789A"
 *   generateRequestId(16)   // Returns 16-byte hex for STS/IAM
 */
function generateRequestId(byteLength) {
...
/**
 * @brief Create standardized authentication error object
 * @details Generates consistent authentication error for AWS API
 *          compatibility across STS, IAM, and S3 operations.
 *          Returns error object with AWS-compatible structure.
 * @param {string} [service='API'] Service name for error message
 * @return {Object} Error object with name, message, statusCode
 * @example
 *   createAuthenticationError('STS')
 *   // Returns: { name: 'InvalidUserID.NotFound',
 *   //           message: 'Authentication required for STS operations',
 *   //           statusCode: 401 }
 */
function createAuthenticationError(service) {
    service = service || 'API';
    return {
        name: 'InvalidUserID.NotFound',
        message: 'Authentication required for ' + service + ' operations',
        statusCode: 401
    };
}

/**
 * @brief Create MD5 hash of input data
 * @details Computes MD5 hash using Node.js crypto module and returns
 *          lowercase hexadecimal digest. Commonly used for ETag
 *          generation and data integrity verification in S3 operations.
 * @param {string|Buffer} data Input data to hash
 * @return {string} Lowercase hexadecimal MD5 hash
 * @example
 *   createMD5Hash('hello world')
 *   // Returns: "5d41402abc4b2a76b9719d911017c592"
 *   createMD5Hash(buffer)         // Hash binary data
 */
...
function s3ResponseFormatter(req, res, next) {
    ...
    if (req.s3Request && (req.s3Request.operation === 'GetBucketObject' ||
        ...
        res.writeHead = function writeHeadWithS3Headers(statusCode, headers) {
            // Add S3 headers
            headers = headers || {};
    ...
    res.writeHead =
        function writeHeadWithHeaderConversion(statusCode, headers) {
    ...
    res.set = function setWithMetadataConversion(key, value) {
    ...
    res.write = function writeWithDataCollection(chunk) {
        if (req._skipS3ResponseProcessing) {
    ...
    res.end = function endWithXMLConversion(endChunk) {
        if (req._skipS3ResponseProcessing) {
    ...
    res.send = function sendWithS3Formatting(code, data) {
        if (req._skipS3ResponseProcessing) {
...
function s3RoleTranslator(req, res, next) {
    ...
    grantHeaders.forEach(function (header) {
        ...
        if (grantValue) {
            ...
            var roles = parseS3GrantToRoles(header, grantValue, req.log);
...
/*
 * parseS3GrantToRoles(header, value, log)
 ...
 *    time.
 */
function parseS3GrantToRoles(header, value, log) {
    ...
    switch (header) {
        ...
        default:
            log.warn({
               header: header,
               value: value
               }, 'S3_GRANT_WARNING: Unrecognized grant header');
        break;
    }
    return (roles);
}
...
///--- S3 Object Tagging XML Utilities

/**
 * Parse S3 tagging XML payload
 */
function parseTaggingXML(body) {
    var tags = {};
    ...
    return (parseXMLTaggingFormat(body.trim()));
}

/**
 * Parse XML tagging format
 */
function parseXMLTaggingFormat(xmlBody) {
    var tags = {};

    // Extract tag elements using regex
    // JSSTYLED
    var tagMatches = xmlBody.match(/<Tag>[\s\S]*?<\/Tag>/g);

    if (tagMatches) {
        tagMatches.forEach(function (tagXml) {
    // JSSTYLED
            var keyMatch = tagXml.match(/<Key>(.*?)<\/Key>/);
    // JSSTYLED
            var valueMatch = tagXml.match(/<Value>(.*?)<\/Value>/);

            if (keyMatch && valueMatch) {
                var key = decodeXMLEntities(keyMatch[1]);
                var value = decodeXMLEntities(valueMatch[1]);

                // Validate tag constraints
                if (key.length > 128) {
                    throw new Error('Tag key too long (max 128 characters)');
                }
                if (value.length > 256) {
                    throw new Error('Tag value too long (max 256 characters)');
                }
    ...
    if (Object.keys(tags).length > 10) {
        throw new Error('Too many tags (max 10 per object)');
    }
 ...
 */
function generateTaggingXML(tags) {
    var xml = '<?xml version="1.0" encoding="UTF-8"?>\n';
    ...
    Object.keys(tags).forEach(function (key) {
        ...
        xml += '      <Value>' + escapeXml(tags[key]) + '</Value>\n';
        xml += '    </Tag>\n';
    });
...
module.exports = {
    ...
    parseTaggingXML: parseTaggingXML,
    generateTaggingXML: generateTaggingXML,
