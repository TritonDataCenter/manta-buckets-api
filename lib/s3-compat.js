...
/**
 * Parse S3-style path and determine operation
 */
function parseS3Request(req) {
    ...
    // Detect S3-style requests (no account prefix)
...
function s3ResponseFormatter(req, res, next) {
    ...
    res.send = function(code, data) {
        if (req.isS3Request) {
            ...
            }, 's3ResponseFormatter: converting response to S3 format');
            ...
            return originalSend.call(this, code, responseBody);
        }
