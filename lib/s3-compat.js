...
function parseS3Request(req) {
    ...
    if (isVirtualHosted) {
        ...
        if (isSigV4) {
            ...
            // AWS CLI sometimes sends /bucket/object to bucket.domain.com
            if (pathParts.length >= 2 && pathParts[0] === bucketFromHost) {
                // This is path-style content: /test5/test1 on test5.localhost:8080
                // Treat it as path-style: bucket from path, object from path
