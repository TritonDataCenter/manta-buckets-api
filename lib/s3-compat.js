...
function parseS3Request(req) {
    ...
    console.log('S3_DEBUG_CONSOLE: parseS3Request inputs - path:',
    path, 'method:', method, 'host:', host);
    console.log('S3_DEBUG_QUERY_MAIN: req.url:', req.url);
    console.log('S3_DEBUG_QUERY_MAIN: queryString extracted:', queryString);
    console.log('S3_DEBUG_QUERY_MAIN: parsed query object:',
    ...
    console.log('S3_DEBUG_CONSOLE: parseS3Request called with path:',
    path, 'host:', host);
    ...
    if (isVirtualHosted) {
        ...
        if (isSigV4) {
            ...
            if (pathParts.length >= 1 && pathParts[0] === bucketFromHost) {
                ...
                console.log('S3_DEBUG_CONSOLE: Detected path-style content '+
                'on virtual-hosted domain, pathParts:', pathParts);
                req.log.info('S3_DEBUG: Detected path-style content on'+
                ...
                if (pathParts.length === 1) {
                    ...
                    // (for GET)
                    console.log('S3_DEBUG_CONSOLE: Single bucket path on'+
                    ...
                    switch (method) {
                        case 'GET':
                            ...
                            if (query['list-type'] === '2') {
                                ...
                                console.log('S3_DEBUG_CONSOLE: Set operation '+
                        ...
                        default:
                            ...
                            console.log('S3_DEBUG_CONSOLE: Unexpected HTTP '+
                ...
                if (pathParts.length === 0) {
                    ...
                    console.log('S3_DEBUG_CONSOLE: Virtual-hosted root path'+
            ...
            if (isSigV4) {
                ...
                switch (method) {
                    case 'GET':
                        ...
                        console.log('S3_DEBUG_QUERY: parseS3Request'+
                        ...
                        console.log('S3_DEBUG_FINAL_OPERATION: parseS3Request'+
                    ...
                    default:
                        // Log unexpected method but don't fail parsing
                        console.log('S3_DEBUG_CONSOLE: Unexpected HTTP method '+
    ...
    console.log('S3_DEBUG_CONSOLE: parseS3Request final result:',
    JSON.stringify(s3Request, null, 2));
...
function convertMantaToS3Response(mantaBody, operation, s3Request, caller) {
    console.log('S3_DEBUG: convertMantaToS3Response - operation:', operation,
    'mantaBody type:', (typeof (mantaBody)),
    'isArray:', Array.isArray(mantaBody));
    console.log('S3_DEBUG: convertMantaToS3Response - s3Request:',
    JSON.stringify(s3Request, null, 2));
    console.log('S3_DEBUG: convertMantaToS3Response - mantaBody preview:',
    JSON.stringify(mantaBody, null, 2));

    switch (operation) {
        case 'ListBuckets':
            console.log('S3_DEBUG: convertMantaToS3Response '+
            '- calling convertBucketListToS3');
            return (convertBucketListToS3(mantaBody, caller));

        case 'ListBucketObjects':
            console.log('S3_DEBUG_CONVERSION: ❌ U'+
            'SING convertObjectListToS3 (V1) - NO <KeyCount>');
            console.log('S3_DEBUG: convertMantaToS3Response - '+
            'calling convertObjectListToS3 with bucket:', s3Request.bucket);
            return (convertObjectListToS3(mantaBody, s3Request.bucket));

        case 'ListBucketObjectsV2':
            console.log('S3_DEBUG_CONVERSION: ✅ USING c'+
            'onvertObjectListToS3V2 - THIS SHOULD GENERATE <KeyCount>');
            console.log('S3_DEBUG: convertMantaToS3Response - '+
            'calling convertObjectListToS3V2 with bucket:', s3Request.bucket);
            var v2Result = convertObjectListToS3V2(mantaBody, s3Request.bucket);
            console.log('S3_DEBUG_CONVERSION: convertObjectListToS3V2'+
            ' result preview:', v2Result.substring(0, 300));
            return (v2Result);
        ...
        case 'DeleteBucketObject':
            ...
            console.log('S3_DEBUG: convertMantaToS3Response - '+
            'returning empty string for operation:', operation);
            return ('');

        case 'GetBucketObject':
            // Object content passes through unchanged
            console.log('S3_DEBUG: convertMantaToS3Response - '+
            'returning mantaBody unchanged for GetBucketObject');
            return (mantaBody);

        default:
            console.log('S3_DEBUG: convertMantaToS3Response - '+
            'unknown operation, returning mantaBody unchanged:', operation);
            return (mantaBody);
    }
}
...
function convertBucketListToS3(mantaResponse, caller) {
    ...
    console.log('S3_DEBUG: convertBucketListToS3 - input data:',
    JSON.stringify(mantaResponse, null, 2));
    console.log('S3_DEBUG: convertBucketListToS3 - buckets array:',
    JSON.stringify(buckets, null, 2));
    console.log('S3_DEBUG: convertBucketListToS3 - caller data:',
    ...
    if (caller && caller.account) {
        ...
        'Manta User';
        console.log('S3_DEBUG: convertBucketListToS3 - '+
        'extracted owner info: ID=' + ownerID +
        ', DisplayName=' + ownerDisplayName);
    } else {
        console.log('S3_DEBUG: convertBucketListToS3 - '+
        'no caller/account info, using defaults');
    }
    ...
    buckets.forEach(function (bucket) {
        console.log('S3_DEBUG: processing bucket:',
        ...
        if (bucket && bucket.name) {
            ...
            console.log('S3_DEBUG: skipping bucket (no name property):',
            JSON.stringify(bucket, null, 2));
        }
    });
    ...
    console.log('S3_DEBUG: convertBucketListToS3 - '+
    'generated XML with IsTruncated field:');
    console.log('===== FULL XML START =====');
    console.log(xml);
    console.log('===== FULL XML END =====');
    console.log('S3_DEBUG: convertBucketListToS3 - processed',
    bucketCount, 'buckets out of', buckets.length, 'total');
...
function convertObjectListToS3(mantaResponse, bucketName) {
    ...
    console.log('S3_DEBUG: convertObjectListToS3 - input data:',
    JSON.stringify(mantaResponse, null, 2));
    console.log('S3_DEBUG: convertObjectListToS3 - bucketName:',
    JSON.stringify(bucketName));

    // Handle edge cases
    if (!bucketName) {
        console.log('S3_DEBUG: convertObjectListToS3'+
    ...
    console.log('S3_DEBUG: convertObjectListToS3 - objects array length:',
    objects.length);
    ...
    objects.forEach(function (obj) {
        console.log('S3_DEBUG: processing object:',
        ...
        if (obj && obj.name) {
            ...
            console.log('S3_DEBUG: skipping object (no name property):',
            JSON.stringify(obj, null, 2));
        }
    });
    ...
    console.log('S3_DEBUG: convertObjectListToS3 - generated XML length:',
    xml.length);
    console.log('S3_DEBUG: convertObjectListToS3 - processed', objectCount,
    'objects out of', objects.length, 'total');
    console.log('S3_DEBUG: convertObjectListToS3 - generated XML preview:',
    xml.substring(0, 500) + (xml.length > 500 ? '...[truncated]' : ''));
...
function convertObjectListToS3V2(mantaResponse, bucketName) {
    var objects = Array.isArray(mantaResponse) ?
    mantaResponse : [mantaResponse];
    console.log('S3_DEBUG: convertObjectListToS3V2 - objects array length:',
    objects.length);
    ...
    var validObjects = [];
    objects.forEach(function (obj) {
        console.log('S3_DEBUG: processing object for V2:',
        JSON.stringify(obj, null, 2));
        if (obj && obj.name) {
            objectCount++;
            validObjects.push(obj);
            console.log('S3_DEBUG: added valid object to V2 list:', obj.name);
        } else {
            console.log('S3_DEBUG: skipping object in V2 (no name property):',
            JSON.stringify(obj, null, 2));
        }
    });
    ...
    console.log('S3_DEBUG_XML_V2: ✅ ADDED <KeyCount>' +
    objectCount + '</KeyCount> TO XML');
    console.log('S3_DEBUG_XML_V2: Current XML contains KeyCount:',
    xml.indexOf('<KeyCount>') !== -1);
    ...
    console.log('S3_DEBUG: convertObjectListToS3V2 - generated XML length:',
    xml.length);
    console.log('S3_DEBUG: convertObjectListToS3V2 - processed',
    objectCount, 'objects out of', objects.length, 'total');
    console.log('S3_DEBUG: convertObjectListToS3V2 - generated XML preview:',
    xml.substring(0, 500) + (xml.length > 500 ? '...[truncated]' : ''));
...
function ensureWellFormedXML(xmlString) {
    if (!xmlString || (typeof (xmlString)) !== 'string') {
        return ('<?xml version="1.0" encoding="UTF-8"?>\n<Error><Code>InternalError</Code><Message>Invalid XML</Message></Error>');
    }
...
function s3ResponseFormatter(req, res, next) {
    ...
    res.end = function (endChunk) {
        if (req.isS3Request && req.s3Request.operation !== 'GetBucketObject') {
            ...
            // Process streaming data OR handle empty bucket case for S3 requests
            if (responseData.length > 0 || req.isS3Request) {
                ...
                if (responseData.length > 0) {
                    combinedData = Buffer.concat(responseData.map(function (chunk) {
                    ...
                    req.log.info('S3_DEBUG: s3ResponseFormatter - '+
                ...
                try {
                    ...
                    // Ensure XML is well-formed for strict parsers like CyberDuck
                    // while maintaining compatibility with lenient parsers like AWS CLI
