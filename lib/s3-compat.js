...
var S3_ACL_TO_MANTA_ROLES = {
    'private': [],
    'public-read': ['public-reader'],
    'public-read-write': ['public-reader', 'public-writer'],
...
function s3RoleTranslator(req, res, next) {
    ...
    var grantHeaders = ['x-amz-grant-read', 'x-amz-grant-write', 'x-amz-grant-full-control'];
    var grantRoles = [];
    ...
    if (grantRoles.length > 0) {
        var existingRoles = req.headers['role-tag'] ? req.headers['role-tag'].split(',') : [];
...
function parseS3GrantToRoles(header, value) {
    ...
    switch (header) {
        case 'x-amz-grant-read':
            if (value.includes('uri="http://acs.amazonaws.com/groups/global/AllUsers"')) {
                roles.push('public-reader');
            } else if (value.includes('uri="http://acs.amazonaws.com/groups/global/AuthenticatedUsers"')) {
...
function rolesToS3ACL(roles) {
    ...
    var hasPublicReader = roles.includes('public-reader');
