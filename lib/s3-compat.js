...
function convertMantaToS3Response(mantaBody, operation, s3Request) {
    ...
    switch (operation) {
        ...
        case 'DeleteBucketObject':
            ...
            console.log('S3_DEBUG: convertMantaToS3Response - returning mantaBody unchanged for operation:', operation);
            return mantaBody;
...
function s3ResponseFormatter(req, res, next) {
    ...
    res.write = function(chunk) {
        if (req.isS3Request) {
            // Collect streaming data for later conversion
            if (chunk) {
        ...
        if (req.isS3Request) {
            ...
            if (responseBody) {
                ...
                req.log.info({
                    ...
                    xmlResponse: responseBody.substring(0, 1000) + (responseBody.length > 1000 ? '...[truncated]' : '')
                ...
                originalWrite.call(this, responseBody, 'utf8');
                originalEnd.call(this);
                return;
            }
        }
