    ...
    if (isVirtualHosted) {
        ...
        if (pathParts.length === 0) {
            ...
            if (isSigV4) {
                ...
                switch (method) {
                    case 'GET':
                        // Check for ListObjectsV2 query parameter (using manually parsed query)
...
function convertBucketListToS3(mantaResponse, caller) {
    ...
    if (caller && caller.account) {
        ownerID = caller.account.uuid || caller.account.login || 'manta-user';
        ownerDisplayName = caller.account.login || caller.account.cn || 'Manta User';
    ...
    xml += '    <DisplayName>' + escapeXml(ownerDisplayName) + '</DisplayName>\n';
    xml += '  </Owner>\n';
    xml += '  <Buckets>\n';
...
function convertObjectListToS3V2(mantaResponse, bucketName) {
    var objects = Array.isArray(mantaResponse) ? mantaResponse : [mantaResponse];
    console.log('S3_DEBUG: convertObjectListToS3V2 - objects array length:', objects.length);

    // Count valid objects first
    var objectCount = 0;
    var validObjects = [];
    objects.forEach(function (obj) {
        console.log('S3_DEBUG: processing object for V2:', JSON.stringify(obj, null, 2));
        if (obj && obj.name) {
            objectCount++;
            validObjects.push(obj);
            console.log('S3_DEBUG: added valid object to V2 list:', obj.name);
        } else {
            console.log('S3_DEBUG: skipping object in V2 (no name property):', JSON.stringify(obj, null, 2));
        }
    });
    ...
    console.log('S3_DEBUG_XML_V2: ✅ ADDED <KeyCount>' + objectCount + '</KeyCount> TO XML');
    console.log('S3_DEBUG_XML_V2: Current XML contains KeyCount:', xml.indexOf('<KeyCount>') !== -1);
    ...
    console.log('S3_DEBUG: convertObjectListToS3V2 - generated XML length:', xml.length);
    console.log('S3_DEBUG: convertObjectListToS3V2 - processed', objectCount, 'objects out of', objects.length, 'total');
    console.log('S3_DEBUG: convertObjectListToS3V2 - generated XML preview:', xml.substring(0, 500) + (xml.length > 500 ? '...[truncated]' : ''));
        ...
        if (req.isS3Request && req.s3Request.operation !== 'GetBucketObject') {
            ...
            if (responseData.length > 0) {
                ...
                try {
                    ...
                    xmlResponse = convertMantaToS3Response(jsonData, req.s3Request.operation, req.s3Request, req.caller);
    ...
    res.send = function (code, data) {
        if (req.isS3Request) {
            ...
            if (actualData instanceof Error || ((typeof actualCode) === 'number' && actualCode >= 400)) {
                ...
                try {
                    responseBody = convertMantaToS3Response(actualData, req.s3Request.operation, req.s3Request, req.caller);
                    req.log.info({
            ...
            if (responseBody) {
                ...
                }, 'S3_DEBUG: s3ResponseFormatter - sending response for s3tests compatibility');
