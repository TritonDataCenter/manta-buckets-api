...
function convertObjectListToS3V2(mantaResponse, bucketName) {
    ...
    objects.forEach(function (obj) {
        ...
        if (obj && obj.name) {
            ...
            xml += '    <StorageClass>STANDARD</StorageClass>\n';
            xml += '  </Contents>\n';
            console.log('S3_DEBUG: added object to V2 XML:', obj.name);
        } else {
            console.log('S3_DEBUG: skipping object in V2 (no name property):', JSON.stringify(obj, null, 2));
        }
    });
    
    // Update KeyCount with actual count
    xml = xml.replace('<KeyCount>0</KeyCount>', '<KeyCount>' + objectCount + '</KeyCount>');
