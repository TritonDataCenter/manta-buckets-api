 ...
 * Assembling all the parts using mako, simplifies the code and the responsibil-
...
function proceedWithV2Commit(req, uploadRecord, partPaths,
    ...
    var discoveredTotalSize = partPaths.reduce(
        function accumulatePartSize(sum, partPath) {
    ...
    req.log.debug({
        ...
        partCount: partPaths.length,
        allPartSizes: partPaths.map(function extractPartDebugInfo(p, idx) {
            return ({
                index: idx,
    ...
    var v2CommitBody = {
        ...
        uploadId: uploadRecord.uploadId,
        parts: partPaths.map(function formatPartForCommit(partPath, index) {
    ...
    tryV2CommitOnSharks(req, targetSharks, v2CommitBody, owner,
            ...
            s3Multipart.createFinalObjectMetadata(req,
                uploadRecord, commitBody, assemblyData, targetSharks,
                function onFinalMetadataCreated(metadataErr, _metadataResult) {
                if (metadataErr) {
                    req.log.error({
                        error: metadataErr,
                        uploadId: uploadRecord.uploadId,
...
function tryV2CommitOnSharks(req, sharks, commitBody, owner, callback) {
    req.log.debug({
        ...
        sharks: sharks.map(function extractSharkId(s) {
    ...
    vasync.forEachParallel({
        ...
        if (vasyncErr) {
            ...
            if (vasyncErr.ase_errors && vasyncErr.ase_errors.length > 0) {
                var invalidPartErrors = vasyncErr.ase_errors.filter(
                    function isInvalidPartError(err) {
        ...
        var failures = operations.filter(function isFailedOperation(op) {
            return (op.err);
        });

        // Extract successful shark IDs from operations that succeeded
        var successfulSharks = operations.filter(
            function isSuccessfulOperation(op) {
            return (!op.err && op.result);
        }).map(function extractSuccessfulSharkId(op) {
        ...
        var firstSuccess = operations.find(function findFirstSuccess(op) {
            return (!op.err && op.result);
        });
...
function getTargetSharks(req, uploadRecord, partPaths) {
    ...
    if (uploadRecord.preAllocatedSharks &&
        ...
        req.log.debug({
            ...
            sharks: uploadRecord.preAllocatedSharks.map(
                function extractPreAllocatedSharkId(s) {
                return (s.manta_storage_id);
            }),
