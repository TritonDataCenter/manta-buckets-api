    ...
    validateSharkSpaceForV2Commit(req, partPaths, commitBody.nbytes,
        ...
        if (spaceErr) {
            req.log.error({
                ...
                uploadId: uploadRecord.uploadId
            }, 'S3_MPU: Insufficient space for v2 commit - failing request');
...
//    if (!targetSharks) {
 //       return (callback(new Error('No sharks available for MPU')));
  //  }
    ...
    // Execute v2 commit (directories auto-created by nginx)
    req.log.info({
        uploadId: uploadRecord.uploadId,
        objectId: commitBody.objectId
    }, 'S3_MPU: Attempting v2 commit (directories auto-created by nginx)');
   ...
   // assert.object(uploadRecord.preAllocatedSharks, 'preAllocatedSharks');
    ...
    req.log.debug({
        ...
        uploadRecordKeys: uploadRecord ? Object.keys(uploadRecord) :
            'null-or-undefined',
        hasPreAllocatedSharks: !!(uploadRecord && uploadRecord.preAllocatedSharks),
        preAllocatedSharksLength: uploadRecord && uploadRecord.preAllocatedSharks ? 
...
function validateSharkSpaceForV2Commit(req, partPaths, finalSizeBytes,
    ...
    // First, try to get upload record from the call stack to access pre-allocated sharks
    var uploadRecord = null;
    var uploadId = req.s3Request ? req.s3Request.uploadId : 'unknown';
    
    // Since we don't have uploadRecord as parameter, we'll need to work with what we have
    // Get shark information from first part (all parts use same sharks)
    var targetSharks = null;
    ...
    targetSharks = samplePart.sharks;
    ...
    var totalSharks = targetSharks.length;

    targetSharks.forEach(function (shark) {
    ...
    req.log.info({
        ...
        sharkSpaces: targetSharks.map(function (s) {
            return {
                shark: s.manta_storage_id,
                availableMB: currentSharkSpaces[s.manta_storage_id] || 'unknown'
            };
        })
    }, 'S3_MPU_V2: All pre-allocated' +
       ' sharks have sufficient space for v2 commit');
