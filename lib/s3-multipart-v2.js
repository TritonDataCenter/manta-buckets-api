...
function tryMakoV2Commit(req, uploadRecord, partPaths, commitBody, callback) {
    ...
    validateSharkSpaceForV2Commit(req, partPaths, commitBody.nbytes,
                                  function (spaceErr) {
        if (spaceErr) {
            req.log.warn({
                ...
                uploadId: uploadRecord.uploadId
            }, 'S3_MPU: Insufficient space for v2 commit' +
...
function proceedWithV2Commit(req, uploadRecord, partPaths,
    ...
    // STEP 1: Create metadata first to establish directory structure
    req.log.info({
        uploadId: uploadRecord.uploadId,
        objectId: commitBody.objectId
    }, 'S3_MPU: Creating metadata' +
       ' first to establish directory structure for v2 commit');

    // Create placeholder assembly data for metadata creation
    var placeholderAssemblyData = {
        totalBytes: commitBody.nbytes,
        md5: '', // Will be updated after v2 commit
        sharks: targetSharks
    };

    s3MultipartModule.createFinalObjectMetadata(req,
        uploadRecord, commitBody, placeholderAssemblyData, targetSharks,
        function (metadataErr, metadataResult) {
        if (metadataErr) {
            req.log.warn({
                error: metadataErr.message,
                uploadId: uploadRecord.uploadId
            }, 'S3_MPU: Failed to create metadata, falling back to streaming');
        ...
        req.log.info({
            uploadId: uploadRecord.uploadId,
            objectId: commitBody.objectId
        }, 'S3_MPU: Metadata created successfully, now attempting v2 commit');

        // STEP 2: Now try v2 commit (directory structure exists)
        tryV2CommitOnSharks(req, targetSharks, v2CommitBody, owner,
            ...
            if (v2Err) {
                req.log.warn({
                    error: v2Err.message,
                    uploadId: uploadRecord.uploadId
                }, 'S3_MPU: Native v2 commit failed' +
                   ', falling back to streaming');
            ...
            }, 'S3_MPU: v2 commit physical assembly completed successfully');

            // STEP 3: Update metadata with correct MD5 from v2 commit result
            /*JSSTYLED*/
            ...
            }, 'S3_MPU: Processing ETag for metadata update');
            ...
            req.log.info({
                ...
                objectId: commitBody.objectId,
                correctMD5: cleanMd5
            }, 'S3_MPU: Updating metadata with correct MD5 from v2 commit');

            updateObjectMetadataWithMD5(req, uploadRecord, commitBody, cleanMd5,
                function (updateErr) {
                if (updateErr) {
                    req.log.warn({
                        ...
                        uploadId: uploadRecord.uploadId,
                        objectId: commitBody.objectId
                    }, 'S3_MPU: Failed to update metadata with correct MD5, ' +
                       'but v2 commit succeeded');
                    // Don't fail the entire operation for metadata update
                    // failure
                    // The object exists physically with correct data
                }
                ...
                req.log.info({
                   ...
                   ' completed v2 commit with metadata update');
...
/**
 * Update object metadata with correct MD5 after successful v2 commit
 */
function updateObjectMetadataWithMD5(req, uploadRecord, commitBody,
                                     correctMD5, callback) {
    var owner = req.owner.account.uuid;
    var bucketId = uploadRecord.bucketId;
    var objectName = getObjectName(req, uploadRecord);
    var objectNameHash = calculateObjectNameHash(objectName);
    ...
    }, 'S3_MPU: Updating object metadata with correct MD5');

    // Get metadata placement for the object
    ...
    // Update headers with correct MD5
    var updatedHeaders = {
        'content-md5': correctMD5
    };

    // Call updateObject to fix the MD5 in metadata
    client.updateObject(
        owner,
        bucketId,
        objectName,
        commitBody.objectId,
        'application/octet-stream', // Keep original content type
        updatedHeaders,
        {}, // props
        ...
        function (updateErr, updateResult) {
            if (updateErr) {
                req.log.error({
                    ...
                    objectName: objectName,
                    correctMD5: correctMD5
                }, 'S3_MPU: Failed to update object metadata with correct MD5');
            ...
            req.log.debug({
                objectId: commitBody.objectId,
                objectName: objectName,
                updatedMD5: correctMD5,
                updateResult: updateResult
            }, 'S3_MPU: Successfully updated object metadata with correct MD5');
 ...
 */
function validateSharkSpaceForV2Commit(req, partPaths, finalSizeBytes,
                                       callback) {
    ...
    if (!samplePart.sharks || !Array.isArray(samplePart.sharks)) {
        ...
        }, 'S3_MPU_V2: No shark information available,' +
           ' proceeding without space validation');
        return (callback()); // Proceed without validation if no shark info
    }

    // Check available space using storinfo
    var isOperator = req.caller && req.caller.account &&
        req.caller.account.isOperator;
    ...
    if (!sharkMap) {
        req.log.warn({
            uploadId: req.s3Request ? req.s3Request.uploadId : 'unknown',
            finalSizeMB: finalSizeMB
        }, 'S3_MPU_V2: No shark map available' +
           ', proceeding without space validation');
    ...
    if (insufficientSharks.length > 0) {
        ...
        }, 'S3_MPU_V2: Pre-allocated' +
           ' sharks have insufficient space for v2 commit');

        // Create NotEnoughSpaceError for consistency with storinfo.choose()
        try {
            var storinfoErrors = require('storinfo/lib/errors');
            var StorinfoNotEnoughSpaceError =
                storinfoErrors.NotEnoughSpaceError;
            ...
            req.log.warn(requireErr, 'S3_MPU_V2: Could not require' +
                         ' storinfo errors, using generic error');
            var genericError = new Error('Insufficient space for v2 commit: ' +
                                         cause);
    ...
    }, 'S3_MPU_V2: All pre-allocated sharks' +
       ' have sufficient space for v2 commit');
