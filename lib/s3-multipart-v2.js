...
function tryMakoV2Commit(req, uploadRecord, partPaths, commitBody, callback) {
    ...
    // FIXME: we just removed custom assembly using mdapi
    // we don't need to check any config, /mpu/v2/commit is the only way
    // we have to assembly mpu uploads.
...
/**
 * Execute v2 commit using nginx mpu module (no fallback)
 */
function proceedWithV2Commit(req, uploadRecord, partPaths,
    ...
    // Recalculate total size from actual part sizes (important for resume scenarios)
    ...
    // Only use discovered size if there's a difference (safer for normal uploads)
    var actualSize = significantDifference ? discoveredTotalSize : commitBody.nbytes;
    ...
    req.log.debug({
        ...
        allPartSizes: partPaths.map(function (p, idx) {
            return ({ 
                ...
                sharkPath: p.sharkPath ? p.sharkPath.substring(p.sharkPath.lastIndexOf('/')) : 'none'
            });
        })
        ...
        func: function (shark, next) {
            ...
            client.post(opts, commitBody, function (postErr, postReq, res) {
                if (postErr || !res || res.statusCode !== 204) {
                    ...
                    // failures, but pass the error to vasync for proper counting
