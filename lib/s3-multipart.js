...
function s3InitiateMultipartUploadHandler() {
    return function s3InitiateMultipartUpload(req, res, next) {
        ...
        uploadRecord.preAllocatedSharks = null; // Will be populated during
                                                // first part upload
    ...
    return function s3UploadPart(req, res, next) {
        ...
        getUploadRecord(req, uploadId, function (getErr, uploadRecord) {
            ...
            getUploadRecord(req, uploadId,
                ...
                // Get durability level from stored durability object (preferred)
                var durabilityKey = '.mpu-uploads/' + uploadId + '.durability';
                getDurabilityObject(req, durabilityKey, function (durErr, durabilityData) {
                    var durabilityLevel;
                    
                    if (!durErr && durabilityData && durabilityData.durabilityLevel !== undefined) {
                        ...
                        }, 'S3_MPU: Using durability level from durability object for part upload');
                    } else {
                        // Fallback to upload record
                        durabilityLevel = (sharkUploadRecord.durabilityLevel !== undefined) ?
                            sharkUploadRecord.durabilityLevel : common.DEF_NUM_COPIES;
                        req.log.info({
                            uploadId: uploadId,
                            partNumber: partNumber,
                            durabilityLevel: durabilityLevel,
                            source: sharkUploadRecord.durabilityLevel !== undefined ? 'upload-record' : 'default',
                            durabilityObjectError: durErr
                        }, 'S3_MPU: Using fallback durability level for part upload');
                    }
                ...
                // FIXME:  this should allow the distribution of
                // mpu final objects be more even and not tax only
                // the first subset of sharks.
                ...
                req.log.info({
                    ...
                    method: 'dcSharkMap',
                    datacenters: Object.keys(sharkMap)
                }, 'S3_MPU: Selected deterministic' +
                   ' sharks from internal shark map');
        ...
        bucketHelpers.getBucketIfExists(durabilityReq, null,
            ...
            var headers = {
                ...
                ((durabilityData.durabilityLevel !== undefined) ?
                 durabilityData.durabilityLevel : common.DEF_NUM_COPIES).toString(),
