...
var awsChunkedEncoding = require('./aws-chunked-encoding');
var bucketHelpers = require('./buckets/buckets');
var common = require('./common');
var constants = require('./constants');
var copyModule = require('./buckets/objects/copy');
var createObjectModule = require('./buckets/objects/create');
var appErrors = require('./errors');
var InvalidDurabilityLevelError = appErrors.InvalidDurabilityLevelError;
// MPU-local error constructors now live in lib/errors.js as proper
// BucketsApiError subclasses. Pull them in by name for ESLint hygiene
// (their runtime global-export is a side effect of lib/errors.js but
// not visible to the linter).
var NoSuchUploadError      = appErrors.NoSuchUploadError;
var InvalidPartNumberError = appErrors.InvalidPartNumberError;
var InvalidPartOrderError  = appErrors.InvalidPartOrderError;
var InvalidPartError       = appErrors.InvalidPartError;
var EntityTooSmallError    = appErrors.EntityTooSmallError;
var EntityTooLargeError    = appErrors.EntityTooLargeError;
var MalformedXMLError      = appErrors.MalformedXMLError;
var InvalidRequestError    = appErrors.InvalidRequestError;
var InternalError          = appErrors.InternalError;
var bucketScopeLib = require('./auth/bucket-scope');
var s3MakoV2Commit = require('./s3-mako-v2-commit');
        ...
        bucketHelpers.loadRequest(uploadReq, null,
            ...
            bucketHelpers.getBucketIfExists(uploadReq, null,
                ...
                client.createObject(owner, uploadReq.bucket.id, uploadRecordKey,
                    ...
                    requestId,
                    function onUploadRecordCreated(createErr, _result) {
...
function validatePartNumber(partNumber) {
    return (typeof (partNumber) === 'number' &&
...
function createIsChunkedFunction(req, partReq) {
    return function () {
        // Transfer-Encoding: chunked is HTTP/1.1 framing — required
        // by the protocol whenever a body is sent without a
        // Content-Length. This is a different layer from the SigV4
        // streaming "aws-chunked" trap; HTTP libraries cannot omit
        // this header without breaking the wire format.
...
function buildPartUploadRequest(req, uploadRecord, bucketName, partKey,
    ...
    awsChunkedEncoding.configureAwsChunkedEncoding(req, partReq, partNumber,
                                                    uploadId);
...
/**
 * Parse the x-amz-copy-source-range header.
 *
 * @param {String} rangeHeader  e.g. "bytes=0-999"
 * @returns {Object|null}  {start, end, length} or null if
 * rangeHeader is undefined or malformed.
 *
 */
function parseCopySourceRange(rangeHeader) {
    if (!rangeHeader) {
        return (null);
    }
...
/**
 * S3 UploadPartCopy — copy a byte range from an existing
 * object as a multipart upload part.
 *
 * Resolves source object metadata, streams the requested
 * byte range from a source shark to the pre-allocated
 * destination sharks, computes MD5, and stores the part
 * metadata.
 *
 * Time:  O(n) source shark attempts + O(bytes) streaming.
 * Space: O(1) beyond stream buffers.
 *
 ...
 * @param {String}   partKey     metadata key for part
 * @param {String}   partId      UUID for this part object
 * @param {String}   copySource  x-amz-copy-source value
 */
function uploadPartCopy(req, res, next, bucketName,
    objectKey, uploadId, partNumber, partKey, partId,
    copySource) {

    var log = req.log;

    // Parse copy source: [/]bucket/key
    var srcPath = copySource.replace(/^\//, '');
    var slashIdx = srcPath.indexOf('/');
    if (slashIdx <= 0) {
        var parseErr = new Error(
            'Invalid x-amz-copy-source: ' + copySource);
        parseErr.statusCode = 400;
        parseErr.restCode = 'InvalidArgument';
        next(parseErr);
        return;
    }
    var srcBucket = srcPath.substring(0, slashIdx);
    var srcObject = srcPath.substring(slashIdx + 1);

    /*
     * Enforce scope on copy source (dest already checked
     * by enforceBucketScope middleware).
     */
    if (bucketScopeLib.enforceCopySourceScope(req, srcBucket,
        next)) {
        return;
    }

    // Build a synthetic request for source resolution
    var sourceReq = {
        params: {
            account: req.params.account ||
                (req.caller && req.caller.account &&
                 req.caller.account.login),
            bucket_name: srcBucket,
            object_name: srcObject
        },
        caller: req.caller,
        owner: req.owner,
        log: log,
        ...
        method: 'GET',
        authContext: { conditions: {} },
        headers: req.headers || {}
    };

    // Step 1: Load source bucket
    bucketHelpers.loadRequest(sourceReq, null,
        function onLoad(loadErr) {
        if (loadErr) {
            log.debug(loadErr,
                'uploadPartCopy: failed to load source');
            var srcErr = new Error('Source object not found');
            srcErr.statusCode = 404;
            srcErr.restCode = 'NoSuchKey';
            next(srcErr);
            return;
        }

        bucketHelpers.getBucketIfExists(sourceReq, null,
            function onBucket(bktErr) {
            if (bktErr) {
                log.debug(bktErr,
                    'uploadPartCopy: source bucket not found');
                var bErr = new Error('Source bucket not found');
                bErr.statusCode = 404;
                bErr.restCode = 'NoSuchBucket';
                next(bErr);
                return;
            }

            // Step 2: Get source object metadata
            copyModule.getSourceObjectMetadata(sourceReq,
                function onMeta(metaErr, srcMeta) {
                if (metaErr) {
                    next(metaErr);
                    return;
                }
                ...
                // Step 3: Parse range
                // We need to return 400 or a 416 according to AWS
                // if clients provide an invalid copy-source-range.
                var rangeHdr =
                    req.headers['x-amz-copy-source-range'];
                var range = parseCopySourceRange(rangeHdr);

                if (rangeHdr && !range) {
                    var rangeErr = new Error(
                        'Invalid copy source range');
                    rangeErr.statusCode = 400;
                    rangeErr.restCode =
                        'InvalidArgument';
                    next(rangeErr);
                    return;
                ...
                if (range && range.end >= srcMeta.content_length) {
                    var oobErr = new Error(
                        'The requested range cannot be satisfied');
                    oobErr.statusCode = 416;
                    oobErr.restCode = 'InvalidRange';
                    next(oobErr);
                    return;
                }

                // When rangeHdr is not present then we copy the whole object
                var copyLen = rangeHdr ?
                    range.length :
                    srcMeta.content_length;

                // 0 bytes copies are disallowed.
                if (copyLen === 0) {
                    next(new EntityTooSmallError(
                        'Copy source has zero bytes'));
                    return;
                }
                // 5GiB limit per part
                if (copyLen > 5368709120) {
                    next(new EntityTooLargeError(
                        'Your proposed upload exceeds ' +
                ...
                log.debug({
                    srcBucket: srcBucket,
                    srcObject: srcObject,
                    srcSize: srcMeta.content_length,
                    range: range || 'full',
                    copyLen: copyLen,
                    partNumber: partNumber,
                    uploadId: uploadId
                }, 'uploadPartCopy: source resolved');

                // Step 4: Get upload record for sharks
                getUploadRecord(req, uploadId,
                    function onRec(recErr, uploadRecord) {
                    if (recErr) {
                        return (next(
                            new NoSuchUploadError(uploadId)));
                    }

                    // Skip S3 response formatter — we send our own XML
                    // on responses, this is here because error responses
                    // above use the S3 error formatter.
                    req._skipS3ResponseProcessing = true;
                    streamPartCopy(req, res, next,
                        sourceReq, srcMeta, range,
                        copyLen, uploadRecord,
                        bucketName, partKey, partId,
                        partNumber, uploadId);
                });
            });
        });
...
/**
 * Stream bytes from source shark to part destination sharks.
 *
 * Tries each source shark with fallback (same pattern as
 * CopyObject).  Pipes through an MD5 hash to compute the
 * part ETag.  Writes the part metadata record on success.
 *
 * @param {Object} req          restify request
 * @param {Object} res          restify response
 * @param {Function} next       restify next
 * @param {Object} sourceReq    source bucket/object info
 * @param {Object} srcMeta      source object metadata
 * @param {Object|null} range   parsed byte range or null
 * @param {Number} copyLen      bytes to copy
 * @param {Object} uploadRecord MPU upload record
 * @param {String} bucketName   destination bucket
 * @param {String} partKey      part metadata key
 * @param {String} partId       part object UUID
 * @param {Number} partNumber   1-based part number
 * @param {String} uploadId     multipart upload id
 */
function streamPartCopy(req, res, next, sourceReq, srcMeta,
    range, copyLen, uploadRecord, bucketName, partKey,
    partId, partNumber, uploadId) {

    var log = req.log;
    var owner = req.owner.account.uuid;
    var sharks = srcMeta.sharks;
    var startIdx = Date.now() % sharks.length;

    var srcStoragePath = sharkClient.storagePath({
    ...
    // Get pre-allocated destination sharks from upload record
    var destSharks = uploadRecord.preAllocatedSharks ||
        (uploadRecord.properties &&
         uploadRecord.properties.sharks) ||
        [];

    if (destSharks.length === 0) {
        var noSharkErr = new Error(
            'No destination sharks for upload');
        noSharkErr.statusCode = 500;
        next(noSharkErr);
        return;
    }
    ...
    /**
     * Try source sharks with fallback.
     */
    function tryNextSourceShark() {
        if (attempt >= sharks.length) {
            log.error({
                totalAttempts: attempt,
                uploadId: uploadId,
                partNumber: partNumber
            }, 'streamPartCopy: all source sharks exhausted');

            var srcErr = new Error(
                'Unable to read source object data');
            srcErr.statusCode = 404;
            srcErr.restCode = 'NoSuchKey';
            next(srcErr);
            return;
        ...
        var idx = (startIdx + attempt) % sharks.length;
        var srcShark = sharks[idx];
        attempt++;

        var srcClient = sharkClient.getClient({
        ...
        if (range) {
            srcOpts.range = 'bytes=' + range.start +
                '-' + range.end;
        }

        log.debug({
            attempt: attempt,
            shark: srcShark.manta_storage_id,
            range: srcOpts.range || 'full',
            partNumber: partNumber
        }, 'streamPartCopy: trying source shark');

        srcClient.get(srcOpts,
            function onSrcGet(getErr, _srcReq, srcRes) {

            if (getErr || !srcRes ||
                (srcRes.statusCode !== 200 &&
                 srcRes.statusCode !== 206)) {
                ...
                log.warn({
                    ...
                    attempt: attempt,
                    statusCode: srcRes ?
                        srcRes.statusCode : 'no-response'
                }, 'streamPartCopy: source shark failed');
            ...
            log.debug({
                shark: srcShark.manta_storage_id,
                statusCode: srcRes.statusCode,
                partNumber: partNumber
            }, 'streamPartCopy: source stream ready');

            pipeToDestSharks(srcShark, srcOpts,
                srcRes);
        });
    }

    /**
     * Pipe source stream to each destination shark,
     * compute MD5, then write part metadata.
     */
    function pipeToDestSharks(srcShark, srcOpts, srcRes) {
        var md5 = crypto.createHash('md5');
        var totalBytes = 0;
        var completed = 0;

        // Part object name hash for storage path
        var partNameHash = crypto.createHash('md5')
            .update(partKey).digest('hex');

        vasync.forEachParallel({
            func: function writeToShark(destShark, cb) {
                ...
                var putOpts = {
                    bucketId: req.bucket ?
                        req.bucket.id : uploadRecord.bucketId,
                    objectId: partId,
                    objectName: partKey,
                    objectNameHash: partNameHash,
                    owner: owner,
                    requestId: req.getId(),
                    storageLayoutVersion: 2,
                    contentType: srcMeta.content_type ||
                        'application/octet-stream',
                    contentLength: copyLen
                };

                // Create a new source stream per dest shark
                var dSrcClient = sharkClient.getClient({
                    connectTimeout:
                        req.sharkConfig.connectTimeout,
                    log: log,
                    shark: srcShark,
                ...
                dSrcClient.get(srcOpts,
                    function onGet(gErr, _gReq, gRes) {
                    if (gErr || !gRes ||
                        (gRes.statusCode !== 200 &&
                         gRes.statusCode !== 206)) {
                        cb(new Error(
                            'Source read failed for ' +
                            destShark.manta_storage_id));
                        return;
                    }

                    destClient.put(putOpts,
                        function onPut(pErr, putReq) {
                        if (pErr) {
                            cb(pErr);
                            return;
                        }
                        ...
                        putReq.on('response',
                            ...
                            totalBytes = sharkBytes;
                            md5 = sharkMd5;
                            completed++;
                            cb();
                        });

                        putReq.on('error', function (sErr) {
                            cb(sErr);
                        });
            ...
            inputs: destSharks
        }, function onAllDone(pipeErr) {
            if (pipeErr) {
                log.error({
                    err: pipeErr,
                    partNumber: partNumber,
                    uploadId: uploadId
                }, 'streamPartCopy: pipe to dest failed');
                next(pipeErr);
                return;
            }

            var partMd5Base64 = md5.digest('base64');
            var partMd5Hex = Buffer.from(
                partMd5Base64, 'base64').toString('hex');
            ...
            log.debug({
                ...
                md5: partMd5Hex,
                completedSharks: completed
            }, 'streamPartCopy: data streamed, ' +
               'creating part metadata');

            // Write part metadata via createObject
            writePartCopyMetadata(req, res, next,
                bucketName, partKey, partId, partNumber,
                uploadId, totalBytes, partMd5Base64,
                partMd5Hex, destSharks,
                srcMeta.content_type);
        });
    }
...
/**
 * Write part metadata record and send the UploadPartCopy
 * XML response.
 *
 * @param {Object}   req           restify request
 * @param {Object}   res           restify response
 ...
 * @param {String}   bucketName    destination bucket
 * @param {String}   partKey       part metadata key
 * @param {String}   partId        part UUID
 * @param {Number}   partNumber    1-based part number
 * @param {String}   uploadId      multipart upload id
 * @param {Number}   contentLength bytes copied
 * @param {String}   md5Base64     base64 MD5
 * @param {String}   md5Hex        hex MD5 (ETag)
 * @param {Array}    sharks        destination shark records
 * @param {String}   contentType   source content type
 */
function writePartCopyMetadata(req, res, next,
    bucketName, partKey, partId, partNumber,
    uploadId, contentLength, md5Base64, md5Hex,
    sharks, contentType) {

    var log = req.log;
    var owner = req.owner.account.uuid;

    var bucketId = req.bucket ? req.bucket.id : null;
    if (!bucketId) {
        // Resolve bucket id from metadata placement
        var bktLoc =
            req.metadataPlacement.getBucketLocation(
                owner, bucketName);
        var bktClient =
            req.metadataPlacement.getBucketsMdapiClient(
                bktLoc);
        bktClient.getBucket(owner, bucketName, bktLoc.vnode,
            req.getId(), function onBkt(bErr, bData) {
            if (bErr) {
                next(bErr);
                return;
            }
    ...
    function doWrite(bId) {
        var partNameHash = crypto.createHash('md5')
            .update(partKey).digest('hex');
        var metaLoc =
            req.metadataPlacement.getObjectLocation(
                owner, bId, partNameHash);
        var mdClient =
            req.metadataPlacement.getBucketsMdapiClient(
                metaLoc);

        var sharkRecords = sharks.map(function (s) {
            return ({
                datacenter: s.datacenter ||
                    s._shark && s._shark.datacenter,
                manta_storage_id: s.manta_storage_id ||
                    s._shark && s._shark.manta_storage_id
            });
        });
        ...
        mdClient.createObject(owner, bId, partKey,
            partId, contentLength, md5Base64,
            contentType || 'application/octet-stream',
            {}, sharkRecords, props, metaLoc.vnode,
            conditions, req.getId(),
            function onCreate(createErr, result) {
            if (createErr) {
                log.error({
                    err: createErr,
                    partNumber: partNumber,
                    uploadId: uploadId
                }, 'writePartCopyMetadata: failed');
                next(createErr);
                return;
            }
            ...
            }, 'writePartCopyMetadata: part created');

            // Send UploadPartCopy XML response
...
function s3UploadPartHandler() {
    return function s3UploadPart(req, res, next) {
        ...
        // Check for UploadPartCopy (x-amz-copy-source header)
        ...
        if (copySource) {
            req.log.debug({
                ...
                partNumber: partNumber,
                uploadId: uploadId
            }, 'S3_MPU: UploadPartCopy detected');

            uploadPartCopy(req, res, next, bucketName,
                objectKey, uploadId, partNumber,
                partKey, partId, copySource);
            return;
        }
        ...
        calculateActualTotalSize(req, uploadId, partETags,
            function onTotalSizeCalculated(sizeErr, actualTotalSize) {
                if (sizeErr) {
                    ...
                    // appErrors.InternalError takes a cause Error (not
                    // a string) — pass the underlying sizeErr through.
                    return (cleanupAndExit(
                        new InternalError(sizeErr),
...
function parseCompleteMultipartUploadXML(xmlBody) {
    ...
    partMatches.forEach(function(partXml) {
        ...
        // Handle both literal quotes and XML-encoded &#34; quotes
        var etagMatch = partXml.match(
            /<ETag>(?:"|&#34;)?([^"<&]+)(?:"|&#34;)?<\/ETag>/);
    ...
    // Preserve the client-supplied order. validatePartOrder (called
    // from validatePartsForComplete) checks for ascending order and
    // emits 400 InvalidPartOrder when violated; sorting here would
    // silently rewrite the input and make that validation
    // unreachable for non-conforming clients.
    return parts;
}
...
function calculateActualTotalSize(req, uploadId, partETags, callback) {
    ...
    bucketHelpers.loadRequest(uploadReq, null, function (loadErr) {
        ...
        bucketHelpers.getBucketIfExists(uploadReq, null, function (bucketErr) {
            ...
            // We pass {partNumber, etag} objects instead of bare ETags
            // because indexOf() deduplicates identical ETags (same MD5).
            var indexedPartETags = partETags.map(function (etag, idx) {
                return ({ partNumber: idx + 1, etag: etag });
            });

            vasync.forEachParallel({
                concurrency: 10, // Limit concurrent requests
                func: function getPartSize(partEntry, next) {
                    var partNumber = partEntry.partNumber;
...
function assembleMultipartUpload(req, uploadRecord, commitBody, callback) {
    ...
    discoverPartPaths(req, uploadRecord, commitBody.parts,
        ...
        s3MakoV2Commit.tryMakoV2Commit(req, uploadRecord, partPaths,
...
function discoverPartPaths(req, uploadRecord, partETags, callback) {
    ...
    // We pass {partNumber, etag} objects instead of bare ETags because
    // indexOf() returns the first match, which silently deduplicates
    // parts with identical content (and therefore identical ETags).
    var indexedParts = partETags.map(function (etag, idx) {
        return ({ partNumber: idx + 1, etag: etag });
    });

    vasync.forEachParallel({
        func: function queryPartObject(partEntry, next) {
            queryPartMetadataAndPath(req, uploadId, partEntry.partNumber,
                partEntry.etag, owner, bucketId, partPaths, next);
        },
        inputs: indexedParts
...
/*
 * Delete the metadata row for one .mpu-parts/{uploadId}/{n} entry on the vnode
 * that owns it. Idempotent: ObjectNotFound counts as a successful sweep. The
 * summary counters live in the caller's closure so partial failure is
 * observable in aggregate; this function neversurfaces an error.
 */
function deleteOnePartMetadata(req, owner, bucketId, partObjectName,
                               summary, callback) {
    var metadataLocation = req.metadataPlacement.getObjectLocation(
        owner, bucketId, crypto.createHash('md5')
            .update(partObjectName)
    ...
    client.deleteObject(owner, bucketId, partObjectName,
        metadataLocation.vnode, {}, req.getId(),
        function (delErr) {
            if (!delErr) {
                summary.deleted += 1;
            } else if (delErr.name === 'ObjectNotFound') {
                summary.missing += 1;
            } else {
                summary.failed += 1;
                req.log.warn({
                    err: delErr,
                    partObjectName: partObjectName
                }, 'S3_MPU: Failed to delete part metadata row');
            }
            return (callback());
        });
}
...
/*
 * Sweep every .mpu-parts/{uploadId}/{partNumber} metadata row from
 * buckets-mdapi after a successful v2 commit. Best-effort:
 *
 *   - Paginates listObjects on the .mpu-parts/{uploadId}/ prefix
 *     (same approach as listUploadedParts) so stragglers from
 *     overwritten parts or unused part numbers are also removed.
 *   - Drives per-part deleteObject with vasync.queue at fixed
 *     concurrency CLEANUP_DELETE_CONCURRENCY to bound mdapi fan-out
 *     across vnodes.
 *   - ObjectNotFound is treated as success (idempotent on retry).
 *   - Never propagates an error; the upload has already been
 *     committed and acknowledged to the client.
 */
function cleanupPartObjects(req, uploadId, bucketId, owner, callback) {
    var partsPrefix = '.mpu-parts/' + uploadId + '/';
    var pageLimit = constants.SIZE_LIMITS.MAX_PAGE_LIMIT;
    var CLEANUP_DELETE_CONCURRENCY = 25;
    var summary = { deleted: 0, missing: 0, failed: 0 };
    var marker = null;
    var listFailed = false;
    var listError = null;

    var queue = vasync.queue(function workerDelete(partObjectName, cb) {
        deleteOnePartMetadata(req, owner, bucketId, partObjectName,
            summary, cb);
    }, CLEANUP_DELETE_CONCURRENCY);

    queue.on('end', function onQueueEnd() {
        if (listFailed) {
            req.log.warn({
                err: listError,
                uploadId: uploadId,
                summary: summary
            }, 'S3_MPU: Part-metadata sweep ended with' +
               ' partial list (continuing anyway)');
        } else if (summary.failed > 0) {
            req.log.warn({
                uploadId: uploadId,
                summary: summary
            }, 'S3_MPU: Part-metadata sweep had per-part failures');
        } else {
            req.log.debug({
                uploadId: uploadId,
                summary: summary
            }, 'S3_MPU: Part-metadata sweep complete');
        }
    ...
    function listNextPage() {
        ...
        // Override isS3Request so the .mpu-parts S3 filter
        // in buckets/common.js does not discard our parts.
        listReq.isS3Request = false;
        listReq.query = {
            prefix: partsPrefix,
            limit: pageLimit,
            marker: marker
        ...
        mreq.once('error', function (listErr) {
            req.log.warn({
                ...
                uploadId: uploadId,
                marker: marker
            }, 'S3_MPU: listObjects failed during part sweep');
            listFailed = true;
            listError = listErr;
            queue.close();
        });
        ...
        mreq.on('entry', function (entry) {
            if (!entry || !entry.name ||
                !entry.name.startsWith(partsPrefix)) {
                return;
            }
    ...
    }, 'S3_MPU: Starting part-metadata sweep');

    // No-parts case: queue.close() with nothing pushed still fires
    // the 'end' handler above, so the single callback path is
    // preserved.
    listNextPage();
}
    ...
    function cleanupUploadRecordAndDurability() {
        ...
        bucketHelpers.loadRequest(uploadReq, null, function (loadErr) {
            ...
            bucketHelpers.getBucketIfExists(uploadReq, null,
                ...
                /*
                 * Sweep .mpu-parts/{uploadId}/ metadata rows first.
                 * The v2 commit physically deleted the backing part
                 * files on the sharks; the metadata rows are
                 * referentially dead and must not survive completion.
                 * Best-effort: any per-part failure is logged inside
                 * cleanupPartObjects and never blocks the
                 * upload-record / durability cleanup below.
                 */
                cleanupPartObjects(req, uploadId, uploadReq.bucket.id,
                    owner, function onPartSweepDone(_sweepErr, _summary) {
                ...
                }); // End cleanupPartObjects callback
            });
        });
...
function listUploadedParts(req, uploadId, uploadRecord, callback) {
    ...
    function listNextPage() {
        ...
        mreq.on('entry', function (entry) {
            ...
            /*
             * Convert base64 MD5 to 32-char hex for
             * S3 ETag compatibility.  All S3 clients
             * (aws-cli, s3cmd, boto3) expect hex ETags
             * in ListParts for resume and completion.
             */
            var etag;
            if (entry.contentMD5) {
                try {
                    etag = Buffer.from(
                        entry.contentMD5, 'base64')
                        .toString('hex');
                } catch (e) {
                    etag = entry.id || '';
                }
            ...
            req.log.debug({
                ...
                selectedETag: etag,
                etagFormat: entry.contentMD5 ?
                    'md5-hex' : 'uuid'
            }, 'S3_MPU: ETag for ListParts');
...
/*
 * MPU-local error constructors used to live here. They returned
 * plain Error instances with statusCode/restCode tacked on, which
 * restify does not recognize — every one of them served a generic
 * 500 InternalError on the wire instead of the intended 4xx /
 * S3-shaped XML body. They have been moved into lib/errors.js as
 * proper BucketsApiError -> restify RestError subclasses. All
 * references in this file resolve to those classes via the global
 * auto-export at the bottom of lib/errors.js (which is required
 * here as `appErrors`). No code change needed at the call sites.
 */
...
module.exports = {
    ...
    validatePartNumber: validatePartNumber,
    generatePartKey: generatePartKey,
    extractPartETags: extractPartETags,
    parseAndValidateXMLBody: parseAndValidateXMLBody,
    validatePartOrder: validatePartOrder,
    validateNoGaps: validateNoGaps,
    resolveETag: resolveETag,

    // Lock management helpers (exported for testing)
    createLockData: createLockData,
    parseLockState: parseLockState,
    determineLockAction: determineLockAction,

    // Complete MPU helpers (exported for testing)
    createCommitBody: createCommitBody,
    transformAssemblyError: transformAssemblyError,
    releaseLockSafely: releaseLockSafely,
