            ...
            updateUploadRecord(req, uploadId, partNumber, {
                ...
                if (updateErr) {
                    req.log.warn(updateErr,
                       'S3_MPU: Failed to update upload record with part info');
...
function s3CompleteMultipartUploadHandler() {
    return function s3CompleteMultipartUpload(req, res, next) {
        ...
        getUploadRecord(req, uploadId, function (getErr, uploadRecord) {
            ...
            // Calculate actual total size from uploaded parts using buckets-mdapi
            calculateActualTotalSize(req, uploadId, partETags, function(sizeErr, actualTotalSize) {
                if (sizeErr) {
                    req.log.error(sizeErr, 'S3_MPU: Failed to calculate actual total size');
                    return next(new InternalError('Failed to calculate multipart upload size'));
                }
...
function assembleMultipartUpload(req, uploadRecord, commitBody, callback) {
    ...
    discoverPartPaths(req, uploadRecord, commitBody.parts, function(err, partPaths) {
        ...
        customAssembleMultipartUpload(req, uploadRecord, partPaths, finalCommitBody, callback);
    });
}
