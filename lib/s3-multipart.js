...
function s3InitiateMultipartUploadHandler() {
    return function s3InitiateMultipartUpload(req, res, next) {
        ...
        // Store durability level for later shark allocation during first part upload
        ...
        uploadRecord.preAllocatedSharks = null; // Will be populated during first part upload
...
function s3UploadPartHandler() {
    return function s3UploadPart(req, res, next) {
        ...
        getUploadRecord(req, uploadId, function (getErr, uploadRecord) {
            ...
            getUploadRecord(req, uploadId, function (recErr, uploadRecord) {
                req.log.debug({
                    uploadId: uploadId,
                    partNumber: partNumber,
                    hasError: !!recErr,
                    hasUploadRecord: !!uploadRecord,
                ...
                if (recErr || !uploadRecord) {
                    req.log.warn({
                        ...
                        partNumber: partNumber,
                        error: recErr
                    }, 'S3_MPU: Could not retrieve upload record, using default allocation');
                    proceedWithPartUpload();
                    return;
                }
                
                // Set durability level from upload record
                if (uploadRecord.durabilityLevel) {
                    partReq.headers['durability-level'] = uploadRecord.durabilityLevel.toString();
                    req.log.info({
                        uploadId: uploadId,
                        partNumber: partNumber,
                        durabilityLevel: uploadRecord.durabilityLevel
                    }, 'S3_MPU: Set durability-level header from upload record');
                }
                
                // Use deterministic shark selection for MPU consistency
                var durabilityLevel = uploadRecord.durabilityLevel || common.DEF_NUM_COPIES;
                ...
                // Access internal dcSharkMap directly (same source as storinfo.choose)
                ...
                if (!sharkMap) {
                    req.log.warn({
                        uploadId: uploadId,
                        partNumber: partNumber
                    }, 'S3_MPU: No shark map available, using default allocation');
                    proceedWithPartUpload();
                    return;
                }
                ...
                if (allSharks.length === 0) {
                    ...
                    }, 'S3_MPU: No sharks found in shark map, using default allocation');
                    proceedWithPartUpload();
                    return;
                }
                
                // Sort ALL sharks by name to ensure deterministic selection
                var sortedSharks = allSharks.slice().sort(function (a, b) {
                    return a.manta_storage_id.localeCompare(b.manta_storage_id);
                });
                
                // Take the first N sharks based on durability level
                ...
                req.log.info({
                    ...
                    method: 'dcSharkMap',
                    datacenters: Object.keys(sharkMap)
                }, 'S3_MPU: Selected deterministic sharks from internal shark map');
...
function updateUploadRecordContent(req, uploadRecordKey, content, callback) {
    ...
    bucketHelpers.loadRequest(uploadReq, null, function (loadErr) {
        ...
        bucketHelpers.getBucketIfExists(uploadReq, null, function (bucketErr) {
            ...
            var contentMD5 = crypto.createHash('md5').update(content).digest('base64');
            var metadataLocation = req.metadataPlacement.getObjectLocation(
                owner, uploadReq.bucket.id, crypto.createHash('md5')
                                               .update(uploadRecordKey)
                                               .digest('hex'));
            ...
            // For simplicity, let's delete and recreate the object with new content
            ...
            client.getObject(owner, uploadReq.bucket.id, uploadRecordKey,
                ...
                client.deleteObject(owner, uploadReq.bucket.id, uploadRecordKey,
                                   ...
                                   function (deleteErr) {
                    if (deleteErr) {
                        req.log.warn(deleteErr, 'S3_MPU: Failed to delete existing upload record');
            ...
            client.getObject(req.owner.account.uuid, durabilityReq.bucket.id,
                ...
                if (result.value) {
                    ...
                    try {
                        ...
                        if (contentData.sharks && Array.isArray(contentData.sharks)) {
                            ...
                            }, 'S3_MPU: Extracted pre-allocated sharks from durability object');
                        }
                    } catch (parseErr) {
                        req.log.warn({
                            durabilityKey: durabilityKey,
