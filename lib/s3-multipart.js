 ...
 *
 *  Assembling parts:
 *  buckets-mdapi is used directly to create the final object ensuring the
...
function s3InitiateMultipartUploadHandler() {
    return function s3InitiateMultipartUpload(req, res, next) {
        ...
        storeDurabilityObject(req, durabilityKey, durabilityData,
            ...
            if (durErr) {
                req.log.warn(durErr,
                 'S3_MPU: Failed to store durability object during initiation');
                // Continue anyway, will fall back to upload record or headers
            }
    ...
    return function s3UploadPart(req, res, next) {
        ...
        getUploadRecord(req, uploadId, function (getErr, uploadRecord) {
            ...
            if (partReq.headers['content-encoding'] === 'aws-chunked') {
                ...
                // Ensure proper size handling for AWS chunked uploads
                var decodedLength =
                    partReq.headers['x-amz-decoded-content-length'];
                if (decodedLength) {
                    partReq._size = parseInt(decodedLength, 10);
                    req.log.debug({
                        uploadId: uploadId,
                        partNumber: partNumber,
                        decodedSize: partReq._size
                    }, 'S3_MPU: Using decoded content length for part size');
                }
            }
...
function s3CompleteMultipartUploadHandler() {
    return function s3CompleteMultipartUpload(req, res, next) {
        ...
        getUploadRecord(req, uploadId, function (getErr, uploadRecord) {
            ...
            // Use the same durability object to store completion status
            // atomically
            var durabilityKey = '.mpu-uploads/' + uploadId + '.durability';

            // First check if this upload is already being completed
            req.log.debug({
                uploadId: uploadId,
                durabilityKey: durabilityKey
            }, 'S3_MPU: Checking for existing completion status in metadata');

            getDurabilityObject(req, durabilityKey,
                                function (durErr, durabilityData) {
                req.log.debug({
                    uploadId: uploadId,
                    durErr: durErr ? durErr.message : null,
                    durabilityData: durabilityData,
                    hasStatus: durabilityData ? !!durabilityData.status : false,
                    statusValue: durabilityData ? durabilityData.status : 'none'
                }, 'S3_MPU: Retrieved durability object for status check');

                if (!durErr && durabilityData &&
                    durabilityData.status === 'completing') {
                    req.log.warn({
                        uploadId: uploadId,
                        status: durabilityData.status,
                        instanceId: req.getId()
                    }, 'S3_MPU: Upload already being processed' +
                                 ' by another instance');
                    return (next
                            (new InternalError
                ...
                // Update the durability object to include completion status
                var updatedDurabilityData = durabilityData || {
                    uploadId: uploadId,
                    durabilityLevel: common.DEF_NUM_COPIES,
                    created: new Date().toISOString()
                };

                // Mark as completing to prevent race conditions
                updatedDurabilityData.status = 'completing';
                updatedDurabilityData.instanceId = req.getId();
                updatedDurabilityData.completingTimestamp =
                                        new Date().toISOString();

                // Store updated durability object with completion status
                req.log.debug({
                    uploadId: uploadId,
                    updatedDurabilityData: updatedDurabilityData,
                    durabilityKey: durabilityKey
                }, 'S3_MPU: Storing completion status in durability metadata');

                storeDurabilityObject(req, durabilityKey, updatedDurabilityData,
                                      function (statusErr) {
                    if (statusErr) {
                        req.log.warn({
                            uploadId: uploadId,
                            ...
                            instanceId: req.getId()
                        }, 'S3_MPU: Failed to update durability' +
                                     ' object with completion status');
                    ...
                    req.log.info({
                        uploadId: uploadId,
                        instanceId: req.getId(),
                        durabilityLevel: updatedDurabilityData.durabilityLevel
                    }, 'S3_MPU: Successfully' +
                                 ' acquired lock for multipart completion');
            ...
            calculateActualTotalSize(req, uploadId, partETags,
                ...
                assembleMultipartUpload(req, uploadRecord, commitBody,
                    ...
                    req.log.info({
                        uploadId: uploadId,
                        step: 'assembly-callback',
                        assembleErr: assembleErr ? assembleErr.message : null,
                        hasResult: !!assembleResult
                    }, 'S3_MPU: CHECKPOINT 7' +
                                 ' - assembleMultipartUpload callback entered');
                    if (assembleErr) {
            ...
            } // Close continueWithCompletion function
        });
    };
}
...
function DistributedLockManager(req) {
    this.req = req;
    this.lockTimeout = 300000; // 5 minutes - should exceed max assembly time
    this.retryInterval = 1000; // 1 second between acquisition attempts
    this.maxRetries = 30; // Maximum 30 seconds wait time for lock
}
 ...
 */
DistributedLockManager.prototype.acquireLock = function(uploadId, options, callback) {
    ...
    var instanceId = self.req.getId() + '-' + Date.now(); // Unique instance identifier
    ...
    function attemptLock() {
        ...
        if (currentRetry >= maxRetries) {
            var timeoutErr = new Error('Failed to acquire lock after ' + maxRetries + ' retries');
        ...
        var lockData = {
            ...
            expires: new Date(Date.now() + lockTimeout).toISOString(), // Lease expiration
        ...
        lockReq.params = {
            bucket_name: self.req.s3Request.bucket, // Store locks in same bucket
            object_name: lockKey
        };
            ...
            bucketHelpers.getBucketIfExists(lockReq, null, function (bucketErr) {
                ...
                var metadataLocation = self.req.metadataPlacement.getObjectLocation(
                    owner, lockReq.bucket.id, crypto.createHash('md5')
                                                   .update(lockKey)
                                                   .digest('hex'));
                var client = self.req.metadataPlacement.getBucketsMdapiClient(metadataLocation);
                ...
                client.getObject(owner, lockReq.bucket.id, lockKey,
                    ...
                    if (!getErr && existingLock) {
                        ...
                        try {
                            ...
                            var expires = new Date(existingData.expires);
                            var now = new Date();
                            ...
                            // CASE 1: We already own this lock (idempotent operation)
                            ...
                            if (now > expires) {
                                ...
                                // ATOMIC UPDATE: Compare-and-swap using object ID
                                attemptLockUpdate(existingLock.id);
                                return;
                            }

                            // CASE 3: Lock valid and owned by someone else - wait and retry
                            ...
                            }, 'S3_MPU_LOCK: Lock held by another instance, waiting for expiry or release');
                            ...
                            }, 'S3_MPU_LOCK: Failed to parse existing lock data, will retry');
                    ...
                    // ATOMIC CREATION: createObject will fail if lock already exists
                    client.createObject(owner, lockReq.bucket.id, lockKey,
                                      ...
                                      self.req.getId(), function (createErr, result) {
                        
                        if (createErr) {
                            // RACE CONDITION: Someone else created lock between our check and create
                ...
                function attemptLockUpdate(existingObjectId) {
                    ...
                    // ATOMIC UPDATE: updateObject will fail if object was deleted/changed
                    client.updateObject(owner, lockReq.bucket.id, lockKey,
                                      existingObjectId, 'application/json',
                                      {
                                          ...
                                          'x-lock-expires': lockData.expires,
                                          'x-lock-hostname': lockData.hostname,
                                          'content-length': lockContent.length
                                      }, {}, metadataLocation.vnode, {}, 
                                      self.req.getId(), function (updateErr, updateResult) {
                        
                        if (updateErr) {
                            // CONCURRENT MODIFICATION: Someone else claimed or deleted lock
                            if (updateErr.name === 'ObjectNotFoundError') {
                                self.req.log.debug({
                                    uploadId: uploadId
                                }, 'S3_MPU_LOCK: Expired lock was deleted by someone else, retrying');
                                setTimeout(attemptLock, self.retryInterval);
                                return;
                            }
            ...
            client.getObject(owner, lockReq.bucket.id, lockKey,
                ...
                if (getErr) {
                    if (getErr.name === 'ObjectNotFoundError') {
                        // ALREADY RELEASED: Lock doesn't exist (idempotent operation)
                        self.req.log.debug({
                            lockKey: lockKey,
                            instanceId: instanceId
                ...
                // VERIFY OWNERSHIP: Prevent accidental release by wrong instance
                try {
                    ...
                    if (lockData.instanceId !== instanceId) {
                        self.req.log.warn({
                            ...
                            actualOwner: lockData.instanceId,
                            actualHostname: lockData.hostname
                    ...
                    self.req.log.warn({
                        err: parseErr,
                        lockKey: lockKey
                    }, 'S3_MPU_LOCK: Failed to parse lock data during release, proceeding anyway');
                }
...
DistributedLockManager.prototype.renewLock = function(lockInfo, callback) {
    var self = this;
    var renewalInterval = Math.floor(self.lockTimeout / 3); // Conservative renewal timing
    var maxRenewals = 10; // Prevent infinite renewal (50 minutes max for 5min lease)
    var renewalCount = 0;
    ...
    function scheduleRenewal() {
        ...
        setTimeout(function() {
            ...
            var newExpiry = new Date(Date.now() + self.lockTimeout).toISOString();
            var lockReq = Object.create(self.req);
            lockReq.params = {
                bucket_name: self.req.s3Request.bucket,
            ...
            bucketHelpers.loadRequest(lockReq, null, function (loadErr) {
                if (loadErr) {
                    self.req.log.warn(loadErr, 'S3_MPU_LOCK: Failed to renew lock - load error');
                ...
                bucketHelpers.getBucketIfExists(lockReq, null, function (bucketErr) {
                    if (bucketErr) {
                        self.req.log.warn(bucketErr, 'S3_MPU_LOCK: Failed to renew lock - bucket error');
                        scheduleRenewal(); // Try again next interval
                        return;
                    }
                    ...
                    client.updateObject(owner, lockReq.bucket.id, lockInfo.lockKey,
                                      lockInfo.objectId, 'application/json',
                                      {
                                          'x-lock-expires': newExpiry,
                                          'x-lock-renewal-count': renewalCount.toString()
                                      }, {}, metadataLocation.vnode, {}, 
...
function validatePartsForComplete(uploadRecord, partsFromXML, req, callback) {
    ...
    // Step 3: Validate each part exists in storage with correct ETag
    vasync.forEachParallel({
        func: function validatePart(xmlPart, next) {
            ...
            client.getObject(owner, bucketId, partObjectName,
                ...
                var storedETag = partMeta.id || partMeta.etag;
        ...
        req.log.info({
            ...
            totalSize: partsFromXML.reduce(function (sum, p) {
                return (sum + (p.size || 0));
            }, 0)
