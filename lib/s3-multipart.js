...
// In-memory cache for upload durability levels (simple and reliable)
var uploadDurabilityCache = {};
...
function s3InitiateMultipartUploadHandler() {
    return function s3InitiateMultipartUpload(req, res, next) {
        ...
        // Extract and validate durability-level header (same logic as regular object creation)
        ...
        // Store durability level in cache for this upload
        uploadDurabilityCache[uploadId] = copies;
        
        req.log.debug({
            uploadId: uploadId,
            durabilityLevel: copies,
            source: req.header('durability-level') ? 'durability-level' : 
                   req.header('x-durability-level') ? 'x-durability-level' : 'default'
        ...
        // The durability level is already stored in uploadRecord.durabilityLevel 
...
function s3UploadPartHandler() {
    return function s3UploadPart(req, res, next) {
        ...
        getUploadRecord(req, uploadId, function (getErr, uploadRecord) {
            if (getErr) {
                req.log.error(getErr, 'S3_MPU: Failed to read upload record for part');
            ...
            // Set durability-level header from cache
            var cachedCopies = uploadDurabilityCache[uploadId];
            if (cachedCopies) {
                partReq.headers['durability-level'] = cachedCopies.toString();
                req.log.debug({
                    uploadId: uploadId,
                    partNumber: partNumber,
                    durabilityLevel: cachedCopies
                }, 'S3_MPU: Set durability-level header from cache');
            } else {
                req.log.warn({
                    uploadId: uploadId,
                    partNumber: partNumber
                }, 'S3_MPU: No durability level found in cache');
            }
        ...
        bucketHelpers.getBucketIfExists(uploadReq, null, function (bucketErr) {
            ...
            client.getObject(owner, uploadReq.bucket.id, uploadRecordKey,
                ...
                try {
                    ...
                    // Extract durability level from object headers/properties
                    if (result.headers && result.headers['x-durability-level-metadata']) {
                        uploadRecord.durabilityLevel = parseInt(result.headers['x-durability-level-metadata'], 10);
                    }
                    ...
                    }, 'S3_MPU: Retrieved durability level from upload record metadata');
...
function streamAndAssembleParts(req, partPaths, callback) {
    ...
    // Use durability level from cache, fall back to request headers 
    var uploadId = req._uploadRecord ? req._uploadRecord.uploadId : null;
    var copies = (uploadId && uploadDurabilityCache[uploadId]) ||
    ...
    req.storinfo.choose(opts, function (sharkErr, sharks) {
        ...
        // final object ID
        streamPartsToFinalSharks(req, partPaths, flatSharks, req._finalObjectId,
