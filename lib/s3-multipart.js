...
function s3UploadPartHandler() {
    return function s3UploadPart(req, res, next) {
        ...
        getUploadRecord(req, uploadId, function (getErr, uploadRecord) {
            ...
            // For AWS chunked MPU, the CheckStream should record the decoded
            // size to match what actually gets stored after AwsChunkedDecoder
            // processing
            if (partReq.headers['content-encoding'] === 'aws-chunked' &&
                ...
                if (!isNaN(decodedSize)) {
                    // Set the expected size for CheckStream validation
                    // and metadata
                    partReq._size = decodedSize;
                    // Mark this request so CheckStream knows to expect
                    // decoded size
                    partReq._awsChunkedExpectedSize = decodedSize;
                    req.log.debug({
                        originalSize: partReq.headers['content-length'],
                        decodedSize: decodedSize,
                        note: 'Set expected decoded size for' +
                            ' AWS chunked MPU part'
                    }, 'S3_MPU:' +
                       ' Will validate against decoded size after' +
                       ' AwsChunkedDecoder');
                }
            }
            req.log.debug({
