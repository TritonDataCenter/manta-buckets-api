...
function customAssembleMultipartUpload(req, uploadRecord, partPaths, commitBody, callback) {
    ...
    streamAndAssembleParts(req, partPaths, function(streamErr, assemblyData) {
        ...
        createFinalObjectMetadata(req, uploadRecord, commitBody, assemblyData, assemblyData.sharks, function(metaErr, objectResult) {
            if (metaErr) {
...
function streamAndAssembleParts(req, partPaths, callback) {
    ...
    // Instead of buffering in memory, stream directly to final storage
    // First, allocate sharks for the final object
    var opts = {
    ...
    req.storinfo.choose(opts, function(sharkErr, sharks) {
        ...
        // Stream parts directly to final sharks
        streamPartsToSharks(req, partPaths, flatSharks, function(streamErr, assemblyResult) {
            if (streamErr) {
                return callback(streamErr);
            }
...
function streamPartsToSharks(req, partPaths, finalSharks, callback) {
    var owner = req.owner.account.uuid;
    var uploadRecord = req._uploadRecord; // Pass this from the calling context
    var objectName = req.s3Request.object;
    var objectId = req._finalObjectId; // Pass this from the calling context
...
function createFinalObjectMetadata(req, uploadRecord, commitBody, assemblyData, sharks, callback) {
    var owner = req.owner.account.uuid;
    var bucketId = uploadRecord.bucketId;
    var objectName = req.s3Request.object;
    ...
    }, 'S3_MPU: Creating final object metadata');
    ...
    // Format sharks as array of objects (sequence) for RPC protocol
    var sharkData = sharks.map(function(shark) {
        return {
