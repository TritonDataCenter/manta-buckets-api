...
function s3InitiateMultipartUploadHandler() {
    return function s3InitiateMultipartUpload(req, res, next) {
        ...
        var estimatedMaxSizeMB = Math.max(
            ...
            100 // Minimum 100MB buffer for safety
        );
        ...
        req.storinfo.choose(spaceValidationOpts, function (spaceErr, sharks) {
            if (spaceErr && spaceErr.name === 'NotEnoughSpaceError') {
                ...
                }, 'S3_MPU: Insufficient space for multipart upload at initiate');
    ...
    return function s3UploadPart(req, res, next) {
        ...
        getUploadRecord(req, uploadId, function (getErr, uploadRecord) {
            ...
            getUploadRecord(req, uploadId,
                ...
                // Maintains determinism while filtering out sharks with insufficient space

                // Step 1: Filter out sharks with critically low space
                // Use a conservative threshold to exclude only truly problematic sharks
                var minimumSpaceMB = 1024; // 1GB minimum - configurable threshold
                ...
                // Sort by name for consistent results, regardless of space changes
                var sortedSharks = viableSharks.slice().sort(function (a, b) {
                    return a.manta_storage_id.localeCompare(b.manta_storage_id);
                });

                // Step 4: Select first N sharks (deterministic but space-filtered)
                ...
                req.log.info({
                    ...
                    method: 'dcSharkMap',
                    datacenters: Object.keys(sharkMap)
                }, 'S3_MPU: Selected deterministic sharks with space filtering' +
            ...
            lockManager.acquireLock(uploadId, function (lockErr, lockInfo) {
                ...
                assembleMultipartUpload(req, uploadRecord, commitBody,
                    ...
                    if (assembleErr) {
                        ...
                        // Return appropriate error to client based on failure type
                        if (assembleErr.name === 'NotEnoughSpaceError') {
                            // Storage space error - return 507 Insufficient Storage
                            res.send(507, {
                                code: 'InsufficientStorage',
                                message: assembleErr.message
                            });
                        } else {
                            // Other assembly errors - return 500 Internal Server Error
                            res.send(500, {
                                code: 'InternalError', 
                                message: 'Multipart upload assembly failed'
                            });
                        }
                        ...
                        if (completionLockInfo) {
                            lockManager.releaseLock(completionLockInfo,
                                ...
                                if (releaseErr) {
                                    req.log.warn(releaseErr,
                                        'S3_MPU: Failed to release lock after failure');
                                }
                            });
                        }
                    ...
                    req.log.info({
                        ...
                        uploadId: uploadId,
                        synchronousAssembly: true
                    }, 'S3_MPU: Assembly completed successfully - returning success to client');
...
function validateSharkSpaceForCommit(req, partPaths, finalSizeBytes, callback) {
    ...
    if (insufficientSharks.length > 0) {
        ...
        }, 'S3_MPU: Pre-allocated sharks have insufficient space for final object');
    ...
    }, 'S3_MPU: All pre-allocated sharks have sufficient space for final object');
