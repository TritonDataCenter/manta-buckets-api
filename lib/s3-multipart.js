...
function s3CompleteMultipartUploadHandler() {
    return function s3CompleteMultipartUpload(req, res, next) {
        ...
        getUploadRecord(req, uploadId, function (getErr, uploadRecord) {
            ...
            req.log.debug({
                uploadId: uploadId
            }, 'S3_MPU: Acquiring distributed lock for multipart completion');
            ...
            lockManager.acquireLock(uploadId, function (lockErr, lockInfo) {
                if (lockErr) {
                    ...
                    }, 'S3_MPU: Failed to acquire distributed lock for completion');
                    return (next(lockErr));
                }
                ...
                }, 'S3_MPU: Successfully acquired distributed lock for completion');

                // Store lock info for cleanup
                req._mpuLockInfo = lockInfo;
                ...
                if (sizeErr) {
                    ...
                    if (req._mpuLockInfo) {
                        lockManager.releaseLock(req._mpuLockInfo, function (releaseErr) {
                            if (releaseErr) {
                ...
                assembleMultipartUpload(req, uploadRecord, commitBody,
                    ...
                    if (assembleErr) {
                        ...
                        if (req._mpuLockInfo) {
                            lockManager.releaseLock(req._mpuLockInfo, function (releaseErr) {
                                if (releaseErr) {
                                    req.log.warn(releaseErr,
                                                'S3_MPU: Failed to release lock on assembly error');
                                }
                                return next(new
                                            InternalError(
                    ...
                    cleanupMultipartUpload(req, uploadId,
                        ...
                        // Release distributed lock before sending response
                        if (req._mpuLockInfo) {
                            lockManager.releaseLock(req._mpuLockInfo, function (releaseErr) {
                                if (releaseErr) {
                                    req.log.warn(releaseErr,
                                                'S3_MPU: Failed to release lock (non-fatal)');
                                }
                                res.setHeader('Content-Type', 'application/xml');
                                res.send(200, xml);
                                next(false);
            ...
            }); // Close lockManager.acquireLock callback
...
function DistributedLockManager(req) {
    ...
    this.maxRetries = 60; // Maximum 30 seconds wait time (60 * 500ms)
}
