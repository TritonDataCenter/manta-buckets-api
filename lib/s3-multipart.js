...
function s3CompleteMultipartUploadHandler() {
    return function s3CompleteMultipartUpload(req, res, next) {
        ...
        getUploadRecord(req, uploadId, function (getErr, uploadRecord) {
            ...
            // Atomically update status to 'completing' to
            // prevent race conditions
            updateUploadRecordStatus(req, uploadId, 'completing',
                                     function (statusErr, updatedRecord) {
                if (statusErr) {
                    if (statusErr.message && (
                        statusErr.message.indexOf
                        ('already being completed') !== -1 ||
                        statusErr.message.indexOf
                        ('already completed') !== -1)) {
                        req.log.warn({
                            uploadId: uploadId,
                            error: statusErr.message
                        }, 'S3_MPU: Multipart upload race condition detected');
                        return (next(new InternalError
                                     ('Upload already being processed')));
                    }
                    req.log.error(statusErr,
                                  'S3_MPU: Failed to update upload status');
                    return (next(new InternalError
            ...
            }); // Close updateUploadRecordStatus callback
        });
    };
}
...
function updateUploadRecordStatus(req, uploadId, newStatus, callback) {
    var uploadRecordKey = '.mpu-uploads/' + uploadId;
    var owner = req.owner.account.uuid;

    // Get current upload record first
    getUploadRecord(req, uploadId, function (getErr, uploadRecord) {
        if (getErr) {
            return (callback(getErr));
        }

        // Check if already completing/completed
        if (uploadRecord.status === 'completing') {
            req.log.warn({
                uploadId: uploadId,
                currentStatus: uploadRecord.status
            }, 'S3_MPU: Upload already being completed by another instance');
            return (callback(new Error('Upload already being completed')));
        }
        ...
        if (uploadRecord.status === 'completed') {
            req.log.warn({
                uploadId: uploadId,
                currentStatus: uploadRecord.status
            }, 'S3_MPU: Upload already completed by another instance');
            return (callback(new Error('Upload already completed')));
        }
        ...
        if (newStatus === 'completing') {
            uploadRecord.completingInstance = req.getId();
        }
        ...
        // Create request context for updating upload record
        var uploadReq = Object.create(req);
        uploadReq.params = {
            bucket_name: req.s3Request.bucket,
            object_name: uploadRecordKey
        ...
        bucketHelpers.loadRequest(uploadReq, null, function (loadErr) {
            ...
            bucketHelpers.getBucketIfExists(uploadReq, null,
                ...
                // Use createObject to update/overwrite the existing record
                client.createObject(owner, uploadReq.bucket.id, uploadRecordKey,
                    uuidv4(), updatedContent.length,
                                    contentMD5, 'application/json',
                    {}, [], {}, metadataLocation.vnode, {},
                    req.getId(), function (createErr, result) {
                    if (createErr) {
                        req.log.error(createErr,
                            'S3_MPU: Failed to update upload record status');
                    ...
                    req.log.debug({
                        ...
                        newStatus: newStatus,
                        instanceId: req.getId()
                    }, 'S3_MPU: Successfully updated upload record status');
...
function cleanupMultipartUpload(req, uploadId, callback) {
    ...
    updateUploadRecordStatus(req, uploadId, 'completed', function (statusErr) {
        if (statusErr) {
            req.log.warn(statusErr,
               'S3_MPU: Failed to mark upload as completed');
