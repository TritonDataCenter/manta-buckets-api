...
function validatePartsForComplete(uploadRecord, partsFromXML, req, callback) {
    ...
    for (var i = 0; i < partsFromXML.length; i += VALIDATION_BATCH_SIZE) {
        validationBatches.push(partsFromXML.slice(i, i + VALIDATION_BATCH_SIZE));
    }
    ...
    vasync.forEachParallel({
        func: function validateBatch(batch, nextBatch) {
            ...
            req.log.debug({
                ...
                partsInBatch: batch.map(p => p.partNumber)
                ...
                if (!isLastPart && partMeta.content_length < minPartSize) {
                    ...
                    if (partsFromXML.length > 100 && validatedCount % progressInterval === 0) {
                        ...
                        req.log.info({
                            ...
                            batchesCompleted: Math.ceil(validatedCount / VALIDATION_BATCH_SIZE),
        ...
        var batchesPerSecond = Math.round((validationBatches.length / validationDuration) * 1000);
...
function assembleMultipartUpload(req, uploadRecord, commitBody, callback) {
    ...
    discoverPartPaths(req, uploadRecord, commitBody.parts,
        ...
        // Step 3: Use custom streaming assembly
