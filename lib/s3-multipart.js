        ...
        getUploadRecord(req, uploadId, function (getErr, uploadRecord) {
            ...
            validatePartsForComplete(uploadRecord, partsFromXML, req,
                ...
                if (validationErr) {
                    ...
                    }, 'S3_MPU_DEBUG: About to call next() with validation error');
                    ...
                    lockManager.releaseLock(lockInfo, function (releaseErr) {
                        if (releaseErr) {
                            req.log.warn(releaseErr, 
                                'S3_MPU: Failed to release lock during error cleanup');
                        } else {
                            req.log.debug({
                                uploadId: uploadId,
                                lockKey: lockInfo.lockKey
                            }, 'S3_MPU: Successfully released lock during error cleanup');
                        }
                        // Always continue with the original callback regardless of release result
                        callback(error);
                    });
                } else {
            ...
            lockManager.acquireLock(uploadId, function (lockErr, lockInfo) {
                ...
                if (sizeErr) {
                    ...
                    return safeCleanupAndExit(
                        new InternalError('Failed to calculate multipart upload size'),
                        completionLockInfo,
                        next
                    );
                }
                ...
                assembleMultipartUpload(req, uploadRecord, commitBody,
                    ...
                    if (assembleErr) {
                        ...
                        // on failure type - use next() to trigger S3 XML conversion
                        var errorToReturn;
                ...
                try {
                    ...
                    return safeCleanupAndExit(
                        new InternalError('Unexpected error during multipart upload completion'),
...
function assembleMultipartUpload(req, uploadRecord, commitBody, callback) {
    ...
    }, 'S3_MPU: Starting multipart assembly - will discover paths then try v2 commit');
