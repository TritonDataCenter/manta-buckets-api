...
function s3InitiateMultipartUploadHandler() {
    return function s3InitiateMultipartUpload(req, res, next) {
        ...
        function createDurabilityAndUploadRecord(validatedSharks) {
                ...
                // Cache sharks from storinfo call for consistent part storage
               ...
               req.log.debug (uploadRecordContent,
                              'S3 DEBUG MPU UPLOAD Record Content');
               ...
               req.log.debug({
                ...
                }, 'S3_MPU: HERE About to store upload record' +
                   ' with durability level');
        ...
        // Need sharks to store JSON content
        uploadReq.sharks = uploadRecord.preAllocatedSharks &&
                uploadRecord.preAllocatedSharks.length > 0 ?
            [uploadRecord.preAllocatedSharks[0]] : [];
        ...
        bucketHelpers.loadRequest(uploadReq, null, function (loadErr) {
            ...
            bucketHelpers.getBucketIfExists(uploadReq, null,
                ...
                // Create upload record object with headers containing the key data
                var uploadHeaders = {
                    'x-upload-record': uploadRecordContent,
                    'x-durability-level': copies.toString(),
                    'x-upload-id': uploadId,
                    'x-preallocated-sharks': JSON.stringify(uploadRecord.preAllocatedSharks || [])
                };
                
                client.createObject(owner, uploadReq.bucket.id, uploadRecordKey,
                    uploadReq.objectId, uploadRecordContent.length,
                                    uploadRecordMD5,
                    'application/json', uploadHeaders, [], {}, metadataLocation.vnode, {},
        ...
        bucketHelpers.getBucketIfExists(uploadReq, null, function (bucketErr) {
            ...
            client.getObject(owner, uploadReq.bucket.id, uploadRecordKey,
                ...
                req.log.debug({
                    ...
                    resultKeys: result ? Object.keys(result) : [],
                    rawValueContent: result && result.value ? result.value : 'NO-VALUE',
                ...
                try {
                    req.log.debug({
                        ...
                        headerUploadRecord: result.headers ? result.headers['x-upload-record'] : 'no-header'
                    ...
                    req.log.debug({result: result}, 'S3_MPU: RESULT') ;
                    if (result.value) {
                        uploadRecord = JSON.parse(result.value);
                        dataSource = 'content';
                    } else if (result.headers && result.headers['x-upload-record']) {
                        uploadRecord = JSON.parse(result.headers['x-upload-record']);
                        dataSource = 'header-json';
                    } else if (result.headers && result.headers['x-durability-level']) {
                        // Reconstruct from individual headers
                        uploadRecord = {
                            durabilityLevel: parseInt(result.headers['x-durability-level'], 10),
                            uploadId: result.headers['x-upload-id'],
                            preAllocatedSharks: result.headers['x-preallocated-sharks'] ? 
                                JSON.parse(result.headers['x-preallocated-sharks']) : []
                        };
                    ...
                    req.log.debug({
                        ...
                        hasPreAllocatedSharks: !!uploadRecord.preAllocatedSharks,
...
function streamAndAssembleParts(req, partPaths, callback) {
    ...
    // Get durability level and sharks from upload record
    var uploadId = req.s3Request.uploadId;

    getUploadRecord(req, uploadId, function (getErr, uploadRecord) {
        if (getErr) {
            req.log.error(getErr, 'S3_MPU: Failed to get upload record for streaming assembly');
            return (next(new NoSuchUploadError(uploadId)));
        }

        var copies = uploadRecord.durabilityLevel || common.DEF_NUM_COPIES;
        var preAllocatedSharks = uploadRecord.preAllocatedSharks;

        req.log.debug({
            uploadId: uploadId,
            durabilityLevel: copies,
            hasPreAllocatedSharks: !!preAllocatedSharks,
            preAllocatedSharkCount: preAllocatedSharks ? preAllocatedSharks.length : 0,
            source: 'upload-record'
        }, 'S3_MPU: Using durability level and sharks from upload record for streaming assembly');

        // Use pre-allocated sharks from upload record instead of re-allocating
        if (preAllocatedSharks && preAllocatedSharks.length > 0) {
            req.log.debug({
                uploadId: uploadId,
                sharkCount: preAllocatedSharks.length,
                sharks: preAllocatedSharks.map(function (s) { 
                    return s.manta_storage_id; 
                })
            }, 'S3_MPU: Using pre-allocated sharks for streaming assembly');
            
            // Store the pre-allocated sharks for metadata creation
            req._finalSharks = preAllocatedSharks;

            // Stream parts directly to the pre-allocated sharks using the
            // final object ID
            streamPartsToFinalSharks(req, partPaths, preAllocatedSharks,
                                     req._finalObjectId,
                ...
                assemblyResult.sharks = preAllocatedSharks;
                callback(null, assemblyResult);
            });
        } else {
            req.log.warn({
                uploadId: uploadId
            }, 'S3_MPU: No pre-allocated sharks found, falling back to storinfo.choose()');
...
function cleanupMultipartUpload(req, uploadId, callback) {
    ...
    // Skip part record cleanup since we use independent part objects
    // Continue with upload record cleanup
...
function storeDurabilityObject(req, durabilityKey, durabilityData, callback) {
    ...
    // Debug: Check if sharks survive JSON serialization
    req.log.debug({
        durabilityKey: durabilityKey,
        originalSharksCount: durabilityData.sharks ?
            durabilityData.sharks.length : 0,
        serializedSharks: durabilityData.sharks ?
            JSON.stringify(durabilityData.sharks).substring(0, 500) : 'null',
        contentLength: content.length
    }, 'S3_MPU: DEBUG - Shark serialization check');
                    ...
                    try {
                        ...
                        req.log.debug({
                            ...
                            rawContent: result.value ?
                                result.value.substring(0, 500) : 'null',
                            contentDataKeys: Object.keys(contentData),
                            hasSharksProperty:
                            contentData.hasOwnProperty('sharks'),
                            sharksValue: contentData.sharks,
                            sharksType: typeof (contentData.sharks),
                            isArray: Array.isArray(contentData.sharks)
                        }, 'S3_MPU: DEBUG - Parsing durability object content');
                        ...
                        if (contentData.sharks &&
                            ...
                            req.log.warn({
                                durabilityKey: durabilityKey,
                                hasSharks: !!contentData.sharks,
                                sharksType: typeof (contentData.sharks),
                                isArray: Array.isArray(contentData.sharks),
                                contentDataKeys: Object.keys(contentData)
                            }, 'S3_MPU:' +
                               ' No sharks found in durability object content');
                        }
                    } catch (parseErr) {
                        req.log.warn({
                            durabilityKey: durabilityKey,
