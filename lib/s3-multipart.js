        ...
        getUploadRecord(req, uploadId, function (getErr, uploadRecord) {
            ...
            if (partReq.headers['content-encoding'] === 'aws-chunked') {
                req.log.debug({
                    ...
                    decodedLength: partReq.headers['x-amz-decoded-content-length']
                ...
                var decodedLength = partReq.headers['x-amz-decoded-content-length'];
                if (decodedLength) {
    ...
    vasync.forEachParallel({
            ...
            client.put(putOpts, function (putErr, sharkReq) {
                ...
                sharkReq.once('response', function (sharkRes) {
                    ...
                    req.log.info({
                        ...
                        objectName: objectName,
                        storageLayoutVersion: common.CURRENT_STORAGE_LAYOUT_VERSION
                    }, 'S3_MPU: Final shark confirmed successful storage - object should be downloadable');
        ...
        barrier.once('drain', function () {
            if (!streamingComplete) {
                req.log.error({
                    uploadId: req._uploadRecord ? req._uploadRecord.uploadId : 'unknown',
                    objectName: objectName,
                    completedSharks: completedSharks.length,
                    expectedSharks: finalSharks.length
                }, 'S3_MPU: CRITICAL ERROR - Sharks completed before streaming finished');
                return callback(new Error('Multipart assembly failed - streaming incomplete'));
            }
            ...
            if (completedSharks.length !== finalSharks.length) {
                ...
                }, 'S3_MPU: CRITICAL ERROR - Not all sharks completed successfully');
                return callback(new Error('Multipart assembly failed - incomplete shark storage'));
            }
            ...
            req.log.info({
                ...
                uploadId: req._uploadRecord ? req._uploadRecord.uploadId : 'unknown',
                objectName: objectName
            }, 'S3_MPU: All sharks completed successfully - assembly validated');
