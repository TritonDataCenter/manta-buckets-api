...
function assembleMultipartUpload(req, uploadRecord, commitBody, callback) {
    ...
    }, 'S3_MPU: Assembling multipart upload with real manta-mako integration');
    
    // Step 1: Discover real shark paths for all parts
    discoverPartPaths(req, uploadRecord, commitBody.parts, function(err, partPaths) {
        if (err) {
        ...
        }, 'S3_MPU: Discovered part paths for manta-mako');
        
        // Step 2: Calculate real total size and update commit body with real shark paths
        var realTotalSize = partPaths.reduce(function(sum, partPath) {
        ...
        req.log.debug({
            ...
            sizeDifference: realTotalSize - commitBody.nbytes
        }, 'S3_MPU: Real vs estimated size comparison');
        
        var realCommitBody = {
            version: 1,
            nbytes: realTotalSize, // Use real calculated size
            account: commitBody.account,
            objectId: commitBody.objectId,
            parts: partPaths.map(function(partPath) {
                // Try sending the full storage path since manta-mako is looking in wrong place
        ...
        req.log.debug({
            ...
            firstPartPath: partPaths[0] ? partPaths[0].sharkPath : 'none'
        }, 'S3_MPU: Starting real part assembly (bypassing manta-mako)');
        
        // Step 3: Use custom assembly since manta-mako is incompatible with buckets-api storage
        customAssembleMultipartUpload(req, uploadRecord, partPaths, realCommitBody, callback);
    });
}

function customAssembleMultipartUpload(req, uploadRecord, partPaths, commitBody, callback) {
    req.log.debug({
        partCount: partPaths.length,
        totalSize: commitBody.nbytes
    }, 'S3_MPU: Starting real multipart assembly through buckets-mdapi');
    ...
    streamAndAssembleParts(req, partPaths, function(streamErr, assemblyData) {
        if (streamErr) {
            req.log.error(streamErr, 'S3_MPU: Failed to stream and assemble parts');
        ...
        // Use the pre-allocated sharks to ensure metadata matches storage location
        createFinalObjectMetadata(req, uploadRecord, commitBody, assemblyData, req._finalSharks, function(metaErr, objectResult) {
            if (metaErr) {
                req.log.error(metaErr, 'S3_MPU: Failed to create final object metadata');
...
function streamAndAssembleParts(req, partPaths, callback) {
    req.log.debug({
        ...
        approxTotalSize: partPaths.reduce(function(sum, p) { return sum + (p.size || 0); }, 0),
        finalObjectId: req._finalObjectId
    }, 'S3_MPU: Starting streaming assembly (memory-efficient)');
    
    // Use normal object creation flow to ensure proper shark allocation and storage
    ...
    var opts = {
        replicas: req._copies || 2,
        requestId: req.getId(),
        size: partPaths.reduce(function(sum, p) { return sum + (p.size || 0); }, 0),
        isOperator: req.caller.account.isOperator
    };
    
    req.storinfo.choose(opts, function(sharkErr, sharks) {
        if (sharkErr) {
            req.log.error(sharkErr, 'S3_MPU: Failed to allocate sharks for streaming assembly');
        ...
        req.log.debug({
            allocatedSharks: flatSharks.map(function(s) { return s.manta_storage_id; }),
            finalObjectId: req._finalObjectId
        ...
        // Stream parts directly to the allocated sharks using the final object ID
        streamPartsToFinalSharks(req, partPaths, flatSharks, req._finalObjectId, function(streamErr, assemblyResult) {
            if (streamErr) {
                return callback(streamErr);
            }
...
function streamPartsToFinalSharks(req, partPaths, finalSharks, finalObjectId, callback) {
    ...
    var objectId = finalObjectId; // Use the provided final object ID consistently
    var objectNameHash = crypto.createHash('md5').update(objectName).digest('hex');
    var totalBytes = 0;
    var md5Hash = crypto.createHash('md5');
    
    req.log.debug({
        finalSharks: finalSharks.map(function(s) { return s.manta_storage_id; }),
        partCount: partPaths.length
    ...
    vasync.forEachParallel({
        func: function setupSharkStream(shark, next) {
            var client = sharkClient.getClient({
                connectTimeout: (req.sharkConfig && req.sharkConfig.connectTimeout) || 5000,
                log: req.log,
            ...
            client.put(putOpts, function(putErr, sharkReq) {
                ...
                sharkReq.once('response', function(sharkRes) {
                    if (sharkRes.statusCode >= 400) {
                        var err = new Error('Final shark storage failed with status ' + sharkRes.statusCode);
                        req.log.error({
                            err: err,
                            shark: shark.manta_storage_id,
                ...
                // Call next() immediately for setup, completion handled by barrier
                next();
            });
        },
        inputs: finalSharks
        ...
        }, 'S3_MPU: Established streams to all final sharks, starting part streaming');
        
        // Set up barrier completion handler
        barrier.once('drain', function() {
            if (!streamingComplete) {
                req.log.warn('S3_MPU: Sharks completed before streaming finished');
                return;
            }
        ...
        vasync.forEachPipeline({
            func: function streamPartToSharks(partPath, nextPart) {
                ...
                streamPartFromSharks(req, partPath, function(partErr, partBuffer) {
                    if (partErr) {
                        req.log.error({
                            err: partErr,
                            partNumber: partPath.partNumber
            ...
            if (streamErr) {
                req.log.error(streamErr, 'S3_MPU: Failed during part streaming to final sharks');
                return callback(streamErr);
            }
            ...
            }, 'S3_MPU: Finished streaming all parts, marked streaming complete');
        });
    });
}
...
function streamPartFromSharks(req, partPath, callback) {
    ...
    if (!sharks || sharks.length === 0) {
        return callback(new Error('No sharks available for part ' + partPath.partNumber));
    }
    
    req.log.debug({
        partNumber: partPath.partNumber,
        availableSharks: sharks.map(function(s) { return s.manta_storage_id; }),
    ...
    function retryPartStream() {
        if (currentRetry >= maxPartRetries) {
            return callback(new Error('Failed to stream part ' + partPath.partNumber + ' after ' + maxPartRetries + ' retries'));
        }
        ...
        tryStreamFromShark(req, partPath, sharks, 0, function(tryErr, partBuffer) {
            if (tryErr) {
                req.log.warn({
                    err: tryErr,
                    partNumber: partPath.partNumber,
...
function tryStreamFromShark(req, partPath, sharks, sharkIndex, callback) {
    if (sharkIndex >= sharks.length) {
        req.log.error({
            partNumber: partPath.partNumber,
            sharksAttempted: sharks.map(function(s) { return s.manta_storage_id; })
        }, 'S3_MPU: Exhausted all sharks for part');
        return callback(new Error('Failed to stream part ' + partPath.partNumber + ' from all available sharks'));
    }
    ...
    // Create a custom shark client with extended timeout specifically for multipart assembly
    // We need to create a new client instance to avoid cache conflicts with different timeouts
    var extendedTimeout = 30000; // 30 seconds for multipart part retrieval
    var sharkOptions = {
        connectTimeout: extendedTimeout,
        log: req.log.child({component: 'multipart-shark-client'}),
        retry: {
            retries: 2,
            minTimeout: 1000,
            maxTimeout: 5000
    ...
    var httpReq = http.request(httpOptions, function(res) {
        ...
        if (res.statusCode >= 400) {
            ...
            // Try next shark
            return tryStreamFromShark(req, partPath, sharks, sharkIndex + 1, callback);
        }
...
function storeAssembledObject(req, uploadRecord, commitBody, assemblyData, callback) {
    req.log.debug({
        objectId: commitBody.objectId,
        assembledSize: assemblyData.totalBytes,
    ...
    req.storinfo.choose(opts, function(sharkErr, sharks) {
        if (sharkErr) {
            req.log.error(sharkErr, 'S3_MPU: Failed to allocate sharks for final object');
            return callback(sharkErr);
        }
        ...
        storeDataToSharks(req, uploadRecord, finalObjectId, finalBuffer, flatSharks, function(storeErr, storedSharks) {
            if (storeErr) {
                req.log.error(storeErr, 'S3_MPU: Failed to store assembled data to sharks');
                return callback(storeErr);
            }
            
            req.log.debug({
                storedSharks: storedSharks.map(function(s) { return s.manta_storage_id; })
            }, 'S3_MPU: Successfully stored assembled data to sharks');
            
            // Step 3: Create final object metadata
            createFinalObjectMetadata(req, uploadRecord, commitBody, assemblyData, storedSharks, function(metaErr, objectResult) {
                if (metaErr) {
                    req.log.error(metaErr, 'S3_MPU: Failed to create final object metadata');
                    return callback(metaErr);
                }
...
function storeDataToSharks(req, uploadRecord, objectId, dataBuffer, sharks, callback) {
    var owner = req.owner.account.uuid;
    ...
    vasync.forEachParallel({
        func: function storeToShark(shark, next) {
            ...
            client.put(putOpts, function(putErr, sharkReq) {
                ...
                sharkReq.once('response', function(sharkRes) {
                    if (sharkRes.statusCode >= 400) {
                        var err = new Error('Shark storage failed with status ' + sharkRes.statusCode);
                        req.log.error({
                            err: err,
                            shark: shark.manta_storage_id,
                            statusCode: sharkRes.statusCode
...
function createFinalObjectMetadata(req, uploadRecord, commitBody, assemblyData, preAllocatedSharks, callback) {
    var owner = req.owner.account.uuid;
    var bucketId = uploadRecord.bucketId;
    var objectName = req.s3Request.object;
    ...
    req.log.debug({
        ...
        preAllocatedSharks: preAllocatedSharks.map(function(s) { return s.manta_storage_id; })
    ...
    var headers = {
        ...
        'content-length': String(assemblyData.totalBytes), // Headers must be strings
    ...
    // Format pre-allocated sharks as array of objects (sequence) for RPC protocol
    // This ensures metadata points to the same sharks where data was actually stored
...
function discoverPartPaths(req, uploadRecord, partETags, callback) {
    var owner = req.owner.account.uuid;
    var bucketId = uploadRecord.bucketId;
    var uploadId = req.s3Request.uploadId; // Get upload ID from the request, not the record!
    var partPaths = [];
    ...
    req.log.debug({
        ...
        partETags: partETags,
        bucketId: bucketId
    }, 'S3_MPU: Discovering real shark paths for parts - debugging upload record');
    ...
    vasync.forEachParallel({
        func: function queryPartObject(partETag, next) {
            ...
            bucketHelpers.loadRequest(partReq, null, function(loadErr) {
                if (loadErr) {
                    req.log.error(loadErr, 'S3_MPU: Failed to load request for part object');
                    return next(loadErr);
                }
                
                bucketHelpers.getBucketIfExists(partReq, null, function(bucketErr) {
                    if (bucketErr) {
                        req.log.error(bucketErr, 'S3_MPU: Bucket not found for part object');
                    ...
                    client.getObject(owner, bucketId, partObjectName,
                        metadataLocation.vnode, {}, req.getId(), function(getErr, partMeta) {
                        
                        if (getErr) {
                            req.log.error(getErr, 'S3_MPU: Failed to get part object metadata');
                            return next(getErr);
                        }
