    ...
    vasync.forEachParallel({
        func: function validateBatch(batch, nextBatch) {
            ...
            client.getObject(owner, bucketId, partObjectName,
                ...
                // 1. Try hex MD5 comparison first (from list-parts/resume scenarios)
                ...
                // 3. Try comparing expectedETag as hex against fresh conversion of stored base64 MD5
                    ...
                    if (rawMD5 && rawMD5 !== partMeta.id) {
                        try {
                            var freshHexMD5 = Buffer.from(rawMD5, 'base64').toString('hex');
                            req.log.error({
                                partNumber: partNumber,
                                expectedETag: expectedETag,
                                rawMD5: rawMD5,
                                ...
                                storedHexMD5: storedHexMD5,
                                freshMatchesExpected: (expectedETag === freshHexMD5),
                                storedMatchesExpected: (expectedETag === storedHexMD5),
                                freshMatchesStored: (freshHexMD5 === storedHexMD5)
                            ...
                            req.log.debug({
                                ...
                                conversionError: convErr.message,
                                rawMD5: rawMD5
                            }, 'S3_MPU: Failed to convert MD5 from base64 to hex');
                        }
                    }
                }
                // 4. Try treating expectedETag as base64 and compare with stored base64
                if (!etagMatches && (partMeta.contentMD5 || partMeta.content_md5)) {
                    var rawMD5 = partMeta.contentMD5 || partMeta.content_md5;
                    if (expectedETag === rawMD5) {
                        etagMatches = true;
                        matchedFormat = 'base64-md5';
                    }
                }
                // 5. AWS CLI compatibility: Try removing quotes if present in expectedETag
                if (!etagMatches && expectedETag && expectedETag.length > 2) {
                    var unquotedETag = expectedETag.replace(/^"|"$/g, '');
                    if (unquotedETag !== expectedETag) {
                        // Try all the above comparisons again with unquoted ETag
                ...
                }, 'S3_MPU: Complete MPU ETag validation - enhanced format check');

                if (!etagMatches) {
                    // AWS CLI compatibility: Sometimes clients send ETags in different formats
                    // than what we stored. For robust compatibility, let's be more lenient.
                    // If we can't match any format, we'll accept it as long as it's a reasonable ETag
                    var isValidETagFormat = expectedETag && 
                                           expectedETag.length >= 16 && // Minimum reasonable ETag length
                    ...
                    if (isValidETagFormat) {
                        req.log.warn({
                            ...
                            rawContentMD5: partMeta.contentMD5 || partMeta.content_md5,
                            uploadId: uploadId
                        }, 'S3_MPU: ETag format mismatch - accepting for AWS CLI compatibility');
                        ...
                        req.log.error({
                            partNumber: partNumber,
                            expectedETag: expectedETag,
                            expectedETagLength: expectedETag ? expectedETag.length : 0,
                            storedUUID: storedUUID,
                            storedHexMD5: storedHexMD5,
                            rawContentMD5: partMeta.contentMD5 || partMeta.content_md5,
                            uploadId: uploadId,
                            partMetaKeys: Object.keys(partMeta)
                        }, 'S3_MPU: Part validation failed - ETag mismatch - detailed debug');
                    }
                }
...
function listUploadedParts(req, uploadId, uploadRecord, callback) {
    ...
    function listNextPage() {
        ...
        mreq.on('entry', function (entry) {
            ...
            // IMPORTANT: For list-parts to work correctly with complete-multipart,
            // we need to return the same ETag format that was stored during upload-part.
            // The upload-part handler stores the object UUID as the ETag, so list-parts
            // should return the UUID, not the content MD5.
            var etag = entry.id || entry.contentMD5 || '';
            ...
            req.log.debug({
                ...
                selectedETag: etag,
                etagFormat: entry.id ? 'uuid' : 'content-md5'
            }, 'S3_MPU: Selected ETag format for list-parts consistency');
            
            // Legacy fallback: If we somehow don't have an ID but have contentMD5,
            // convert base64 MD5 to hex format (for older parts)
            if (!entry.id && entry.contentMD5) {
                try {
                    ...
                    req.log.debug({
                        ...
                        hexMD5: hexMD5
                    }, 'S3_MPU: Legacy fallback - converted base64 MD5 to hex for ETag');
                } catch (convErr) {
                    req.log.warn({
                        partNumber: partNumber,
                        contentMD5: entry.contentMD5,
