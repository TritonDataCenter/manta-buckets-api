...
function s3InitiateMultipartUploadHandler() {
    return function s3InitiateMultipartUpload(req, res, next) {
        ...
        // Pre-allocate sharks for all parts in this MPU
        // This ensures all parts go to the same set of sharks
        req._copies = copies; // Set for shark picker
        req._size = 0; // Set default size for shark selection
        
        // Use common.findSharks directly with callback wrapper
        var commonModule = require('./common');
        
        // Call findSharks directly and capture the sharks
        var originalNext = function (err) {
            if (err) {
                req.log.error(err, 'S3_MPU: Failed to allocate sharks for MPU');
                return next(err);
            }
            // At this point req._sharks should be populated
            var sharks = req._sharks;
            
            req.log.debug({
                uploadId: uploadId,
                allocatedSharks: sharks.map(function (s) { return s.manta_storage_id; }),
                durabilityLevel: copies
            }, 'S3_MPU: Pre-allocated sharks for all parts in this MPU');

            // Store both durability level and pre-allocated sharks
            var durabilityKey = '.mpu-uploads/' + uploadId + '.durability';
            var durabilityData = {
                uploadId: uploadId,
                durabilityLevel: copies,
                sharks: sharks, // Store pre-allocated sharks
                created: new Date().toISOString()
            };

            continueInitiation();

            function continueInitiation() {
        ...
        bucketHelpers.loadRequest(uploadReq, null, function (loadErr) {
            ...
            } // Close continueInitiation function
        }; // Close originalNext callback
        
        // Call findSharks with the callback
        commonModule.findSharks(req, null, originalNext);
    };
}
...
function s3UploadPartHandler() {
    return function s3UploadPart(req, res, next) {
        ...
        getUploadRecord(req, uploadId, function (getErr, uploadRecord) {
            ...
            // Get durability level and pre-allocated sharks from stored metadata
            var durabilityKey = '.mpu-uploads/' + uploadId + '.durability';
            getDurabilityObject(req, durabilityKey,
                                function (durErr, durabilityData) {
                if (!durErr && durabilityData) {
                    // Set durability level from metadata
                    if (durabilityData.durabilityLevel) {
                        ...
                        }, 'S3_MPU: Set durability-level header from durability object');
                    }
                    
                    // Use pre-allocated sharks if available
                    if (durabilityData.sharks && Array.isArray(durabilityData.sharks)) {
                        partReq.preAllocatedSharks = durabilityData.sharks;
                        req.log.debug({
                            uploadId: uploadId,
                            partNumber: partNumber,
                            sharkCount: durabilityData.sharks.length,
                            sharks: durabilityData.sharks.map(function (s) { 
                                return s.manta_storage_id; 
                            })
                        }, 'S3_MPU: Using pre-allocated sharks for part upload');
                    }
                } else {
                    req.log.warn({
                        uploadId: uploadId,
                        partNumber: partNumber,
                        error: durErr
                    }, 'S3_MPU: Could not retrieve durability data, using default allocation');
                }
    ...
    return function s3CompleteMultipartUpload(req, res, next) {
        ...
        getUploadRecord(req, uploadId, function (getErr, uploadRecord) {
            ...
            lockManager.acquireLock(uploadId, function (lockErr, lockInfo) {
                        ...
                        lockManager.releaseLock(completionLockInfo,
                            ...
                            if (releaseErr) {
                                req.log.warn(releaseErr,
                                    'S3_MPU: Failed to release lock on error');
                            }
                        });
                    }
                ...
                // PERFORMANCE OPTIMIZATION: Return success immediately to
                // prevent client timeout.
                // All parts are validated, so assembly will succeed
                ...
                req.log.info({
                   ...
                   ' - assembly will continue in background');
                ...
                assembleMultipartUpload(req, uploadRecord, commitBody,
                    ...
                    if (assembleErr) {
                        ...
                        }, 'S3_MPU: BACKGROUND ASSEMBLY FAILED' +
                           ' - Client already received success');

                        // Note: Client already got success response,
                        // so just log the failure.
                        // FIXME: consider implementing retry logic or
                        // alerting here
                        return;
                    }
...
function assembleMultipartUpload(req, uploadRecord, commitBody, callback) {
    ...
    discoverPartPaths(req, uploadRecord, commitBody.parts,
        ...
        // Step 3: Try native v2 commit first, fall back to streaming
