...
function s3CompleteMultipartUploadHandler() {
    return function s3CompleteMultipartUpload(req, res, next) {
        ...
        getUploadRecord(req, uploadId, function (getErr, uploadRecord) {
            ...
            // TEMPORARILY DISABLED: Distributed locking to test regression
            req.log.debug({
                uploadId: uploadId
            }, 'S3_MPU: DISTRIBUTED LOCKING TEMPORARILY DISABLED FOR TESTING');

            // Skip distributed locking and proceed directly to assembly
            (function skipLockingAndProceed() {
                ...
                if (sizeErr) {
                    ...
                    // TEMPORARILY DISABLED: Lock release on error (no locking active)
                ...
                // PERFORMANCE OPTIMIZATION: Return success immediately to prevent client timeout
                // All parts are validated, so assembly will succeed (or fail gracefully in background)
                ...
                req.log.info({
                    ...
                    responseStrategy: 'immediate-async-assembly'
                }, 'S3_MPU: Returning immediate success - assembly will continue in background');

                res.setHeader('Content-Type', 'application/xml');
                res.send(200, xml);
                next(false);

                // Continue assembly asynchronously in background
                // Client has already received success response
                req.log.debug({
                    uploadId: uploadId,
                    backgroundProcess: true
                }, 'S3_MPU: Starting background assembly process');
                ...
                assembleMultipartUpload(req, uploadRecord, commitBody,
                    ...
                    if (assembleErr) {
                        req.log.error({
                            err: assembleErr,
                            uploadId: uploadId,
                            backgroundAssembly: true
                        }, 'S3_MPU: BACKGROUND ASSEMBLY FAILED - Client already received success');
                        
                        // Note: Client already got success response, so just log the failure
                    ...
                    req.log.info({
                        ...
                        uploadId: uploadId,
                        backgroundAssembly: true
                    }, 'S3_MPU: BACKGROUND ASSEMBLY COMPLETED SUCCESSFULLY');
                    ...
                    cleanupMultipartUpload(req, uploadId, function (cleanupErr) {
                        if (cleanupErr) {
                            ...
                            }, 'S3_MPU: Background cleanup failed - upload record may remain');
                        } else {
                            req.log.debug({
                                uploadId: uploadId,
                                backgroundCleanup: true
                            }, 'S3_MPU: Background cleanup completed successfully');
                        }
                    });
                }); // Close assembleMultipartUpload callback
            }); // Close calculateActualTotalSize function call
            })(); // Close skipLockingAndProceed function
...
function DistributedLockManager(req) {
    ...
    this.retryInterval = 500; // 500ms between acquisition attempts for faster response  
                ...
                client.getObject(owner, lockReq.bucket.id, lockKey,
                    ...
                    if (!getErr && existingLock) {
                        ...
                        try {
                            ...
                            // Validate the parsed date to prevent race conditions
                            ...
                            if (!expiresValue || isNaN(expires.getTime())) {
                                ...
                                expires = new Date(0); // Set to epoch, making it expired
                            }
                            
                            // CRITICAL: If we had parsing errors and another instance owns the lock,
                            // be conservative and wait rather than immediately claiming
                            if (hadParsingError && existingData.instanceId !== instanceId) {
                                self.req.log.warn({
                                    uploadId: uploadId,
                                    currentOwner: existingData.instanceId,
                                    retryCount: currentRetry
                                }, 'S3_MPU_LOCK: Cannot parse expiration for lock owned by another instance, will retry');
            ...
            client.getObject(owner, lockReq.bucket.id, lockKey,
                ...
                try {
                    ...
                    self.req.log.warn({
                        err: parseErr,
                        lockKey: lockKey
                    }, 'S3_MPU_LOCK: Failed to parse lock data during release' +
                       ', proceeding anyway');
                }
            ...
            vasync.forEachParallel({
                ...
                func: function getPartSize(partETag, next) {
                    ...
                    var timeoutId = setTimeout(function () {
                        ...
                        }, 'S3_MPU: getObject timeout for part size calculation');
                        next(new Error('Timeout getting part size for part ' + partNumber));
...
function validatePartsForComplete(uploadRecord, partsFromXML, req, callback) {
    ...
    // Calculate optimal concurrency for large uploads
    var optimalConcurrency = Math.min(50, Math.max(10, Math.floor(partsFromXML.length / 10)));
    var validatedCount = 0;
    var progressInterval = Math.ceil(partsFromXML.length / 10); // Report every 10%
    ...
    req.log.debug({
        ...
        estimatedValidationTime: Math.ceil(partsFromXML.length / optimalConcurrency) + ' batches',
        progressReportingInterval: progressInterval
    }, 'S3_MPU: Starting optimized part validation with progress tracking');
    ...
    vasync.forEachParallel({
        func: function validatePart(xmlPart, next) {
            ...
            client.getObject(owner, bucketId, partObjectName,
                ...
                if (partsFromXML.length > 100 && validatedCount % progressInterval === 0) {
                    var progressPct = Math.round((validatedCount / partsFromXML.length) * 100);
                    req.log.info({
                        validatedParts: validatedCount,
                        totalParts: partsFromXML.length,
                        progressPercent: progressPct,
                        uploadId: uploadId
                    }, 'S3_MPU: Part validation progress');
                }
        ...
        inputs: partsFromXML,
        // Use calculated optimal concurrency for large uploads
        concurrency: optimalConcurrency
    }, function (err) {
        if (err) {
            req.log.error({
        ...
        var partsPerSecond = Math.round((partsFromXML.length / validationDuration) * 1000);
        ...
        req.log.info({
            ...
            concurrencyUsed: optimalConcurrency,
            totalSize: partsFromXML.reduce(function (sum, p) {
                return (sum + (p.size || 0));
            }, 0)
        }, 'S3_MPU: All parts validated successfully with performance metrics');
...
function streamAndAssembleParts(req, partPaths, callback) {
    ...
    // Calculate dynamic timeout based on file size (1 minute per GB + 5 min base)
    var estimatedAssemblyTimeMs = Math.max(300000, // 5 min minimum
        (approxTotalSize / (1024 * 1024 * 1024)) * 60000 + 300000); // 1 min/GB + 5 min base
