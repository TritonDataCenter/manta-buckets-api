...
function s3DeleteBucketObjectsHandler() {
    return function s3DeleteBucketObjects(req, res, next) {
        ...
        // Extract object keys from matches
        var objectKeys = keyMatches.map(function(match) {
            return match.replace(/<\/?Key>/g, '');
        });
        ...
        objectKeys.forEach(function(objectKey) {
            ...
            deleteReq.url = '/' + bucketName + '/' + objectKey;
                ...
                bucketHelpers.getBucketIfExists(deleteReq, null, function(bucketErr) {
                    ...
                    deleteObjectFunc(deleteReq, null, function(delErr, result) {
                        ...
                        if (delErr) {
                            ...
                            errors.push({
                                Key: objectKey,
                                Code: delErr.restCode || 'InternalError',
                                Message: delErr.message || 'Failed to delete object'
                            });
                        } else {
                            req.log.debug({
                                objectKey: objectKey
