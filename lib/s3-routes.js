...
// Import S3 CORS handlers
var bucketCors = require('./buckets/cors');

// Import anonymous auth for SYSTEM_ROLES constant
var anonymousAuth = require('./anonymous-auth');

// Import object operation modules
var copyObjectModule = require('./buckets/objects/copy');
var deleteObjectModule = require('./buckets/objects/delete');
...
function executeMiddlewareChain(chain, req, res, next) {
    ...
    function runNext(err) {
        if (err) {
            ...
            var isUserError = common.isClientError(err);
            if (isUserError) {
                req.log.debug({
                    errorName: err.name,
...
function s3ListBucketsHandler() {
    return function s3ListBuckets(req, res, next) {
        ...
        if (!req.caller || !req.caller.account) {
            ...
            err.statusCode = constants.HTTP_STATUS.UNAUTHORIZED;
            next(err);
            return;
        }
        ...
        res.write = function (chunk) {
            if (chunk) {
                try {
                    ...
                    }, 'ListBuckets: failed to parse bucket entry ' +
                        'from Manta — response may be incomplete');
                }
            }
        };
        ...
        res.end = function () {
            ...
            /*
             * If any bucket chunk failed to parse, return an error
             * rather than a silent partial list.  A partial listing
             * is worse than an error: the caller cannot distinguish
             * "no access" from "missing due to a bug", and a scoped
             * key could silently omit buckets it should see.
             */
            if (parseErrors > 0) {
                req.log.error({
                    parseErrors: parseErrors,
                    parsedCount: bucketData.length
                }, 'ListBuckets: aborting due to Manta response ' +
                    'parse failures');
                next(new errors.InternalError(
                    'ListBuckets: failed to read bucket list'));
                return;
            }
            ...
            // Filter buckets by access key scope if present
            if (req.scopeContext && req.scopeContext.patterns) {
                var patterns = req.scopeContext.patterns;
                bucketData = bucketData.filter(function (b) {
                    for (var fi = 0; fi < patterns.length; fi++) {
                        if (
                            bucketScopeLib.matchBucketPattern(
                                patterns[fi], b.name)) {
                            return (true);
                        }
                    }
            ...
            // empty <Buckets/>
            res.send(constants.HTTP_STATUS.OK, bucketData);
        };
        ...
        executeMiddlewareChain(mantaHandlerChain, req, res,
                               function onListBucketsComplete(error) {
...
function s3CreateBucketHandler() {
    return function s3CreateBucket(req, res, next) {
        ...
        // Most S3 clients just accept a 2XX HTTP response
        // and carry on, minio mc also takes a 2XX response
        // code but complains, that could alert users that
        // something is not working when there is no problem
        // at all. Following the HTTP spec 204 is ok, but AWS
        // just returns a 200 HTTP status code.
        var orig_res_send = res.send;
        res.send = function patchs3CreateBucketSend(code, body) {
            switch (code) {
                case 204:
                    // AWS S3 returns 200 with an empty body for
                    // CreateBucket. Sending the Manta JSON body
                    // confuses the AWS CLI which tries to parse it.
                    res.writeHead(200);
                    res.end();
                    return;
                default:
                    return (orig_res_send.call(this, code, body));
            }
        };
        ...
        executeMiddlewareChain(mantaHandlerChain, req, res,
                               function onCreateBucketComplete(error) {
...
function s3ListBucketObjectsHandler() {
    return function s3ListBucketObjects(req, res, next) {
        ...
        if (handleBucketLocation(req, res, next)) {
            return;
        }
        ...
        executeMiddlewareChain(mantaHandlerChain, req, res,
                               function onListBucketObjectsComplete(error) {
            // Always terminate the route chain for S3 requests to
            // prevent double execution
            if (error) {
                next(error);
...
function s3ListBucketObjectsV2Handler() {
    return function s3ListBucketObjectsV2(req, res, next) {
        ...
        var mantaHandlerChain = buckets.listBucketObjectsHandler();
        executeMiddlewareChain(mantaHandlerChain, req, res,
                               function onListBucketObjectsV2Complete(error) {
...
function s3HeadBucketHandler() {
    return function s3HeadBucket(req, res, next) {
        ...
        executeMiddlewareChain(mantaHandlerChain, req, res,
                               function onHeadBucketComplete(error) {
            // Always terminate the route chain for S3 requests
            // to prevent double execution
            if (error) {
                next(error);
...
function s3DeleteBucketHandler() {
    return function s3DeleteBucket(req, res, next) {
        ...
        executeMiddlewareChain(mantaHandlerChain, req, res,
                               function onDeleteBucketComplete(error) {
            // Always terminate the route chain for S3 requests
            // to prevent double execution
            if (error) {
                next(error);
...
function s3CreateBucketObjectHandler() {
    return function s3CreateBucketObject(req, res, next) {
        ...
        s3Compat.s3RoleTranslator(req, res,
            function onPutObjectRoleTranslated(translationErr) {
            if (translationErr) {
                next(translationErr);
                return;
            }
            ...
            executeMiddlewareChain(mantaHandlerChain, req, res,
                function onCreateBucketObjectComplete(error) {
                // Always terminate the route chain for S3 requests to
                // prevent double execution
                if (error) {
                    next(error);
...
function s3GetBucketObjectHandler() {
    return function s3GetBucketObject(req, res, next) {
        ...
        // Call the original Manta get object handler chain with CORS support
        var mantaHandlerChain = buckets.getBucketObjectHandler();
        executeMiddlewareChain(mantaHandlerChain, req, res,
                               function onGetBucketObjectComplete(error) {
            // Always terminate the route chain for S3 requests to
            // prevent double execution
            if (error) {
                next(error);
...
function s3HeadBucketObjectHandler() {
    return function s3HeadBucketObject(req, res, next) {
        ...
        executeMiddlewareChain(mantaHandlerChain, req, res,
                               function onHeadBucketObjectComplete(error) {
            // Always terminate the route chain for S3 requests to
            // prevent double execution
            if (error) {
                next(error);
...
function s3DeleteBucketObjectHandler() {
    return function s3DeleteBucketObject(req, res, next) {
        ...
        executeMiddlewareChain(mantaHandlerChain, req, res,
                               function onDeleteBucketObjectComplete(error) {
            // Always terminate the route chain for S3 requests to
            // prevent double execution
            if (error) {
                next(error);
...
function s3DeleteBucketObjectsHandler() {
    return function s3DeleteBucketObjects(req, res, next) {
        req.log.debug('S3_DEBUG: s3DeleteBucketObjects' +
            ' - bulk delete operation');

        // Validate S3 request format
        if (!req.s3Request || !req.s3Request.bucket) {
            var InvalidBucketNameError = errors.InvalidBucketNameError;
            return (next(new InvalidBucketNameError('missing bucket name')));
        }
        ...
        // Get request body
        var body = req.body || req._rawBodyString ||
            (req._rawBodyBuffer ? req._rawBodyBuffer.toString('utf8') : null);
        ...
        // Extract object keys from XML
        var objectKeys = extractObjectKeysFromXML(body);
        if (!objectKeys) {
            return next(new InvalidParameterError('body',
        ...
        if (objectKeys.length === 0) {
            return (sendDeleteResult(req, res, next, deleted, deleteErrors));
        }

        // Delete each object
        processBulkDelete(req, bucketName, objectKeys, deleted, deleteErrors,
            function () {
                sendDeleteResult(req, res, next, deleted, deleteErrors);
            });
    };
}

// Helper: Extract object keys from XML delete request
function extractObjectKeysFromXML(body) {
    // JSSTYLED
    ...
    // Extract keys and decode HTML entities from XML
    return keyMatches.map(function (match) {
        // Decode HTML entities that may be present in XML
        /* BEGIN JSSTYLED */
...
// Helper: Process bulk delete operations
function processBulkDelete(req, bucketName, objectKeys, deleted, deleteErrors,
    callback) {
    var completed = 0;

    objectKeys.forEach(function (objectKey) {
        deleteSingleObject(req, bucketName, objectKey,
            function (success, error) {
                completed++;

                if (success) {
                    deleted.push({ Key: objectKey });
                    req.log.debug({ objectKey: objectKey },
                        'S3_DEBUG: Successfully deleted object');
                } else {
                    deleteErrors.push(error);
                    req.log.debug({ objectKey: objectKey, error: error },
                        'S3_DEBUG: Failed to delete object');
                }
...
// Helper: Delete a single object with full context loading
function deleteSingleObject(req, bucketName, objectKey, callback) {
    var deleteReq = createDeleteRequest(req, bucketName, objectKey);

    // Load bucket and object metadata
    bucketHelpers.loadRequest(deleteReq, null,
                               function onLoadRequestComplete(loadErr) {
        if (loadErr) {
            return callback(false, createDeleteError(objectKey, loadErr,
                'NoSuchKey', 'Object not found'));
        }

        loadBucketAndDelete(deleteReq, objectKey, callback);
    });
}

// Helper: Create delete request object
function createDeleteRequest(req, bucketName, objectKey) {
    var deleteReq = Object.create(req);
    // As the objectKey comes from parsing a XML
    // we need to decode/encode some of the characters
    // so MANTA is able to find the object for deletion.
    // https://docs.aws.amazon.com/AmazonS3/latest/userguide/object-keys.html
    deleteReq.params = {
        bucket_name: bucketName,
        object_name: encodeURIComponent(objectKey)
            .replace(/\&/g, '%26')
            .replace(/%2F/g, '/')
            .replace(/\'/g, '%27')
            .replace(/\(/g, '%28')
    ...
    req.log.debug({key: deleteReq.params.object_name},
        'S3_DEBUG bulk delete params');

    return (deleteReq);
}

// Helper: Load bucket context and perform deletion
function loadBucketAndDelete(deleteReq, objectKey, callback) {
    bucketHelpers.getBucketIfExists(deleteReq, null,
                                     function onGetBucketComplete(bucketErr) {
        if (bucketErr) {
            return callback(false, createDeleteError(objectKey, bucketErr,
                'NoSuchBucket', 'Bucket not found'));
        }

        executeObjectDeletion(deleteReq, objectKey, callback);
    });
}

// Helper: Execute the actual object deletion
function executeObjectDeletion(deleteReq, objectKey, callback) {
    // Perform the actual deletion
    var deleteHandler = deleteObjectModule.deleteBucketObjectHandler();
    // deleteObject function
    var deleteObjectFunc = deleteHandler[3];

    deleteObjectFunc(deleteReq, null,
                     function onDeleteObjectComplete(delErr, _result) {
        if (delErr) {
            return callback(false, createDeleteError(objectKey, delErr,
                'NoSuchKey',
                'Object not found - ensure key is properly encoded'));
        }
...
// Helper: Create delete error object
function createDeleteError(objectKey, err, defaultCode, defaultMessage) {
    return {
        Key: objectKey,
        Code: err.restCode || defaultCode,
        Message: err.message || defaultMessage
    };
}

// Helper: Send bulk delete result as XML
function sendDeleteResult(req, res, next, deleted, deleteErrors) {
    // Build XML response
    ...
    deleted.forEach(function (item) {
        xml += '  <Deleted>\n' +
               '    <Key>' + s3Compat.escapeXml(item.Key) + '</Key>\n' +
               '  </Deleted>\n';
    });
    ...
    deleteErrors.forEach(function (item) {
        xml += '  <Error>\n' +
               '    <Key>' + s3Compat.escapeXml(item.Key) + '</Key>\n' +
               '    <Code>' + s3Compat.escapeXml(item.Code) + '</Code>\n' +
               '    <Message>' + s3Compat.escapeXml(item.Message) +
               '</Message>\n' +
               '  </Error>\n';
    });
    ...
    }, 'S3_DEBUG: Sending bulk delete result');
...
function s3SetBucketACLHandler() {
    return function s3SetBucketACL(req, res, next) {
        ...
        if (req.body && req.body.length > 0) {
            ...
            parseS3ACLFromXML(req, function onBucketACLParsed(parseErr) {
                if (parseErr) {
        ...
        function processACLUpdate() {
            ...
            s3Compat.s3RoleTranslator(req, res,
                function onSetBucketACLRoleTranslated(translationErr) {
                if (translationErr) {
                    next(translationErr);
                    return;
                }
                ...
                var mantaHandlerChain = buckets.createBucketHandler();
                executeMiddlewareChain(mantaHandlerChain, req, res,
                                       function onSetBucketACLComplete(error) {
...
function s3GetBucketACLHandler() {
    return function s3GetBucketACL(req, res, next) {
        ...
        executeMiddlewareChain(headBucketChain, req, res,
                               function onGetBucketACLComplete(error) {
            if (error) {
                next(error);
            } else {
...
function updateObjectRoles(req, res, callback) {
    ...
    resolveRolesAndUpdate(req, roleNames, owner, bucket, bucketObject,
        requestId, log, callback);
}

/**
 * Resolve role names to UUIDs and perform object update
 */
function resolveRolesAndUpdate(req, roleNames, owner, bucket, bucketObject,
    requestId, log, callback) {
    // Handle empty roles (private ACL)
    if (roleNames.length === 0) {
        log.debug('updateObjectRoles: no role names provided, ' +
            'setting empty roles (private)');
        performObjectUpdate(req, [], owner, bucket, bucketObject,
            requestId, log, callback);
        return;
    }
    ...
    // If only system roles, no UUID lookup needed
    if (userRoleNames.length === 0) {
        log.debug({
            systemRoleNames: systemRoleNames,
            finalRoles: finalRoles
        }, 'updateObjectRoles: using only system roles');
        performObjectUpdate(req, finalRoles, owner, bucket, bucketObject,
            requestId, log, callback);
        return;
    }

    // Convert user-defined roles to UUIDs
    resolveUserRoleUuids(req, userRoleNames, finalRoles, owner, bucket,
        bucketObject, requestId, log, callback);
}

/**
 * Resolve user role names to UUIDs using Mahi
 */
function resolveUserRoleUuids(req, userRoleNames, finalRoles, owner, bucket,
    bucketObject, requestId, log, callback) {
    req.mahi.getUuid({
    ...
    function handleUuidLookup(err, lookup) {
        if (err) {
            log.error(err,
        ...
        log.debug({
            systemRoleNames: finalRoles.filter(function (r) {
                return (anonymousAuth.SYSTEM_ROLES.indexOf(r) !== -1);
            }),
            userRoleNames: userRoleNames,
...
/**
 * Perform the actual object metadata update
 */
function performObjectUpdate(req, roles, owner, bucket, bucketObject,
    requestId, log, callback) {
    ...
    client.getObject(owner, bucket.id, bucketObject.name,
        metadataLocation.vnode, {}, requestId,
        handleExistingObject);

    function handleExistingObject(getErr, existingObject) {
        if (getErr) {
            log.error(getErr,
        ...
        updateObjectWithRoles(client, owner, bucket, bucketObject,
            existingObject, roles, metadataLocation, requestId, log, callback);
    }
}

/**
 * Update object with new roles
 */
function updateObjectWithRoles(client, owner, bucket, bucketObject,
    existingObject, roles, metadataLocation, requestId, log, callback) {
    ...
    function handleUpdateResult(updateErr, updatedObject) {
        if (updateErr) {
            log.error(updateErr,
...
function s3SetObjectACLHandler() {
    return function s3SetObjectACL(req, res, next) {
        ...
        parseS3ACLFromXML(req, function onObjectACLParsed(parseErr) {
            if (parseErr) {
                req.log.warn(parseErr,
                    's3SetObjectACL: failed to parse XML ACL body');
            }
            processSetObjectACL(req, res, next, bucketName, objectPath);
        });
    };
}

// Helper: Process ACL update after XML parsing
function processSetObjectACL(req, res, next, bucketName, objectPath) {
    // Apply S3 role translation middleware
    s3Compat.s3RoleTranslator(req, res,
        function onSetObjectACLRoleTranslated(translationErr) {
        if (translationErr) {
            next(translationErr);
            return;
        }
        ...
        loadContextAndUpdateRoles(req, res, next, bucketName, objectPath);
    });
}

// Helper: Load bucket/object context and update roles
function loadContextAndUpdateRoles(req, res, next, bucketName, objectPath) {
    req.log.debug({
        bucket: bucketName,
        object: objectPath,
    ...
    executeMiddlewareChain(loadChain, req, res,
                           function onLoadContextComplete(loadErr) {
        if (loadErr) {
        ...
        applyRolesToObject(req, res, next, bucketName, objectPath);
    });
}

// Helper: Apply roles to object metadata
function applyRolesToObject(req, res, next, bucketName, objectPath) {
...
function s3GetObjectACLHandler() {
    return function s3GetObjectACL(req, res, next) {
        ...
        var metadataChain = getObjectChain.slice(0, -1);
        metadataChain.push(createACLMetadataHandler(bucketName, objectPath));

        executeMiddlewareChain(metadataChain, req, res,
                               function onGetObjectACLComplete(error) {
            if (error) {
                next(error);
            } else {
                next(false);
...
// Helper: Create metadata handler for ACL extraction
function createACLMetadataHandler(bucketName, objectPath) {
    return function handleACLMetadata(req, res, next) {
        var roleUuids = extractRoleUuidsFromMetadata(req);
        ...
        if (roleUuids.length === 0) {
            sendACLResponse(req, res, next, bucketName, objectPath,
                'private', []);
            return;
        }

        // Separate system roles from UUID roles
        var separatedRoles = separateSystemAndUuidRoles(roleUuids);

        // If only system roles, no UUID lookup needed
        if (separatedRoles.uuidRoles.length === 0) {
            var s3ACL = s3Compat.rolesToS3ACL(separatedRoles.systemRoles);
            sendACLResponse(req, res, next, bucketName, objectPath,
                s3ACL, separatedRoles.systemRoles);
            return;
        }

        // Resolve UUID roles to names
        resolveRoleUuidsToNames(req, separatedRoles, bucketName,
            objectPath, res, next);
    };
}

// Helper: Extract role UUIDs from request metadata
function extractRoleUuidsFromMetadata(req) {
    if (!req.metadata || !req.metadata.properties ||
        !req.metadata.properties.roles) {
        return ([]);
    }
    return (req.metadata.properties.roles);
}

// Helper: Separate system roles from UUID roles
function separateSystemAndUuidRoles(roleUuids) {
    var systemRoles = anonymousAuth.SYSTEM_ROLES;
    var literalRoles = [];
    var uuidRoles = [];
    ...
    return {
        systemRoles: literalRoles,
        uuidRoles: uuidRoles
    };
}

// Helper: Resolve role UUIDs to names via Mahi
function resolveRoleUuidsToNames(req, separatedRoles, bucketName,
    objectPath, res, next) {
    req.mahi.getName({
        uuids: separatedRoles.uuidRoles
    }, function (roleErr, roleNames) {
        if (roleErr) {
            req.log.warn(roleErr,
                's3GetObjectACL: failed to resolve role UUIDs');
            // Use only system roles if UUID resolution fails
            var fallbackACL = s3Compat.rolesToS3ACL(separatedRoles.systemRoles);
            sendACLResponse(req, res, next, bucketName, objectPath,
                fallbackACL, separatedRoles.systemRoles);
            return;
        }

        var allRoles = combineSystemAndResolvedRoles(separatedRoles.systemRoles,
            roleNames);
        var s3ACL = s3Compat.rolesToS3ACL(allRoles);

        req.log.debug({
            bucket: bucketName,
            object: objectPath,
            roleUuids: separatedRoles.uuidRoles,
            literalRoles: separatedRoles.systemRoles,
...
// Helper: Combine system roles with resolved role names
function combineSystemAndResolvedRoles(systemRoles, roleNames) {
    ...
    return (systemRoles.concat(resolvedRoleNames));
}

// Helper: Send ACL XML response
function sendACLResponse(req, res, next, bucketName, objectPath,
    s3ACL, roles) {
    req.log.debug({
        bucket: bucketName,
        object: objectPath,
        roles: roles,
        s3ACL: s3ACL
    }, 's3GetObjectACL: sending ACL response');
...
function s3CopyObjectHandler() {
    return function s3CopyObject(req, res, next) {
        ...
        /*
         * Enforce bucket scope on the copy source.  The destination
         * bucket was already checked by enforceBucketScope middleware
         * (from the URL path).  The source bucket comes from
         * x-amz-copy-source and must be explicitly validated here.
         */
        if (req.scopeContext && req.scopeContext.scope) {
            if (!bucketScopeLib.scopeGrantsAccess(
                    req.scopeContext.scope.permissions,
                    sourceBucketName,
                    bucketScopeLib.LEVEL_READ)) {
                next(new errors.AccessDeniedByKeyScopeError(
                    sourceBucketName, req.method));
                return;
            }
        }
        ...
        executeMiddlewareChain(copyHandlerChain, req, res,
                               function onCopyObjectComplete(error) {
            if (error) {
                next(error);
            } else {
                next(false); // Stop route processing
    ...
    return function s3PutObjectTagging(req, res, next) {
        ...
        req.on('end', function () {
            ...
            try {
                ...
                req.log.debug(parseErr, 'Failed to parse tagging XML');
...
/*
 * Handle GetBucketLocation requests (GET|HEAD /<bucket>?location).
 * Returns the region where the bucket is deployed, latest AWS documentation
 * recomends that HeadBucket operations should be used instead of GetBucket,
 * We assume older S3 clients are still using GetBucket routes, so we cover
 * both scenarios.
 * The implementation of this API is not required for full S3 compatiblity,
 * but S3 clients when region is not configured in their parameters, will
 * try to determine the region value using this API, for example: minio.
 * Users of S3 clients could also set the region envar or parameter of their
 * S3 clients to avoid using this API.
 *
 * HTTP verb GET returns canned XML with bucket location.
 * HTTP verb HEAD returns location in x-amz-bucket-region header.
 *
 * @param {Object} req  - restify request
 * @param {Object} res  - restify response
 * @param {function} next - restify next
 * @returns boolean if request is a bucket location request
 *
 */
function handleBucketLocation(req, res, next) {
    assert.object(req, 'req');
    assert.object(res, 'res');
    assert.func(next, 'next');

    if (!req.query || req.query.location === undefined) {
        return (false);
    }

    // We need to bypass the S3 response formatter
    // as this is a canned response.
    req._skipS3ResponseProcessing = true;
    // TODO:
    // In multi-DC, make this to be populated through SAPI
    var region = 'us-east-1';
    switch (req.method) {

        case 'HEAD':
           res.setHeader('x-amz-bucket-region', region);
           res.writeHead(200);
           res.end();
           break;
...
module.exports = {
    ...
    // S3 CORS handlers
    s3PutBucketCorsHandler: function s3PutBucketCorsHandler() {
        return function s3PutBucketCors(req, res, next) {
            // Convert S3 parameters to Manta format
            req.params.account = req.caller.account.login;
            req.params.bucket_name = req.params.bucket;
            var chain = bucketCors.putBucketCorsHandler();
            executeMiddlewareChain(chain, req, res, next);
        };
    },
    s3GetBucketCorsHandler: function s3GetBucketCorsHandler() {
        return function s3GetBucketCors(req, res, next) {
            // Convert S3 parameters to Manta format
            req.params.account = req.caller.account.login;
            req.params.bucket_name = req.params.bucket;
            var chain = bucketCors.getBucketCorsHandler();
            executeMiddlewareChain(chain, req, res, next);
        };
    },
    s3DeleteBucketCorsHandler: function s3DeleteBucketCorsHandler() {
        return function s3DeleteBucketCors(req, res, next) {
            // Convert S3 parameters to Manta format
            req.params.account = req.caller.account.login;
            req.params.bucket_name = req.params.bucket;
            var chain = bucketCors.deleteBucketCorsHandler();
            executeMiddlewareChain(chain, req, res, next);
        };
    },
