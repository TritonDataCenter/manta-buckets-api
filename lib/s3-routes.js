...
function s3DeleteBucketObjectsHandler() {
    return function s3DeleteBucketObjects(req, res, next) {
        ...
        if (!req.s3Request || !req.s3Request.bucket) {
            var InvalidBucketNameError = require('./errors').
        ...
        if (objectKeys.length === 0) {
            return (sendDeleteResult(req, res, next, deleted, errors));
        }

        // Delete each object
        processBulkDelete(req, bucketName, objectKeys, deleted, errors,
            function () {
                sendDeleteResult(req, res, next, deleted, errors);
            });
    };
}
...
function processBulkDelete(req, bucketName, objectKeys, deleted, errors,
    callback) {
    var completed = 0;
...
function sendDeleteResult(req, res, next, deleted, errors) {
    // Build XML response
