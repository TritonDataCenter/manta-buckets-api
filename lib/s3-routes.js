...
function s3DeleteBucketObjectsHandler() {
    return function s3DeleteBucketObjects(req, res, next) {
        req.log.debug('S3_DEBUG: s3DeleteBucketObjects - bulk delete operation');

        // Validate S3 request format
        if (!req.s3Request || !req.s3Request.bucket) {
            var InvalidBucketNameError = require('./errors').InvalidBucketNameError;
        ...
        // Extract keys and URL decode them to handle special characters
        ...
        objectKeys.forEach(function(objectKey) {
            deleteObject(objectKey, function(success, error) {
                ...
                if (success) {
                    ...
                    req.log.debug({ objectKey: objectKey }, 'S3_DEBUG: Successfully deleted object');
                } else {
                    errors.push(error);
                    req.log.warn({ objectKey: objectKey, error: error }, 'S3_DEBUG: Failed to delete object');
                }
        ...
        function deleteObject(objectKey, callback) {
            ...
            req.log.debug({deleteReq: deleteReq.params},
            ...
            bucketHelpers.loadRequest(deleteReq, null, function(loadErr) {
                ...
                bucketHelpers.getBucketIfExists(deleteReq, null, function(bucketErr) {
                    ...
                    var deleteObjectFunc = deleteHandler[3]; // deleteObject function
                    ...
                    deleteObjectFunc(deleteReq, null, function(delErr, result) {
                        if (delErr) {
                            return callback(false, {
                                ...
                                Message: delErr.message || 'Object not found - ensure key is properly encoded'
                            });
                        }
