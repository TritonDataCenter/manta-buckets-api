...
function s3ListBucketsHandler() {
    return function s3ListBuckets(req, res, next) {
        ...
        res.end = function () {
            ...
            /*
             * If any bucket chunk failed to parse, return an error rather
             * than a silent partial list. A partial listing is worse than
             * an error: the caller cannot distinguish "no access" from
             * "missing due to a bug", and a scoped key could silently
             * omit buckets it should see.
             */
            if (parseErrors > 0) {
                req.log.error(
                    {
            ...
            req.log.debug(
                {
                    ...
                    buckets: bucketData.map(function (b) {
                        return (b.name);
                    }),
                    bucketData: bucketData
                },
                'S3_DEBUG: s3ListBuckets - collected' +
                    ' all buckets, sending S3 response');

            // Filter buckets by access key scope if present
            if (req.scopeContext &&
                req.scopeContext.patterns) {
                ...
                req.log.debug(
                    {
                        filteredCount: bucketData.length,
                        scopePatterns: patterns
                    },
                    'Filtered ListBuckets by access key scope');
            }
...
function s3CreateBucketObjectHandler() {
    return function s3CreateBucketObject(req, res, next) {
        ...
        if (req.query && req.query.restore !== undefined) {
            req.log.debug(
                ...
                'S3_DEBUG: PUT Object restore operation detected ' +
                    '- not supported');
            var restoreError = new NotImplementedError(
                'Restore operations ' + 'are not supported');
        ...
        if (req.headers['content-type']) {
            req.log.debug(
                'S3_DEBUG: Setting binary mode for S3' +
        ...
        deleteSingleObject(
            ...
            function (success, error) {
                ...
                if (success) {
                    ...
                    req.log.debug(
                        { objectKey: objectKey, error: error },
...
function createDeleteRequest(req, bucketName, objectKey) {
    ...
    req.log.debug(
        { key: deleteReq.params.object_name },
...
function sendDeleteResult(req, res, next, deleted, deleteErrors) {
    ...
    deleteErrors.forEach(function (item) {
        xml +=
            ...
            '</Key>\n' +
            '    <Code>' +
            s3Compat.escapeXml(item.Code) +
            '</Code>\n' +
            '    <Message>' +
            s3Compat.escapeXml(item.Message) +
            '</Message>\n' +
            '  </Error>\n';
    });
    ...
    return function s3GetBucketACL(req, res, next) {
        ...
        if (Array.isArray(headBucketChain)) {
            ...
            headBucketChain.push(function (req2, resp, nextstep) {
                ...
                var roles =
                    req2.bucket && req2.bucket.roles ? req2.bucket.roles : [];
            ...
            headBucketChain = [
                ...
                function (req2, resp, nextstep) {
                    ...
                    var aclXml = generateS3ACLXml(
                        req2.caller.account.login,
                        s3ACL);
                    resp.setHeader('Content-Type', 'application/xml');
                    resp.send(200, aclXml);
                    nextstep();
...
function parseS3ACLFromXML(req, callback) {
    ...
    if (!req.body || req.body.length === 0) {
        ...
        if (contentLength && contentLength > 0) {
            ...
            req.log.debug(
                'parseS3ACLFromXML' + ': no body found, skipping XML parsing');
            callback(null);
            return;
        }
    }
 ...
 */
function resolveRolesAndUpdate(
    req,
    roleNames,
    owner,
    bucket,
    ...
    if (roleNames.length === 0) {
        log.debug(
            'updateObjectRoles: no role names provided, ' +
        ...
        log.debug(
            {
                ...
                existingRoles: existingObject.properties
                    ? existingObject.properties.roles
                    : existingObject.roles,
 ...
 */
function updateObjectWithRoles(
    client,
    owner,
    bucket,
    bucketObject,
    ...
    function handleUpdateResult(updateErr, updatedObject) {
        if (updateErr) {
            log.error(updateErr, 'updateObjectRoles: failed to update object');
            callback(updateErr);
            return;
        }
    ...
    return function handleACLMetadata(req, res, next) {
        ...
        if (separatedRoles.uuidRoles.length === 0) {
            ...
            sendACLResponse(
                ...
                objectPath,
                s3ACL,
                separatedRoles.systemRoles);
            return;
        }

        // Resolve UUID roles to names
        resolveRoleUuidsToNames(
            req,
            separatedRoles,
            bucketName,
            objectPath,
...
function resolveRoleUuidsToNames(
    req,
    separatedRoles,
    bucketName,
    objectPath,
    ...
    req.mahi.getName(
        ...
        function (roleErr, roleNames) {
            if (roleErr) {
                ...
                var fallbackACL = s3Compat.rolesToS3ACL(
                    separatedRoles.systemRoles);
                sendACLResponse(
                    req,
                    res,
            ...
            var allRoles = combineSystemAndResolvedRoles(
                separatedRoles.systemRoles,
                roleNames);
...
function sendACLResponse(req, res, next, bucketName, objectPath, s3ACL, roles) {
    req.log.debug(
        {
            bucket: bucketName,
            object: objectPath,
...
function s3CopyObjectHandler() {
    return function s3CopyObject(req, res, next) {
        ...
        /*
         * Enforce per-bucket scope on the source bucket.
         * The destination bucket was already checked by
         * enforceBucketScope middleware (from URL path).
         * The source bucket comes from x-amz-copy-source
         * and must be explicitly validated here.
         */
        if (req.caller && req.caller.bucketScope &&
            req.scopeContext && req.scopeContext.scope) {
            ...
            req.log.debug(
                {
                    ...
                    bodyContent: body,
                    bodyLength: body.length,
                    detectedFormat: body.trim().startsWith('TagSet=')
                        ? 's3cmd'
...
function updateObjectTagsInProperties(req, res, tags, callback) {
    ...
    var loadChain = [
        ...
        function loadObjectMetadata(loadReq, loadRes, next) {
            ...
            var client =
                loadReq.metadataPlacement.getBucketsMdapiClient(
                    metadataLocation);
