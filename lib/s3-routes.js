...
function parseS3ACLFromXML(req, callback) {
    ...
    if (!req.body || req.body.length === 0) {
        ...
        if (contentLength && contentLength > 0) {
            req.log.debug('parseS3ACLFromXML: reading body from request stream');
            var chunks = [];
            ...
            req.on('error', function(err) {
                req.log.error(err, 'parseS3ACLFromXML: error reading request body');
                callback(err);
            });
            
            return;
        } else {
            req.log.debug('parseS3ACLFromXML: no body found, skipping XML parsing');
            callback(null);
            return;
        }
    }
...
function updateObjectRoles(req, res, callback) {
    ...
    if (roleNames.length > 0) {
        req.mahi.getUuid({
            ...
            if (err) {
                log.error(err, 'updateObjectRoles: failed to resolve role names');
                callback(err);
                return;
            }
            ...
            for (var i = 0; i < roleNames.length; i++) {
                ...
                if (!lookup.uuids[roleName]) {
                    var InvalidRoleTagError = require('./errors').InvalidRoleTagError;
                    callback(new InvalidRoleTagError(roleName));
                    return;
                }
                roleUuids.push(lookup.uuids[roleName]);
            }
            ...
            performObjectUpdate(roleUuids);
        });
    } else {
        // No roles to set - this handles private ACL (empty roles)
        log.debug('updateObjectRoles: no role names provided, setting empty roles (private)');
        performObjectUpdate([]);
    }
    ...
    function performObjectUpdate(roles) {
        ...
        client.getObject(owner, bucket.id, bucketObject.name,
            metadataLocation.vnode, {}, requestId, function(getErr, existingObject) {
            if (getErr) {
                log.error(getErr, 'updateObjectRoles: failed to get existing object');
                callback(getErr);
                return;
            }
            ...
            log.debug({
                ...
                existingRoles: existingObject.properties ? existingObject.properties.roles : existingObject.roles,
                existingObjectId: existingObject.id,
                newRoles: roles
            }, 'updateObjectRoles: got existing object metadata, updating with new roles');

            // Update object with new roles, preserving existing content_type and object_id
        ...
        function processObjectACLUpdate() {
            ...
            s3Compat.s3RoleTranslator(req, res, function(translationErr) {
                ...
                executeMiddlewareChain(loadChain, req, res, function(loadErr) {
                    if (loadErr) {
                        req.log.error(loadErr, 's3SetObjectACL: failed to load bucket/object context');
                        next(loadErr);
                        return;
                    }
                    ...
                    updateObjectRoles(req, res, function(updateErr) {
                        if (updateErr) {
                            req.log.error(updateErr, 's3SetObjectACL: failed to update object roles');
                            next(updateErr);
                        } else {
                            req.log.debug({
                                bucket: bucketName,
...
function s3GetObjectACLHandler() {
    return function s3GetObjectACL(req, res, next) {
        ...
        metadataChain.push(function(req, res, next) {
            // Extract roles from object metadata - roles are in properties.roles as UUIDs
            var roleUuids = req.metadata && req.metadata.properties && req.metadata.properties.roles ? 
            ...
            var systemRoles = ['public-read', 'public-writer', 'authenticated-reader', 
                              'owner-reader', 'owner-full-control', 'log-writer'];
            var literalRoles = [];
            var uuidRoles = [];
            ...
            if (uuidRoles.length === 0) {
                var s3ACL = s3Compat.rolesToS3ACL(literalRoles);
            ...
            req.mahi.getName({
                ...
                if (err) {
                    req.log.warn(err, 's3GetObjectACL: failed to resolve role UUIDs');
                ...
                var s3ACL = s3Compat.rolesToS3ACL(allRoleNames);
