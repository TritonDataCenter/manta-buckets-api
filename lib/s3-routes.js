...
function s3DeleteBucketObjectsHandler() {
    return function s3DeleteBucketObjects(req, res, next) {
        ...
        var body = req.body || (req._rawBodyString ? req._rawBodyString : null);
        if (!body) {
            return (next(new InvalidParameterError('body', 'missing delete request body')));
        }
        ...
        if (!keyMatches || keyMatches.length === 0) {
            ...
            return next(new InvalidParameterError('body', 'No objects specified for deletion'));
        }
        ...
        objectKeys.forEach(function(objectKey) {
            ...
            (function(currentObjectKey) {
                ...
                }, 'S3_DEBUG: Creating delete request using original object key');
                ...
                bucketHelpers.loadRequest(deleteReq, null, function(loadErr) {
                    ...
                    bucketHelpers.getBucketIfExists(deleteReq, null, function(bucketErr) {
                        ...
                        // Execute the delete chain: the delete function is at index 4
                        // [loadRequest, getBucketIfExists, authorizationHandler, deleteObject, successHandler]
                        var deleteObjectFunc = deleteHandler[3]; // The actual deleteObject function
                        
                        deleteObjectFunc(deleteReq, null, function(delErr, result) {
                            if (delErr && delErr.restCode === 'ObjectNotFound') {
                                // First try with URL-decoded version in case XML key was encoded
                                var decodedKey;
                                try {
                                    decodedKey = decodeURIComponent(currentObjectKey);
                                } catch (decodeErr) {
                                    decodedKey = currentObjectKey; // Use original if decode fails
                                }
                                ...
                                if (decodedKey !== currentObjectKey) {
                                    ...
                                    }, 'S3_DEBUG: Object not found with original key, trying URL-decoded version');
                                    ...
                                    decodedDeleteReq.url = '/' + bucketName + '/' + decodedKey;
                                    decodedDeleteReq.conditions = {};
                                    
                                    // Try loading with decoded key
                                    bucketHelpers.loadRequest(decodedDeleteReq, null, function(decLoadErr) {
                                        if (decLoadErr) {
                                        ...
                                        bucketHelpers.getBucketIfExists(decodedDeleteReq, null, function(decBucketErr) {
                                            if (decBucketErr) {
                                                // Fall through to encoded attempts
                                                tryEncodedVersions();
                                                return;
                                            ...
                                            deleteObjectFunc(decodedDeleteReq, null, function(decDelErr, decResult) {
                                                completed++;
                                                ...
                                                if (decDelErr) {
                                                    req.log.debug({
                                                        ...
                                                        objectKey: currentObjectKey,
                                                        decodedKey: decodedKey
                                                    }, 'S3_DEBUG: Failed to delete object even with decoded key, trying encoded versions');
                                                    
                                                    // Fall through to encoded attempts by not returning here
                                                    // Reset completed counter and try encoded versions
                                                    completed--;
                                                    tryEncodedVersions();
                                                } else {
                                                    req.log.debug({
                                                        objectKey: currentObjectKey,
                                                        decodedKey: decodedKey
                                                    }, 'S3_DEBUG: Successfully deleted object using URL-decoded key');
                                ...
                                function tryEncodedVersions() {
                                    ...
                                    req.log.debug({
                                        originalKey: currentObjectKey,
                                        error: delErr.message
                                    }, 'S3_DEBUG: Object not found, trying encoded versions');
                                    
                                    var encodedKey = encodeURIComponent(currentObjectKey)
                                        .replace(/%2F/g, '/')  // Keep forward slashes unencoded
                                        .replace(/%28/g, '(')  // Keep parentheses unencoded for some clients
                                        .replace(/%29/g, ')');
                                    if (encodedKey !== currentObjectKey) {
                                        // Create new request with encoded key
                                        var encodedDeleteReq = Object.create(req);
                                        encodedDeleteReq.params = {
                                            bucket_name: bucketName,
                                            object_name: encodedKey
                                        };
                                        encodedDeleteReq.method = 'DELETE';
                                        encodedDeleteReq.url = '/' + bucketName + '/' + encodedKey;
                                        encodedDeleteReq.conditions = {};
                                        
                                        req.log.debug({
                                            originalKey: currentObjectKey,
                                            encodedKey: encodedKey
                                        }, 'S3_DEBUG: Retrying delete with encoded object key');
                                        
                                        // Try loading with encoded key
                                        bucketHelpers.loadRequest(encodedDeleteReq, null, function(encLoadErr) {
                                        if (encLoadErr) {
                                            completed++;
                                            errors.push({
                                                Key: currentObjectKey,
                                        ...
                                        bucketHelpers.getBucketIfExists(encodedDeleteReq, null, function(encBucketErr) {
                                            if (encBucketErr) {
                                                completed++;
                                                errors.push({
                                                    Key: currentObjectKey,
                                            ...
                                            deleteObjectFunc(encodedDeleteReq, null, function(encDelErr, encResult) {
                                                if (encDelErr && encDelErr.restCode === 'ObjectNotFound') {
                                                    // Try with parentheses encoded (some clients encode parentheses differently)
                                                    var parenEncodedKey = currentObjectKey
                                                        .replace(/\(/g, '%28')
                                                        .replace(/\)/g, '%29');
                                                    
                                                    if (parenEncodedKey !== currentObjectKey && parenEncodedKey !== encodedKey) {
                                                        req.log.debug({
                                                            originalKey: currentObjectKey,
                                                            encodedKey: encodedKey,
                                                            parenEncodedKey: parenEncodedKey
                                                        }, 'S3_DEBUG: Trying parentheses-encoded key');
