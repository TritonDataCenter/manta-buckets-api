...
function createServer(options, clients) {
    ...
    // Add conditional S3 compatibility middleware (only for SigV4 requests)
    server.use(function conditionalS3Middleware(req, res, next) {
        // Check if this is a SigV4 request
        var authHeader = req.headers.authorization || '';
        var isSigV4 = authHeader.toLowerCase().indexOf('aws4-hmac-sha256') === 0;
        
        if (isSigV4) {
            // Apply S3 compatibility middleware for SigV4 requests
            req.log.debug('Detected SigV4 request, applying S3 compatibility middleware');
            req.isS3Request = true;
            
            // Apply S3 middleware manually
            s3Compat.s3RequestDetector(req, res, function(err) {
                if (err) {
                    next(err);
                    return;
                }
                s3Compat.s3HeaderTranslator(req, res, function(err) {
                    ...
                    s3Compat.s3ResponseFormatter(req, res, next);
                });
            });
        } else {
            // Skip S3 middleware for traditional Manta requests
            req.log.debug('Traditional Manta request, skipping S3 middleware');
    ...
    // Add conditional S3 routes (only for SigV4 requests)
    addConditionalS3Routes(server);

    // Buckets API (traditional Manta routes)
...
/*
 * Conditional S3 API Routes (only for SigV4 requests)
 * These routes handle S3-style paths but only when the request contains
 * AWS4-HMAC-SHA256 authorization headers
 */
function addConditionalS3Routes(server) {
    // S3-style route handlers with SigV4 check
    
    // S3 List Buckets: GET / (only for SigV4)
    server.get({
        path: '/',
        name: 'S3ListBuckets'
    }, function(req, res, next) {
        if (!req.isS3Request) {
            // Not a SigV4 request, continue to next route
            next('route');
            return;
        }
        // Handle S3 list buckets
        s3Routes.s3ListBucketsHandler()(req, res, next);
    });
    ...
    server.put({
        ...
        s3Routes.s3CreateBucketHandler()(req, res, next);
    });
    ...
    server.get({
        ...
        s3Routes.s3ListBucketObjectsHandler()(req, res, next);
    });
    ...
    server.head({
        ...
        s3Routes.s3HeadBucketHandler()(req, res, next);
    });
    ...
    server.del({
        ...
        s3Routes.s3DeleteBucketHandler()(req, res, next);
    });
    ...
    server.put({
        ...
        s3Routes.s3CreateBucketObjectHandler()(req, res, next);
    });
    ...
    server.get({
        ...
        s3Routes.s3GetBucketObjectHandler()(req, res, next);
    });
    ...
    server.head({
        ...
        s3Routes.s3HeadBucketObjectHandler()(req, res, next);
    });
    ...
    server.del({
        ...
        s3Routes.s3DeleteBucketObjectHandler()(req, res, next);
    });
}
