    ...
    server.get('/', function consolidatedRootHandler(req, res, next) {
        if (req.isS3Request) {
            ...
            s3Compat.s3HeaderTranslator(req, res, function (headerErr) {
                if (headerErr) {
                    req.log.error(headerErr, 'S3_DEBUG: Error in s3HeaderTranslator');
                    next(headerErr);
                    return;
                }
                ...
                s3Compat.s3ResponseFormatter(req, res, function (formatterErr) {
                    if (formatterErr) {
                        req.log.error(formatterErr, 'S3_DEBUG: Error in s3ResponseFormatter');
                        next(formatterErr);
                        return;
                    }
                    ...
                    handleS3Request(req, res, function (handlerErr) {
                        ...
                        if (handlerErr) {
                            ...
                            var isUserError = (handlerErr.statusCode >= 400 && handlerErr.statusCode < 500) ||
                                             (handlerErr.name && (handlerErr.name.includes('NotFound') || handlerErr.name.includes('Exists') || handlerErr.name.includes('BadRequest')));
                            ...
                            if (handlerErr.name === 'ObjectNotFoundError' || handlerErr.restCode === 'ObjectNotFoundError') {
                                statusCode = 404;
                            } else if (handlerErr.name === 'BucketNotFoundError' || handlerErr.restCode === 'BucketNotFoundError') {
                                statusCode = 404;
                            } else if (handlerErr.name === 'BucketExistsError' || handlerErr.restCode === 'BucketExistsError') {
                                statusCode = 409;
...
function handleS3Request(req, res, next) {
    ...
    var pathParts = requestPath.split('/').filter(function (part) { return part.length > 0; });
