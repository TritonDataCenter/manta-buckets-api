...
///--- Anonymous Access Helper Functions (exported for testing)
...
function flattenHandlers(rawHandlers) {
    var handlers = [];
    rawHandlers.forEach(function flattenHandler(handler) {
...
function createServer(options, clients) {
    ...
    // Pass the cueball HTTP agent so DNS resolves via
    // binder, not the system resolver.
    var stsClient = stsHandlers.createSTSClient(
        options, log, clients.agent);

    // Create IAM client for AWS IAM operations
    var iamClient = iamHandlers.createIAMClient(
        options, log, clients.agent);
    ...
    // Per-bucket access key scope enforcement.
    // Runs after gatherContext (req.caller is set) and
    // before route handlers. Scoped keys are restricted
    // to the buckets and permission levels in their scope.
    server.use(auth.enforceBucketScope);
     ...
     * Uses module-level helper functions (exported for testing)
     */
...
module.exports = {
    ...
    // Anonymous access helper functions (exported for testing)
    parseMantaBucketObjectPath: parseMantaBucketObjectPath,
    isMantaAnonymousObjectAccess: isMantaAnonymousObjectAccess,
    setupMantaObjectParams: setupMantaObjectParams,
    flattenHandlers: flattenHandlers,
    executeHandlerChain: executeHandlerChain
};
