...
function createServer(options, clients) {
    ...
    // This MUST run before ANY other middleware that might consume the request stream
    server.pre(function preserveRawBodyPreMiddleware(req, res, next) {
        ...
        if (req.method === 'POST' && 
            ...
            req.log.debug({
                ...
                contentType: req.headers['content-type'],
                url: req.url
            }, 'S3_DEBUG: PRE-MIDDLEWARE - Preserving raw body for S3 signature verification');
            ...
            req.on('end', function() {
                if (chunks.length > 0) {
                    ...
                    req.log.debug({
                        ...
                        md5Match: req._originalContentMD5 === req.headers['content-md5'],
                        bodyPreview: req._rawBodyString.substring(0, 200)
                    }, 'S3_DEBUG: PRE-MIDDLEWARE - Raw body preserved for signature verification');
                }
            });
            
            req.on('error', function(err) {
                req.log.error(err, 'S3_DEBUG: PRE-MIDDLEWARE - Error while preserving raw body');
            });
    ...
    server.post(/^\/([^\/]+)$/, function (req, res, next) {
        if (req.isS3Request) {
            req.log.debug('S3_DEBUG: S3 single-segment POST route handling S3 request');
            s3RouteHandler(req, res, next);
        } else {
            req.log.debug('S3_DEBUG: S3 single-segment POST route passing through non-S3 request');
            next();
        }
    });

    // S3 object operations (two or more path segments) - for S3 requests AND Manta bucket objects
    server.get(/^\/([^\/]+)\/(.+)$/, function (req, res, next) {
        req.log.debug({
            path: req.path(),
        ...
        if (req.isS3Request) {
            ...
            // Check if this is a Manta bucket object request that needs special handling
            var path = req.path();
            var pathParts = path.split('/').filter(function(part) {
                return part.length > 0;
            });
            
            // Check for Manta-style bucket object path: /account/buckets/bucket_name/objects/...
            if (pathParts.length >= 5 && pathParts[1] === 'buckets' && pathParts[3] === 'objects' && req.potentialAnonymousAccess) {
                req.log.debug('S3_DEBUG: Detected Manta bucket object request with potential anonymous access, routing to bucket handler');
                
                // Set up the route parameters that the bucket object handler expects
                ...
                }, 'S3_DEBUG: Set up Manta route parameters for bucket object request');
                
                // Execute the bucket object handler chain using Restify's built-in chaining
                ...
                var chainedHandler = function(req, res, next) {
                    var index = 0;
                    ...
                    function executeNext(err) {
                        ...
                        if (typeof currentHandler === 'function') {
                            ...
                            next(new Error('Invalid handler in bucket object chain at index ' + (index - 1)));
                        }
                    }
        ...
        if (pathParts.length === 1) {
            ...
            switch (method) {
                ...
                case 'post':
                    ...
                    if (req.s3Request && req.s3Request.operation === 'DeleteBucketObjects') {
                        req.log.debug('S3_DEBUG_ROUTING: ROUTING TO s3DeleteBucketObjectsHandler');
                        return (s3Routes.s3DeleteBucketObjectsHandler()(req, res, next));
                    } else {
                        req.log.warn('S3_WARN: Unsupported POST operation for bucket');
                        res.send(405, {
...
function addBucketsRoutes(server) {
    ...
    // NOTE: GetBucketObject route moved to priority position before generic regex routes
    // server.get({
    //     path: '/:account/buckets/:bucket_name/objects/:object_name',
