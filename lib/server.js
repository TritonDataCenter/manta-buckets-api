...
function createServer(options, clients) {
    ...
    // Add S3 request detection and routing middleware
    server.use(function s3RequestRouter(req, res, next) {
        ...
        if (isSigV4) {
            // This is a SigV4 request - handle with S3 compatibility
            req.log.debug('Detected SigV4 request, routing to S3 handlers');
            ...
            // Traditional Manta request - continue to normal routing
...
function handleS3Request(req, res, next) {
    ...
    req.log.debug({
        ...
        path: path,
        pathParts: pathParts
    }, 'handleS3Request: routing S3 request');
    ...
    try {
        ...
        req.log.warn({
            method: method,
            path: path
        }, 'handleS3Request: no S3 route matched for SigV4 request');
        ...
        req.log.error(err, 'handleS3Request: error routing S3 request');
        next(err);
    }
}
