...
function createServer(options, clients) {
    ...
    // Anonymous access handler for public buckets (before authentication, with metadata client)
    ...
    // Browser-friendly redirect: /user/buckets/bucketname/filename -> /user/buckets/bucketname/objects/filename
    ...
    server.get(/^\/([^\/]+)\/buckets\/([^\/]+)\/(.+)$/, function (req, res, next) {
        ...
        var pathSegments = req.path().split('/').filter(function(s) { return s.length > 0; });
        ...
        if (isBrowserRequest && !hasObjectsSegment && !req.isS3Request) {
            var redirectPath = '/' + userName + '/buckets/' + bucketName + '/objects/' + objectPath;
