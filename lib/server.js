...
function createServer(options, clients) {
    ...
    server.pre(function s3RequestDetectorEarly(req, res, next) {
        ...
        // Consider it S3 request if:
        // 1. SigV4 auth header present, OR
        // 2. Virtual-hosted format detected, OR  
        // 3. Has authorization header (for backward compatibility)
        var isSigV4 = startsWithAws4;
        var hasAuth = authHeader.length > 0;
        var isS3Format = isSigV4 || isVirtualHostedFormat || hasAuth;
        ...
        req.log.info({
            ...
            localhostCheck: !host.match(/^localhost(:\d+)?$/),
            ipCheck: !host.match(/^\d+\.\d+\.\d+\.\d+(:\d+)?$/)
        }, 'S3_DEBUG: Enhanced request detection with detailed breakdown');
    ...
    // S3 Object Operations - catch all pattern for S3 requests
    server.put(/^\/(.+)$/, function s3ObjectPutRoute(req, res, next) {
        if (req.isS3Request) {
            req.log.info('S3_DEBUG: S3 PUT route matched for S3 request');
            ...
            // Not an S3 request, continue to traditional routes
            next();
        }
    });
    
    server.get(/^\/(.+)$/, function s3ObjectGetRoute(req, res, next) {
        if (req.isS3Request) {
            req.log.info('S3_DEBUG: S3 GET route matched for S3 request');
    ...
    server.head(/^\/(.+)$/, function s3ObjectHeadRoute(req, res, next) {
        if (req.isS3Request) {
            req.log.info('S3_DEBUG: S3 HEAD route matched for S3 request');
    ...
    server.del(/^\/(.+)$/, function s3ObjectDeleteRoute(req, res, next) {
        if (req.isS3Request) {
            req.log.info('S3_DEBUG: S3 DELETE route matched for S3 request');
