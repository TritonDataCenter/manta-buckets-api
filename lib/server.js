...
function createServer(options, clients) {
    ...
    server.pre(function preserveRawBodyPreMiddleware(req, res, next) {
        ...
        // Include: XML content-type requests OR Complete Multipart Upload
        // requests
        if (req.method === 'POST' &&
            ...
            ' S3 signature verification or multipart upload completion');
                ...
                s3Compat.s3ConditionalHeaders(req, res,
                    ...
                    s3Compat.s3RoleTranslator(req, res, function (roleErr) {
                        ...
                        s3Compat.s3ResponseFormatter(req, res,
                            ...
                            // Route to S3 handler (supports both
                            // addressing styles)
                            handleS3Request(req, res, function (handlerErr) {
                                ...
                                if (handlerErr) {
                                    ...
                                    if (isUserError) {
                                        ...
                                        req.log.error(handlerErr, 'S3_DEBUG:' +
                                        ' System error in S3 handler,' +
                                        ' converting to S3 XML format');
                                    }

                                    var s3XmlError =
                                        s3Compat.convertErrorToS3
    ...
    server.get(/^\/([^\/]+)\/(.+)$/, function (req, res, next) {
        ...
        if (req.isS3Request) {
            ...
            s3RouteHandler(req, res, next);
              ...
              * that falls into a Manta route so we need to execute more steps
              * before falling through, only if the object has anonymous access.
              */
            ...
            if (pathParts.length >= 5 && pathParts[1] === 'buckets' &&
                ...
                }, 'S3_DEBUG: Set up Manta route parameters for bucket' +
                              ' object request');

                // Execute the bucket object handler chain using Restify's
                // built-in chaining
                var rawHandlers = buckets.getBucketObjectHandler();

                // Flatten the handler chain since some handlers return arrays
                var handlers = [];
                 ...
                 * The relevant thing here is that getBucketObjectHandler has
                 * the middleware that allows us to determinate if we have
                 * access to the object, through setting req.caller to roles
                 * ['public-read'] and, isAnonymousPublicAccess: true
                 */
                var chainedHandler = function (request, response, nextHandler) {
                    var index = 0;
                    ...
                    function executeNext(err) {
                        ...
                        // Ensure we have a valid function before calling
                        if (typeof (currentHandler) === 'function') {
                            ...
                            nextHandler(new Error('Invalid handler in bucket' +
                               ' object chain at index ' + (index - 1)));
                        }
                    }
...
function handleS3Request(req, res, next) {
    ...
    try {
        ...
        if (method === 'get' && pathParts.length === 0) {
            /*
             * This is wrong, virtual-hosted request means that
             * every every request will have this form:
             * https://my-bucket.<your-domain>/my-object.txt
             * This involves that DNS should use wildcards,
             * as every bucket name should be resolvable, and also
             * TLS wildcard certificates needs to be in place.
             * Knowing that the below check is plain wrong, but I
             * use it to test it someway in a development environment.
             * as virtual-hosted style is more complex to deploy that
             * path-based-style, this has been disabled. Instead of localhost
             * we should check the domain where this service is deployed,
             * but for future testing if this feature is required, the code has
             * not been remove.
             * path-style is supported widely and AWS SDK allows you to specify
             * the access style path/virtual when using the API.
             */

            /*
             * FIXME: This should use an actual domain
             */
            var host = req.headers.host || '';
            var hostParts = host.split('.');
            ...
            /*
             * Today we should never see isVirtualHosted = true.
             * The process.env is just a placeholder when this feature
             * is worked on.
             */

            if (isVirtualHosted) {
                // XXX Today, we should not get here.
                ...
                /*
                 * Non-virtual-hosted GET / = List all buckets for the account.
                 */
