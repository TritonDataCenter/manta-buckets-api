...
function createServer(options, clients) {
    ...
    // Add S3 request detection middleware BEFORE other routes
    server.use(function s3RequestDetector(req, res, next) {
        ...
        if (isSigV4) {
            ...
            }, 'S3_DEBUG: Detected SigV4 request, marking for S3 processing');
            ...
            req.log.debug('Traditional Manta request, using normal routing');
    ...
    // Add conditional redirect for root path (skip S3 requests for now)
    server.get('/', function conditionalRedirect(req, res, next) {
        if (req.isS3Request) {
            // Skip redirect for S3 requests, let them continue to auth and S3 handlers
            req.log.info('S3_DEBUG: Root path route - skipping redirect, continuing to auth middleware');
            next();
        } else {
            // Traditional redirect for docs
            req.log.debug('Root path route - redirecting to docs');
    ...
    // set up random stuff (excluding root redirect since we handle it above)
    server.get({
        path: '/ping',
        name: 'ping'
    ...
    // Add dedicated S3 routes AFTER authentication
    server.get('/', function s3RootHandler(req, res, next) {
        if (req.isS3Request) {
            req.log.info('S3_DEBUG: S3 root handler - processing S3 list buckets request');
            ...
            s3Compat.s3HeaderTranslator(req, res, function(err) {
                ...
                s3Compat.s3ResponseFormatter(req, res, function(err) {
                    ...
                    // Route to S3 list buckets handler
                    handleS3Request(req, res, next);
                });
            });
        } else {
            // Not an S3 request, skip this handler
            next();
        }
    });

    // Add S3 routing middleware for other paths AFTER authentication
    server.use(function s3RequestRouter(req, res, next) {
        if (req.isS3Request && req.path() !== '/') {
            req.log.info('S3_DEBUG: Processing S3 request for non-root path with authentication complete');
            ...
            s3Compat.s3HeaderTranslator(req, res, function(err) {
                ...
                s3Compat.s3ResponseFormatter(req, res, function(err) {
                    ...
                    // Route to appropriate S3 handler based on path and method
                    handleS3Request(req, res, next);
                });
            });
        } else {
    ...
    // Buckets API (traditional Manta routes only)
