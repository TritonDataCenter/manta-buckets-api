...
function createServer(options, clients) {
    ...
    server.post('/', function stsRequestHandler(req, res, next) {
        ...
        if (action === 'AssumeRole' || action === 'GetSessionToken' ||
            ...
            // AWS restriction: GetSessionToken temporary credentials
            // (MSTS prefix) cannot call IAM APIs. Only permanent
            // credentials or AssumeRole credentials (MSAR prefix)
            // are allowed to call IAM operations.
            if (accessKeyId.indexOf('MSTS') === 0) {
                req.log.warn({
                    accessKeyId: accessKeyId,
                    action: action
                }, 'IAM: GetSessionToken credentials cannot call IAM APIs');

                var stsCredError = s3Compat.convertErrorToS3({
                    name: 'AccessDenied',
                    message: 'Cannot call IAM operations with ' +
                        'GetSessionToken credentials. Use permanent ' +
                        'credentials or credentials from AssumeRole.',
                    statusCode: 403
                }, null, req);
                res.setHeader('Content-Type', constants.CONTENT_TYPES.XML);
                res.writeHead(403);
                res.end(stsCredError);
                return (next(false));
            }
