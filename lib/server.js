                    ...
                    if (stsMahiClient) {
                        stsMahiClient.getUserByAccessKey(stsAccessKeyId,
                            ...
                            if (authErr || !authRes.account) {
                                ...
                                req.caller = authRes;
                                // Also set req.auth for temporary credentials
                                // (needed for GetCallerIdentity)
                                req.log.info({authRes: authRes,
                                              keyused: stsAccessKeyId },
                                             'STS AUTH');
                                /*
                                 * Temporary Keys begin with MSTS (AWS style).
                                 * Use authRes.assumedRole directly from Mahi
                                 * response instead of looking in accesskeys
                                 * (which may not be synced yet from UFDS).
                                 */
                                if (stsAccessKeyId.indexOf('MSTS') === 0) {
                                    var assumedRoleData = authRes.assumedRole;
                                    req.log.info({
                                        assumedRole: assumedRoleData,
                                        hasAssumedRole: !!assumedRoleData
                                    }, 'STS TEMP: using authRes.assumedRole');
                                    req.auth = {
                                        accessKeyId: stsAccessKeyId,
                                        assumedRole: assumedRoleData ?
                                            (assumedRoleData.arn ||
                                             assumedRoleData) : null,
                                        isTemporaryCredential: true,
                                        sessionName: authRes.sessionName || null,
                                        principalUuid: authRes.account ?
                                            authRes.account.uuid : null
                                    };
                                } else {
                                    req.auth = {
                                    accessKeyId: stsAccessKeyId,
                                    ...
                                    principalUuid: authRes.principalUuid ||
                                        (authRes.account && authRes.account.uuid)
                                    };
                                }
                                req.log.debug({
