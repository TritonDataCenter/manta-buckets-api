                ...
                s3Compat.s3ConditionalHeaders(req, res, function (conditionalErr) {
                    ...
                    s3Compat.s3RoleTranslator(req, res, function (roleErr) {
                        ...
                        s3Compat.s3ResponseFormatter(req, res, function (formatterErr) {
                            ...
                            req.log.debug('S3_DEBUG: s3ResponseFormatter completed, calling handleS3Request');
                            ...
                            handleS3Request(req, res, function (handlerErr) {
                                ...
                                if (handlerErr) {
                                    ...
                                    if (isUserError) {
                                        ...
                                        req.log.error(handlerErr, 'S3_DEBUG: System error in S3 handler, converting to S3 XML format');
                                    }
                                    
                                    var s3XmlError = s3Compat.convertErrorToS3(handlerErr, req.s3Request);
                                    ...
                                    handlerErr.restCode === 'BucketExistsError') {
                                        statusCode = 409;
                                    } else if (handlerErr.statusCode && typeof (handlerErr.statusCode) === 'number') {
                                        statusCode = handlerErr.statusCode;
                                    } else if (handlerErr.code && typeof (handlerErr.code) === 'number') {
