...
function createServer(options, clients) {
    ...
    server.pre(function preserveRawBodyPreMiddleware(req, res, next) {
        ...
        if (req.method === 'POST' && req.url) {
            // Simple check for uploadId query parameter (Complete Multipart Upload)
        ...
        // Include: XML content-type requests OR Complete Multipart Upload requests
        if (req.method === 'POST' &&
            req.headers['content-length'] &&
            parseInt(req.headers['content-length'], 10) > 0 &&
            req.headers.authorization &&
            (req.headers['content-type'] === 'application/xml' || isCompleteMultipartUpload)) {
    ...
    // S3 object operations (two or more path segments) -
    // for S3 requests AND Manta bucket objects
    server.get(/^\/([^\/]+)\/(.+)$/, function (req, res, next) {
        req.log.debug({
            path: req.path(),
        ...
        if (req.isS3Request) {
            ...
            // Check if this is a Manta bucket object request
...
function handleS3Request(req, res, next) {
    ...
    try {
        ...
        if (pathParts.length >= 2) {
            ...
            if (req.s3Request && req.s3Request.operation === 'InitiateMultipartUpload') {
                req.log.debug('S3_DEBUG_ROUTING: ROUTING TO s3InitiateMultipartUploadHandler');
                return s3Routes.s3InitiateMultipartUploadHandler()(req, res, next);
            } else if (req.s3Request && req.s3Request.operation === 'UploadPart') {
                req.log.debug('S3_DEBUG_ROUTING: ROUTING TO s3UploadPartHandler');
                return s3Routes.s3UploadPartHandler()(req, res, next);
            } else if (req.s3Request && req.s3Request.operation === 'CompleteMultipartUpload') {
                req.log.debug('S3_DEBUG_ROUTING: ROUTING TO s3CompleteMultipartUploadHandler');
                return s3Routes.s3CompleteMultipartUploadHandler()(req, res, next);
            } else if (req.s3Request && req.s3Request.operation === 'AbortMultipartUpload') {
                req.log.debug('S3_DEBUG_ROUTING: ROUTING TO s3AbortMultipartUploadHandler');
                return s3Routes.s3AbortMultipartUploadHandler()(req, res, next);
            }
