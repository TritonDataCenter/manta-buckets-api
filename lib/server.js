...
function createServer(options, clients) {
    ...
    // Add S3 compatibility middleware
    server.use(s3Compat.s3RequestDetector);
    server.use(s3Compat.s3HeaderTranslator);
    server.use(s3Compat.s3ResponseFormatter);

    // Add various fields to the 'req' object before the handlers get called.
    server.use(common.setupHandler(options, clients));

    // S3 API routes (must come before Manta routes to handle S3-style paths)
    s3Routes.addS3Routes(server);
