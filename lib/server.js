...
function createServer(options, clients) {
    ...
    server.pre(function preserveRawBodyPreMiddleware(req, res, next) {
        ...
        if (((req.method === 'POST' &&
            ...
            ' multipart upload, or CORS configuration');
    ...
    // This must come before auth middleware to bypass signature validation
    server.opts(/^\/([^\/]+)\/(.+)$/, function (req, res, next) {
        ...
        if (req.headers.origin &&
            ...
            req.log.debug({
                ...
                hasPresignedParams:
                !!(req.query && req.query['X-Amz-Algorithm'])
            }, 'CORS: Handling preflight request (bypassing auth for OPTIONS)');

            // For OPTIONS preflight, bypass authentication and directly return
            // CORS headers, this is safe because OPTIONS doesn't perform
            // actual operations
            ...
            res.setHeader('Access-Control-Allow-Headers', 'Content-Type,' +
                          ...
                          ',x-amz-meta-access-control-allow-methods,' +
                          'x-amz-meta-access-control-allow-credentials');
            res.setHeader('Access-Control-Max-Age', '3600');
            res.setHeader('Access-Control-Allow-Credentials', 'true');

            res.send(200);
            next(false); // Stop processing - bypass all auth
        } else {
            next();
        }
    });
