...
function createServer(options, clients) {
    ...
    // Add conditional redirect for root path (skip for S3 requests)
    server.get('/', function conditionalRedirect(req, res, next) {
        if (req.isS3Request) {
            // Skip redirect for S3 requests, let them continue to S3 handlers
            req.log.info('S3_DEBUG: Root path route - continuing to S3 handlers');
            next();
        } else {
    ...
    // Add S3 routing middleware AFTER authentication
    server.use(function s3RequestRouter(req, res, next) {
        if (req.isS3Request) {
            req.log.info('S3_DEBUG: Processing S3 request with authentication complete');
