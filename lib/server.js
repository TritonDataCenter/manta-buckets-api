...
function createServer(options, clients) {
    ...
    server.pre(function preserveRawBodyPreMiddleware(req, res, next) {
        ...
        // Complete Multipart Upload requests, OR STS requests
        ...
        if (req.method === 'POST' &&
            ...
            ' multipart upload completion, or STS operations');
    ...
    server.use(function preAuthDebug(req, res, next) {
        ...
        next();
        if (req.isS3Request) {
            req.log.debug('S3_DEBUG: preAuthDebug next() called successfully');
        }
    });

    // Create STS client for AWS STS operations
    log.debug('S3_DEBUG: About to create STS client');
    var stsClient = stsHandlers.createSTSClient(options, log);
    log.debug('S3_DEBUG: STS client created successfully');

    // Create IAM client for AWS IAM operations
    log.debug('S3_DEBUG: About to create IAM client');
    var iamClient = iamHandlers.createIAMClient(options, log);
    log.debug('S3_DEBUG: IAM client created successfully');
    ...
    server.use(function (req, res, next) {
        if (req.isS3Request) {
            req.log.debug({
                method: req.method,
                url: req.url,
                hasMetadataClient: !!clients.metadataPlacement
            }, 'S3_DEBUG: About to enter anonymous access handler');
        }
    ...
    // Add debug logging after anonymous access handler
    server.use(function postAnonymousHandler(req, res, next) {
        if (req.isS3Request) {
            req.log.debug('S3_DEBUG: Anonymous access handler completed');
        }
        next();
    });

    // Add debug logging before authentication handler
    server.use(function preAuthHandler(req, res, next) {
        if (req.isS3Request) {
            req.log.debug({
                method: req.method,
                url: req.url,
                hasAuthHeader: !!req.headers.authorization,
                mahiClient: !!clients.mahi,
                mahiUrl: clients.mahi ? clients.mahi.url : 'undefined'
            }, 'S3_DEBUG: About to enter authentication handler');
        }
        next();
    });
    ...
    // Add debug logging after authentication handler
    server.use(function postAuthHandler(req, res, next) {
        if (req.isS3Request) {
            req.log.debug('S3_DEBUG: ' +
                          'Authentication handler completed successfully');
        }
        next();
    });
