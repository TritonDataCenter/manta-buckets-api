...
function createServer(options, clients) {
    ...
    server.pre(function logAllRequests(req, res, next) {
        ...
        // For PUT requests (potential uploads),
        // add binary data debugging and force binary mode immediately
        if (req.method === 'PUT' && req.headers['content-type'] &&
        req.headers['content-type'].indexOf('image/') === 0) {
            ...
            }, 'BINARY_DEBUG: Binary upload debugging'+
            ' - forcing binary mode immediately');

            // Force binary mode as early as possible to prevent text
            // encoding corruption
            if (req.setEncoding) {
                req.setEncoding(null);
                req.log.info('BINARY_DEBUG: Set request encoding '+
                'to null (binary mode)');
            }
            ...
            // Mark for binary handling and disable automatic content-encoding
            // processing
            req._binaryMode = true;
            req._forceRawData = true;

            // CRITICAL: For binary uploads, bypass restify
            // processing and use raw HTTP request
            if (req.headers['content-type'] &&
            req.headers['content-type'].indexOf('image/') === 0) {
                req.log.info('BINARY_DEBUG: Using raw HTTP request'+
                ' for binary upload');

                // Get access to the raw Node.js request object
                var rawReq = req.raw || req.connection.parser.incoming || req;
                if (rawReq && rawReq !== req) {
                    req.log.info('BINARY_DEBUG: Found raw request object'+
                    ', preserving binary stream');
    ...
    server.pre(function s3RequestDetectorEarly(req, res, next) {
        req.log.info('S3_DEBUG: S3 detection middleware executing for ' +
        req.method + ' ' + req.path());
        ...
        if (isS3Format) {
            ...
            if ((req.method === 'PUT' &&
                ...
                (req.method === 'GET' &&
                  req.s3Request.operation === 'GetBucketObject')) {
                req.log.info('S3_DEBUG: Marking S3 object operation'+
    ...
    server.get(/^\/([^\/]+)$/, function (req, res, next) {
        if (req.isS3Request) {
            req.log.info('S3_DEBUG: S3 single-segment GET route'+
            ' handling S3 request');
    ...
    server.get(/^\/([^\/]+)\/(.+)$/, function (req, res, next) {
        ...
        if (req.isS3Request) {
            req.log.info('S3_DEBUG: Processing as S3 request'+
            ' in GET two-segment route');
            s3RouteHandler(req, res, next);
        } else {
            req.log.info('S3_DEBUG: Passing through to next '+
        ...
        if (method === 'get' && pathParts.length === 0) {
            ...
            if (isVirtualHosted) {
                ...
                req.log.info({
                    ...
                    bucket: bucketName,
                    host: host
                }, 'S3_DEBUG: Routing virtual-hosted GET / to bucket objects handler');
                
                // Route to bucket objects handler
                if (req.s3Request && req.s3Request.operation === 'ListBucketObjectsV2') {
                    console.log('S3_DEBUG_ROUTING: ✅ ROUTING TO s3ListBucketObjectsV2Handler (virtual-hosted)');
                    return (s3Routes.s3ListBucketObjectsV2Handler()(req, res, next));
                } else {
                    console.log('S3_DEBUG_ROUTING: ❌ ROUTING TO s3ListBucketObjectsHandler (V1 virtual-hosted)');
        ...
        if (pathParts.length === 1) {
            ...
            switch (method) {
                ...
                case 'get':
                    ...
                    console.log('S3_DEBUG_ROUTING: handleS3Request GET case'+
                    ' - making routing decision');
                    console.log('S3_DEBUG_ROUTING: req.s3Request exists:',
                    !!req.s3Request);
                    console.log('S3_DEBUG_ROUTING: req.s3Request.operation:',
                    req.s3Request ? req.s3Request.operation : 'undefined');
                    console.log('S3_DEBUG_ROUTING: operation ==='+
                    ' ListBucketObjectsV2:',
