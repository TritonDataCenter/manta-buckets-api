...
function selectFormatter(contentType, req, _res) {
    ...
    // use NO formatter — EXCEPT for errors, which must
    // always be serialized.  Without this, a scope deny
    // on a GET/PUT object path passes the raw Error object
    // to res.write(), crashing Node v6 with
    ...
    req._binaryUpload || req._binaryOperation)) {
        return function passthroughFormatter(innerReq, innerRes, body) {
            if (body instanceof Error) {
                return (formatJSON(innerReq, innerRes, body));
            }
            return (body);
        };
    }
