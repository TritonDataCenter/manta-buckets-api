...
function createStsIamHandler(clients, iamClient, stsClient,
    ...
    return function stsRequestHandler(req, res, next) {
        ...
        // Body takes precedence per AWS Query API spec
        ...
        if (action === 'AssumeRole' || action === 'GetSessionToken' ||
            ...
            /*
             * Check for session token header. IAM operations cannot be
             * called with temporary credentials (MSAR from AssumeRole or
             * MSTS from GetSessionToken).
             */
            ...
            if (sessionToken) {
                ...
                // Return XML error response for IAM operations
                var errorXml =
                    '<?xml version="1.0" encoding="UTF-8"?>\n' +
                    '<ErrorResponse xmlns="https://iam.amazonaws.com/' +
                    'doc/2010-05-08/">\n' +
                    '  <Error>\n' +
                    '    <Type>Sender</Type>\n' +
                    '    <Code>InvalidClientTokenId</Code>\n' +
                    '    <Message>' +
                    s3Compat.escapeXml('The security token included in the ' +
                        'request is invalid.') +
                    '</Message>\n' +
                    '  </Error>\n' +
                    '  <RequestId>' + req.getId() + '</RequestId>\n' +
                    '</ErrorResponse>';

                res.setHeader('Content-Type', 'application/xml');
                res.send(403, errorXml);
                return (next(false));
            }
