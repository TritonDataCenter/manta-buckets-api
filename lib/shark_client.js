...
var STORAGE_LAYOUT_V2 = 2;
var STORAGE_LAYOUT_PREFIX_LEN = 2;
...
function SharkResponseError(res, body) {
    ...
    this.message = util.format('storage node failure:\nHTTP %d\n%s%s',
                               res.statusCode,
...
function _request(opts, cb) {
    ...
    function onResponse(res) {
        ...
        if (res.statusCode >= 400) {
            ...
            res.on('data', function accumulateBodyChunk(chunk) {
                body += chunk;
            });
            res.once('end', function handleErrorResponseEnd() {
                cb(new SharkResponseError(res, body), req, res);
            });
            res.once('error', function handleResponseError(err) {
    ...
    req.once('continue', function handleContinue() {
        clearTimeout(connectionTimer);
...
/**
 * Use information about the object to craft the storage path.
 *
 * An important detail to note is the use of the MD5 hash of the object name
 * when constructing the storage node file path rather than the actual name of
 * the object. Metadata placement uses the MD5 hash of the object name as input
 * to determine where to place an object's metadata record. This decision was
 * made in order to allow us to construct the storage file path such that all
 * the inputs to the metadata placement hash function are present on the
 * storage node and also maintain a predictable, fixed size file path. The
 * benefit of this information on the storage node is that it becomes possible
 * to determine the location of the metadata for a storage node file without
 * having to scan every metadata shard. The inputs can be fed into the hash
 * function in the same manner as is done in the getObjectLocation function in
 * the metadata_placement module.
 *
 * Another detail worth mentioning is the use of the STORAGE_LAYOUT_PREFIX_LEN.
 * This is a prefix of the object UUID and the purpose is to minimize the number
 * of files in any given directory on a storage node. There are some known
 * performance problems when a ZFS directory has more than around 20 million
 * files in it. The use of the object UUID prefix in the storage file path
 * allows us to be reasonably sure that we avoid this problem.
 *
 * @param {object} opts                      - options object
 * @param {number} opts.storageLayoutVersion - Storage layout scheme version
 * @param {uuid} opts.owner                  - Account owner UUID
 * @param {uuid} opts.bucketId               - Bucket UUID
 * @param {string} opts.objectNameHash       - MD5 hash of the object name
 * @param {uuid} opts.objectId               - Object UUID
 * @return {string}
 */
function storagePath(opts) {
    if (opts.storageLayoutVersion === STORAGE_LAYOUT_V2) {
        var storageLayoutVersion = '/v' + opts.storageLayoutVersion;
        var owner = opts.owner;
        var bucketId = opts.bucketId;
        var objectNameHash = opts.objectNameHash;
        var objectId = opts.objectId;
        var objectIdAndHash = objectId + ',' + objectNameHash;
        var objectIdPrefix = objectId.substr(0, STORAGE_LAYOUT_PREFIX_LEN);

        return ([storageLayoutVersion, owner, bucketId, objectIdPrefix,
            objectIdAndHash].join('/'));
    }

    throw new verror.VError('encountered unknown storage layout version: ' +
        opts.storageLayoutVersion);
}
...
function request(thisp, method, opts, cb) {
    ...
    if (opts.range) {
        defaultHeaders.range = opts.range;
    }
    ...
    var retry = backoff.call(_request, _opts,
        function handleBackoffRetry(err, req, res) {
        if (err) {
            cb(err);
        } else {
            log.debug({
...
function SharkClient(options) {
    ...
    // Debug shark information and fail fast if invalid
    if (!this.hostname) {
        options.log.error({
            shark: options.shark,
            sharkKeys: Object.keys(options.shark || {}),
            sharkStorageId: options.shark ? options.shark.manta_storage_id :
                'undefined'
        }, 'SharkClient: ERROR - undefined hostname from shark data');

        throw new
        Error('SharkClient: invalid shark data - missing manta_storage_id');
    }

    this.log = options.log.child({
        component: 'SharkClient',
        storage_hostname: self.hostname
    }, true);
    this.port = 80;
    ...
    this.close = once(function close() {
        ...
        Object.keys(sockets).forEach(function closeIdleSocketGroup(k) {
            sockets[k].forEach(function closeIdleSocket(s) {
                s.end();
            });
        });

        sockets = self.agent.sockets || {};
        Object.keys(sockets).forEach(function closeActiveSocketGroup(k) {
            sockets[k].forEach(function closeActiveSocket(s) {
                s.end();
            });
        });
    });
...
SharkClient.prototype.put = function put(opts, cb) {
    ...
    assert.uuid(opts.bucketId, 'options.bucketId');
    assert.uuid(opts.objectId, 'options.objectId');
    assert.string(opts.objectName, 'options.objectName');
    assert.string(opts.objectNameHash, 'options.objectNameHash');
    assert.uuid(opts.owner, 'options.owner');
    assert.string(opts.requestId, 'options.requestId');
    assert.number(opts.storageLayoutVersion, 'options.storageLayoutVersion');
    ...
    if (opts.contentLength !== undefined) {
        ...
        // Outbound write — we are the client here, constructing
        // our PUT to mako. Not reading an incoming client header,
        // so the unsigned-hint trap does not apply.
        _opts.headers['transfer-encoding'] = 'chunked';
    }

    if (opts.contentMd5 !== undefined)
        // Outbound write — see above.
...
module.exports = {
    STORAGE_LAYOUT_V2: STORAGE_LAYOUT_V2,
    STORAGE_LAYOUT_PREFIX_LEN: STORAGE_LAYOUT_PREFIX_LEN,
