...
function STSClient(options) {
    ...
    var baseOpts = {
        ...
        /*
         * 30s should be wide enough to ride out a UFDS LDAP pool refill (mass
         * disconnect + reconnect) without surfacing a RequestTimeout to the
         * caller. Mahi's stsAssumeRole / stsGetSessionToken handler does a
         * synchronous ufdsClient.add() inline on the request path; when the
         * pool is empty it blocks waiting for a connection. We observed handler
         * durations of 15-37 seconds (in COAL )during a single mass-disconnect
         * event. The earlier default of 10s turned every such event into a
         * customer-visible 500. AWS CLI's own retry budget covers the rare
         * casesthat exceed 30s.
         */
    ...
    /*
     * Use a cueball HTTP agent when provided.  This
     * resolves DNS via the Manta nameservice (binder)
     * instead of the system resolver, preventing
     * DNSTimeout after idle periods when the system
     * DNS cache expires.
     */
    if (options.agent) {
        baseOpts.agent = options.agent;
    }

    this.client =
        restifyClients.createJsonClient(baseOpts);
    /* String client for XML responses (GetCallerIdentity) */
    this.stringClient =
        restifyClients.createStringClient(baseOpts);
}
...
STSClient.prototype.assumeRole = function assumeRole(opts, callback) {
    ...
    var requestBody = {
        ...
        caller: {
          ...
          /**
           * @type {string|null} bucketScope - Inherited from parent
           * access key; restricts temp creds to the named bucket.
           */
          bucketScope: opts.caller.bucketScope || null
      }
    };
