...
function STSClient(options) {
    ...
    if (Array.isArray(options.resolvers) &&
        ...
        jsonOpts.resolvers = options.resolvers;
    }
    this.client = restifyClients.createJsonClient(jsonOpts);
    ...
    if (Array.isArray(options.resolvers) &&
        options.resolvers.length > 0) {
        strOpts.resolvers = options.resolvers;
    }
    this.stringClient = restifyClients.createStringClient(strOpts);
}
