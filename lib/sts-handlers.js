 ...
 */
/**
 * @brief Create STS client instance
 *
 * @param {Object} config - Server configuration
 * @param {Object} log - Bunyan logger
 * @param {Object} [agent] - Cueball HTTP agent for
 *   DNS resolution via binder.  Without this, the
 *   restify client falls back to system DNS which
 *   times out on Manta-internal hostnames after the
 *   DNS cache expires.
 */
function createSTSClient(config, log, agent) {
...
function assumeRoleHandler(stsClient) {
    return function handleAssumeRole(req, res, next) {
        ...
        // AWS STS uses POST with body params; body takes precedence.
        var roleArn = req.body.RoleArn || req.query.RoleArn;
        var roleSessionName = req.body.RoleSessionName ||
            req.query.RoleSessionName;
        var durationSeconds = req.body.DurationSeconds ||
            req.query.DurationSeconds;
