...
function buildAssumeRoleXMLResponse(result) {
    ...
    xml += '    <Credentials>\n';
    xml += '      <AccessKeyId>' + escapeXml(credentials.AccessKeyId) + '</AccessKeyId>\n';
    xml += '      <SecretAccessKey>' + escapeXml(credentials.SecretAccessKey) + '</SecretAccessKey>\n';
    xml += '      <SessionToken>' + escapeXml(credentials.SessionToken) + '</SessionToken>\n';
    xml += '      <Expiration>' + escapeXml(credentials.Expiration) + '</Expiration>\n';
    xml += '    </Credentials>\n';
    xml += '    <AssumedRoleUser>\n';
    xml += '      <AssumedRoleId>' + escapeXml(assumedRoleUser.AssumedRoleId) + '</AssumedRoleId>\n';
    xml += '      <Arn>' + escapeXml(assumedRoleUser.Arn) + '</Arn>\n';
    xml += '    </AssumedRoleUser>\n';
    xml += '  </AssumeRoleResult>\n';
