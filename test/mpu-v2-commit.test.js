...
test('integration with existing MPU', function (t) {
    ...
    helper.createBucket(server, bucketName, function (createErr) {
        t.error(createErr, 'bucket creation should succeed');

        helper.initiateMPU(server, bucketName, objectName,
                           function (initErr, uploadId) {
            t.error(initErr, 'MPU initiation should succeed');
            t.ok(uploadId, 'upload ID should be returned');

            // Clean up
            helper.abortMPU(server, bucketName, objectName, uploadId,
                            function (abortErr) {
